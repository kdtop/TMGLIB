...
GETNAMES(OUT,TMGDFN) ;"Get prepped names, including aliases
  ...
  . IF SHOULDGARBLE SET ALIAS=$$GARBLENAME^TMGMISC4(ALIAS)
  . SET OUT(ALIAS)=""
  QUIT
  ;
PREPNAME(NAME,OUTLNAME,OUTFNAME) ;"Format names into desired format
  ...
  SET OUTLNAME=$TR(OUTLNAME,"'","")  ;"REMOVE APOS FOR FOLDER CREATION
  SET OUTFNAME=$TR(OUTFNAME,"'","")  ;"REMOVE APOS FOR FOLDER CREATION
  QUIT
  ;
