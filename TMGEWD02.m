TMGEWD02 ;TMG/kst/TMG Local version of %zewdHTMLParser; 2/26/23
         ;;1.0;TMG-LIB;**1**;2/26/23

 ;
 ;"NOTICE: This code is copied from EWD library, with orginal license as below.
 ;"        Only CHANGES to this code would come under Toppenberg copyright/license
 ;"          as below.  
 ;"        I am moving this into TMG namespace primarily to allow edits to 
 ;"          not conflict with the rest of the EWD package and to include
 ;"          changes with backup to GitHub with the rest of the TMG library. 
 ;"        Also note, that parts of the EWD library that call to %zewdHTMLParser,
 ;"          will NOT call into this file.  
 ;"        This file still calls OUT to other parts of the %zewd code base. 
 ;"~--~--~--~--~--~--~--~--~--~--~--~--~--~--~--~--~--~--~--~--~--~--~--~--
 ;"Copyright (c) 2/36/2023  Kevin S. Toppenberg MD
 ;"
 ...
 ;"parseURL(server,getPath,docName,port,isHTML,responseTime,browserType,post,maxLineLength,headers,sslHost,sslPort)
 ;"importFile(filepath,maxLength)
 ;"gtmImportFile(filepath)
 ;"importNotExists
 ;"parseDocument(docName,isHTML)
 ;"addChild(%line,parentOID,docOID,nextLine,%buf,%error,impliedClose,isHTML)  ;"//kt mod -- original below
 ;"addChSub ; //kt mod
 ;"hasChildren(nodeOID)
 ;"hasText(nodeOID)
 ;"tidyText(text)
 ...
 ;"closeLI(tagName,buf)
 ;"getLineFromBuffer(isText,tagName,buf,nextLine,textArray) ;" //kt nod -- original below
 ;"getCommentText(buf)
 ;"getCDATAText(buf)
 ;"getScriptText(buf,tagname,nextLine,textArray)
 ;"getTagFromBuffer(buf)
 ;"ENSUREBUF(buf,nextLine) ;"//kt added
 ;"getTextFromBuffer(buf,nextline,textArray) ;"//kt mod -- original below
 ;"extractChar(buf)
...
parseURL(server,getPath,docName,port,isHTML,responseTime,browserType,post,maxLineLength,headers,sslHost,sslPort)
 ...
 i port'="" s getPath=":"_port_getPath
 s http="http://"
 i $g(sslPort)'="" s http="https://"
 s url=http_server_getPath
 i $g(headers("Cookie"))'="" s reqHeaders(1)="Cookie: "_headers("Cookie")
 k headers
 i $g(responseTime)="" s responseTime=10
 s ok=$$httpGET^%zewdGTM(url,.html,.reqHeaders,responseTime,,,.headers,$g(sslHost),$g(sslPort))
...
parseDocument(docName,isHTML)
 ...
 ;"//kt i nLines>1 f lineNo=2:1:nLines d
 set lineNo=1  ;"//kt rewrote much of section below
 f  set lineNo=$order(^CacheTempEWD($j,lineNo)) quit:lineNo'>0  do   ;//kt mod
 . new prevLineNo set prevLineNo=$order(^CacheTempEWD($j,lineNo),-1)
 . new nextLineNo set nextLineNo=+$order(^CacheTempEWD($j,lineNo)) if nextLineNo'>0 set nextLineNo=lineNo+1  ;"//kt
 . s %line=$GET(^CacheTempEWD($j,lineNo)) QUIT:%line=""  ;"//kt mod
 . s np=$l(%line,dlim)
 . s prevLine=$GET(^CacheTempEWD($j,prevLineNo))
 . if np>1 do   ;"//kt added
 . . s %p1=$p(%line,dlim,1)
 . . s %p2=$p(%line,dlim,2,np)
 . . set prevLine=prevLine_%p1
 . . set %line=%p2
 . . s ^CacheTempEWD($j,prevLineNo)=prevLine
 . . s ^CacheTempEWD($j,lineNo)=%line
 . i $$rightStr(prevLine,$length(dlim))=dlim do
 . . set prevLine=$extract(prevLine,1,$length(prevLine)-$length(dlim))
 . . set ^CacheTempEWD($j,prevLineNo)=prevLine
 . set np=$length(prevLine,"<") quit:np=1
 . set %p1=$piece(prevLine,"<",1,np-1)
 . set %p2="<"_$piece(prevLine,"<",np)
 . if %p2[">" quit
 . if %p1="" do  quit   ;"prevLine starts with <, but doesn't have closing >
 . . if %line'[">" quit
 . . set %p1=$piece(%line,">",1)_">"
 . . set prevLine=prevLine_%p1,^CacheTempEWD($j,prevLineNo)=prevLine
 . . set %line=$piece(%line,">",2,9999999),^CacheTempEWD($j,lineNo)=%line
 . else  do
 . . set prevLine=%p1,^CacheTempEWD($j,prevLineNo)=prevLine
 . . set %line=%p2_%line,^CacheTempEWD($j,lineNo)=%line
 ;
 s nextLine=1
 ;"//kt original --> f  s %line=$$getLineFromBuffer(0,"",.buf,.nextLine) q:$$isOpenTag(%line)
 NEW DONE SET DONE=0
 f  do  QUIT:DONE
 . s %line=$$getLineFromBuffer(0,"",.buf,.nextLine) 
 . SET DONE=$$isOpenTag(%line)!((buf="")&(%line=""))  ;"//kt added !((buf="")&(%line=""))
 s %error=""
 ...
 Q $g(%error)
 ;
rightStr(str,count) ;"//kt added
 new len set len=$length(str)
 quit $extract(str,len-count+1,len)
 ;
addChild(%line,parentOID,docOID,nextLine,%buf,%error,impliedClose,isHTML)  ;"//kt mod -- original below
 ;
 ...
 . . set %stop=1
 . i i>16000 do  quit
 . . s %error="Unable to parse page just before "_$e(%buf,1,400)_$c(13,10)_"Hint : check for improperly matched <tr> and/or <td> tags" 
 . . set %stop=1
 QUIT
 ;
addChSub ; //kt mod
 ;"NOTE: to be called only from addChild.  Uses everything from that scope, in global scope.  
 s %text="" 
 ;
 ...
 . i tagName=("/"_$$getTagName^%zewdDOM($GET(childOID))),$$hasText($GET(childOID)) s %stop=0  ;"//kt mod, added $GET()
 ;
 i $$empty(.%attr) d  Q
 ...
 ;
 s isText=("^script^style^textarea^csp:method^server^ewd:javascript^"["^"_tagName_"^")
 s %line2=$$getLineFromBuffer(isText,tagName,.%buf,.nextLine,.textArray)
 ;"//kt original --> s %text="" i %line2'="**mgwArray***",$$isText(%line2) s %text=%line2 i 'isText s %text=$$tidyText(%line2) ; text found for tag
 s %text="" i %line2'="**mgwArray***",$$isText(%line2) do
 . s %text=%line2 
 . i 'isText s %text=$$tidyText(%line2) ; text found for tag
 ;
...
addTag(tagName,parentOID,docOID,%attr,%text,textArray)
 ...
 QUIT childOID
 ;
isOpenTag(%line)  ;"//kt doc.  Return TRUE (1) if %line starts with open tag, e.g. '<'
 ;f  q:$e(%line,1)?1ANP  d
...
getLineFromBuffer(isText,tagName,buf,nextLine,textArray) ;" //kt nod -- original below
 ...
 ;"       tagName   -- may be tag name or ""
 ;"       buf       -- current buffer
 ;"       nextLine  -- PASS BY REFERENCE  This is index of NEXT line in ^CacheTempEWD($j, to get data from
 ;"       textArray -- OPTIONAL.  PASS BY REFERENCE  An array of text that may be filled
 ;
 k textArray
 DO ENSUREBUF(.buf,.nextLine)  ;"//kt
 NEW RESULT SET RESULT=""
 ;
 IF isText d  
 . n buf1 s buf1=$$stripSpaces^%zewdAPI(buf)
 ...
 . SET RESULT=$$getScriptText(.buf,tagName,.nextLine,.textArray)
 ELSE  IF $e(buf,1,4)="<!--" DO
 . SET RESULT=$$getCommentText(.buf)
 ELSE  IF $e(buf,1)="<" DO
 . SET RESULT=$$getTagFromBuffer(.buf)
 ELSE  DO
 . SET RESULT=$$getTextFromBuffer(.buf,.nextLine,.textArray)
 QUIT RESULT 
 ;
getCommentText(buf)
 ;
...
getScriptText(buf,tagname,nextLine,textArray)
 ...
 . ;"//kt added mod
 . IF buf="" DO
 . . NEW OK SET OK=$$ENSUREBUF(.buf,.nextLine)
 . . IF OK=0 SET %stop=1
 ;
 i $d(textArray) d
 . n lastLine
...
getTagFromBuffer(buf)
 ...
 ;
ENSUREBUF(buf,nextLine) ;"//kt added
 ;"Purpose: to fill buf from ^CacheTempEWD($j
 ;"Input:  buf -- IN AND OUT PARAMETER.  PASS BY REFERENCE.
 ;"           likely comes in with information
 ;"        nextLine  -- PASS BY REFERENCE  This is index of NEXT line in ^CacheTempEWD($j, to get data from
 ;"Result: 1 if OK, or 0 if ^CacheTempEWD($j,nextLine) is empty (i.e. end of file)
 ;
 NEW RESULT SET RESULT=1
 FOR  QUIT:($LENGTH(buf)>2000)!(RESULT=0)  DO 
 . SET buf=buf_$GET(^CacheTempEWD($j,nextLine))
 . NEW TEMP SET TEMP=$ORDER(^CacheTempEWD($j,nextLine))
 . SET nextLine=nextLine+1
 . IF TEMP="" SET RESULT=0 QUIT
 . SET nextLine=TEMP
 QUIT RESULT
 ;
getTextFromBuffer(buf,nextline,textArray) ;"//kt mod -- original below
 ;"Called only from getLineFromBuffer()
 ;"Purpose: To fill up textArray with text -- i.e. all characters up to, but not including, an HTML tag
 ;"Input:  buf       -- PASS BY REFERENCE  This is the working buffer, will pass out residual
 ;"        nextLine  -- PASS BY REFERENCE  This is index of NEXT line in ^CacheTempEWD($j, to get data from
 ;"        textArray -- PASS BY REFERENCE  This is array of text to be filled
 ;"Result: returns text up to, but not including '<'
 ;"        Will also fill textArray[] with separated lines (i.e. for javascript etc) 
 ;
 n text,pos,tag,i
 ;
 k textArray
 ...
 NEW DONE SET DONE=0
 FOR  DO  QUIT:DONE
 . IF (buf=""),OK=0 SET DONE=1 QUIT
 . SET OK=$$ENSUREBUF(.buf,.nextLine)  ;"//kt
 . IF buf["<" DO  QUIT
 . . NEW PARTA SET PARTA=$PIECE(buf,"<",1)
 . . SET pos=$FIND(buf,"<")
 . . SET buf=$EXTRACT(buf,pos-1,$l(buf))  ;"//kt keep all the characters from < forward in buf
 . . IF PARTA'="" DO
 . . . ;"IF text="" SET text="**mgwArray***"  ;"signal of data in textArray
 . . . SET textArray(IDX)=PARTA,IDX=IDX+1
 . . . SET text=text_PARTA
 . . SET DONE=1
 . ELSE  DO
 . . ;"IF text="" SET text="**mgwArray***"  ;"signal of data in textArray
 . . SET textArray(IDX)=buf,IDX=IDX+1
 . . SET text=text_buf
 . . SET buf=""
 QUIT text
 ;
...
parseElement(%line,%attr,%error,isHTML)
 ...
 s %apos=""
 s %stop=0
 SET isSVG=$get(isSVG)  ;"//kt added <-- isSVG used in global scope.  
 ;
