...
GETPROV(TMGRESULT,TMGDFN,TMGDUZ) ;"-- Get the patient's current provider (RPC: TMG CPRS GET CURRENT PROVIDER)
 ...
 . SET TMGRESULT=DUZ
 IF +$P($G(^DPT(TMGDFN,"TMGPCP")),"^",1)>0 DO  GOTO GPDN
 . SET TMGRESULT=+$P($G(^DPT(TMGDFN,"TMGPCP")),"^",1)
 SET TMGRESULT=$$FINDPCP(TMGDFN)
 IF TMGRESULT="-1" SET TMGRESULT=TMGDUZ
GPDN
 QUIT
 ;"
...
AUTHRCNT(TMGDFN,AUTHORARR,TARGETCT) ;"Find last 100 notes for patient, and get total for each author
 ...
 NEW OVIEN,PCEIEN  ;"WE ARE HARD CODING THESE IENS FOR USE IN TESTING FOR NOW 11/12/18
 SET OVIEN=1408,PCEIEN=1402
 ...
 . ;"END 4/4/19 CHANGE
 . IF (TIUTYPE'=OVIEN)&(TIUTYPE'=PCEIEN) QUIT
