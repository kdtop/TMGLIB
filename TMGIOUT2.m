TMGIOUT2 ;TMG/kst/IO Utilities -- File browser ;05/16/09; 6/18/25
...
L1 ;
  ...
  . IF TMGSELECT=CURDIR DO  QUIT
  . . WRITE !,"Choose ["_CURDIR_"] as selected directory"
  ...
  IF $$ISDIR(TMGSELECT,.OPTION) SET CURDIR=$$MKTRALDV^TMGIOUTL(TMGSELECT,NODEDIV) GOTO L1 ;"browse into directory
...
LD2 ;
  SET INDEX=""
  IF SELDIR DO
  . SET @PARRAY@(COUNT,".  <CURRENT DIR>")=CURDIR
  . SET COUNT=COUNT+1
