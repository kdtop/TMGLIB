...
GTPOPRPT(OUT,ADUZ,ALOC,PARAMS,SORT,INACTIVE,REFRESH,NUMRESULTS,DIRECTION,LASTDISPLAYED)  ;"RPC ENTRY POINT: TMG CPRS GET POP HEALTH REPORT
  ...
  SET OPTION("INCLUDE INACTIVE")=+$GET(INACTIVE)
  SET MODE=$PIECE(PARAMS,";",1),PARAM1=$PIECE(PARAMS,";",2),PARAM2=$PIECE(PARAMS,";",3)
  ;"
  ...
  ;"DO TESTPATS(.PTDUEARR,.PATARR,.REMARR,ADUZ)
...
DISPREPORT   ;"HERE IS WHERE WE WILL DISPLAY THE ACTUAL REPORT
  ;"NOTE: This is a label inside GTPOPRPT, and thus inherits its scope
  ...
  . FOR  SET TMGNAME=$ORDER(PTDUEARR("P",TMGNAME)) QUIT:TMGNAME=""  DO
  . . ;"SET OUT(OUTIDX)=TMGNAME,OUTIDX=OUTIDX+1
  . . SET TMGDFN=0
  . . FOR  SET TMGDFN=$ORDER(PTDUEARR("P",TMGNAME,TMGDFN)) QUIT:TMGDFN'>0  DO
  . . . SET REMIEN=0
  . . . SET TOTAL=TOTAL+1
  ...
  . . . . FOR  SET REMIEN=$ORDER(PTDUEARR("P",TMGNAME,TMGDFN,REMIEN)) QUIT:REMIEN'>0  DO
  . . . . . IF REMSTR'="" SET REMSTR=REMSTR_"<br>"
  . . . . . SET REMSTR=REMSTR_$PIECE($GET(^PXD(811.9,REMIEN,0)),"^",1)
  . . . . DO ADDONE(.OUT,.OUTIDX,TMGNAME,TMGDFN,REMSTR,SORT,0)
  ELSE  IF SORT=1 DO
  . ;"SET OUT(OUTIDX)="SORTING 1",OUTIDX=OUTIDX+1
  . FOR  SET REMIEN=$ORDER(PTDUEARR("R",REMIEN)) QUIT:REMIEN'>0  DO
  . . SET TMGNAME=""
  . . FOR  SET TMGNAME=$ORDER(PTDUEARR("R",REMIEN,TMGNAME)) QUIT:TMGNAME=""  DO
  . . . SET TMGDFN=0
  . . . FOR  SET TMGDFN=$ORDER(PTDUEARR("R",REMIEN,TMGNAME,TMGDFN)) QUIT:TMGDFN'>0  DO
  . . . . SET TOTAL=TOTAL+1
  . . . . IF TOTAL=FIRST SET STARTLIST=1
  . . . . IF (STARTLIST=1)&(DONE=0) DO
  ...
  . . . . . IF TOTAL=LASTDISPLAYED SET DONE=1
  . . . . . SET LAST=TOTAL
  . . . . . DO ADDONE(.OUT,.OUTIDX,TMGNAME,TMGDFN,$PIECE($GET(^PXD(811.9,REMIEN,0)),"^",1),SORT,REMIEN)
  IF (FIRST'=1)!(TOTAL'=LAST) SET PARTIAL=1
  SET OUT(OUTIDX)="</table></body></html>"
  NEW REF SET REF=$$RPTREF($J,ADUZ)
  NEW STATUS SET STATUS=$GET(@REF@("STATUS"))
...
GETPTBOLUS(OUT,ADUZ,ALOC,START,END,MODE,OPTION) ;"Get list of patients for Reminder Dashboard 
  ...
  NEW INACTIVE SET INACTIVE=+$GET(OPTION("INCLUDE INACTIVE"))
  ...
  . FOR  SET NAME=$ORDER(^DPT("B",NAME)) QUIT:(NAME="")!($ASCII($E(NAME,1,1))>$ASCII(PARAM2))  DO
  . . SET TMGDFN=0
  . . FOR  SET TMGDFN=$ORDER(^DPT("B",NAME,TMGDFN)) QUIT:TMGDFN'>0  DO
  . . . IF INACTIVE=1 DO
  . . . . SET TEMPARR(TMGDFN)=""
  ...
  QUIT
  ;
JTESTPAT(PARENTJOB,ADUZ)
  NEW REF SET REF=$$RPTREF(PARENTJOB,ADUZ)
  SET @REF@("STATUS")="JOB LAUNCHED"
  NEW PATARR,PTDUEARR,REMARR
  MERGE PATARR=^TMP(PARENTJOB,"PATARR")
  NEW PTCOUNT SET PTCOUNT=0
  NEW DFN SET DFN=0
  FOR  SET DFN=$ORDER(PATARR(DFN)) QUIT:DFN'>0  DO
  . SET PTCOUNT=PTCOUNT+1
  SET PATARR("TOTAL COUNT")=PTCOUNT
  MERGE REMARR=^TMP(PARENTJOB,"REMARR")
  DO DT^DICRW  ;"ensure Fileman environment.
  ;" NEW U SET U="^"
  IF $DATA(DUZ)=0 SET DUZ=ADUZ
  DO TESTPATS(.PTDUEARR,.PATARR,.REMARR,PARENTJOB,ADUZ)
  QUIT
  ;"
RPTREF(AJOB,ADUZ) ;"GET REF TO STORAGE LOCATION FOR POP HEALTH.
  QUIT $NAME(^TMP("POP HEALTH REPORT",AJOB,ADUZ))
  ;
TESTPATS(PTDUEARR,PATARR,REMARR,PARENTJOB,ADUZ) ;"This function will test each patient against each Reminder
 NEW PATCOUNT SET PATCOUNT=$GET(PATARR("TOTAL COUNT"))
 NEW TODAY SET TODAY=$$TODAY^TMGDATE
 KILL PATARR("TOTAL COUNT")
 NEW REF SET REF=$$RPTREF(PARENTJOB,ADUZ)
 NEW SHOULDGARBLE SET SHOULDGARBLE=$$SHOULDGARBLE^TMGMISC4(ADUZ) 
 SET @REF@("STATUS2")="SHOULDGARBLE="_SHOULDGARBLE_", and ADUZ="_$GET(ADUZ)
 NEW CURPAT SET CURPAT=0
 SET @REF@("STATUS")="UPDATING"
 NEW TMGDFN SET TMGDFN=0
 FOR  SET TMGDFN=$ORDER(PATARR(TMGDFN)) QUIT:TMGDFN'>0  DO
 . SET CURPAT=CURPAT+1
 . SET @REF@("STATUS")="RUNNING #"_CURPAT_" OF "_PATCOUNT
 . NEW PATNAME SET PATNAME=$PIECE($GET(^DPT(TMGDFN,0)),"^",1)
 . IF SHOULDGARBLE SET PATNAME=$$GARBLENAME^TMGMISC4(PATNAME) 
 . NEW REMIEN SET REMIEN=0
 . FOR  SET REMIEN=$ORDER(REMARR(REMIEN)) QUIT:REMIEN'>0  DO
 . . ;"SET @REF@("STATUS")="TESTING "_REMIEN_" ON "_PATNAME
 . . NEW X DO NOW^%DTC       
 . . NEW REMRESULT SET REMRESULT=$$DOREM^TMGPXR03(TMGDFN,REMIEN,5,X)  ;"This will run the reminder and return "DUE NOW" if due
 . . ;"SET @REF@("STATUS")="FINISHED TEST "_REMIEN_" ON "_PATNAME
 . . ;"SET @REF@("REM STATUS")=REMRESULT
 . . ;"SET ^EDDIE("POPHEALTH",PATNAME,REMIEN)=REMRESULT
 . . IF (REMRESULT["DUE NOW")!(REMRESULT["DUE SOON") DO
 . . . SET PTDUEARR("P",PATNAME,TMGDFN,REMIEN)=""   ;"P FOR PATIENT INDEX
 . . . SET PTDUEARR("R",REMIEN,PATNAME,TMGDFN)=""   ;"R FOR REMINDER INDEX
 . . . MERGE @REF=PTDUEARR  ;"KEEP UPDATING THE GLOBAL
 SET @REF@("STATUS")="COMPLETED"
 QUIT  
  ;"=======================================================================================================================================================================================
...
GETRLIST(OUT,ADUZ,ORLOC) ;Returns a list of all cover sheet reminders
  ...
  N I,ORY
  D REMLIST(.ORY,ADUZ,$GET(ORLOC))
  S I=0
  F  S I=$ORDER(ORY(I)) Q:'I  D
  .S ORY(I)=$PIECE(ORY(I),U,2)
  NEW REMIEN,OUTIDX,TEMPIDX
  SET TEMPIDX=0,OUTIDX=1
  FOR  SET TEMPIDX=$ORDER(ORY(TEMPIDX)) QUIT:TEMPIDX'>0  DO
  . SET OUT($GET(ORY(TEMPIDX)))="",OUTIDX=OUTIDX+1
  QUIT
  ;"
REMLIST(ORY,ADUZ,LOC)   ;"  THIS IS COPIED FROM REMLIST^ORQQPX, so the DUZ could be a parameter rather than assumed
  ...
  ;S SRV=$PIECE($GET(^VA(200,ADUZ,5)),U)
  S SRV=$$GET1^DIQ(200,ADUZ,29,"I")
  D NEWCVOK^ORQQPX(.NEWP)
  I 'NEWP D GETLST^XPAR(.ORY,"USR^LOC.`"_$GET(LOC)_"^SRV.`"_+$GET(SRV)_"^DIV^SYS^PKG","ORQQPX SEARCH ITEMS","Q",.ORERR) Q
  D REMACCUM(.ORLST,ADUZ,"PKG","Q",1000)
  D REMACCUM(.ORLST,ADUZ,"SYS","Q",2000)
  D REMACCUM(.ORLST,ADUZ,"DIV","Q",3000)
  I +SRV D REMACCUM^ORQQPX(.ORLST,"SRV.`"_+$GET(SRV),"Q",4000)
  I +LOC D REMACCUM^ORQQPX(.ORLST,"LOC.`"_+$GET(LOC),"Q",5000)
  D REMACCUM(.ORLST,ADUZ,"CLASS","Q",6000)
  D REMACCUM(.ORLST,ADUZ,"USR","Q",7000)
  S I=0
  F  S I=$ORDER(ORLST(I)) Q:'I  D
  .S IDX=$PIECE(ORLST(I),U,1)
  .F  Q:'$D(ORY(IDX))  S IDX=IDX+1
  .S CODE=$E($PIECE(ORLST(I),U,2),2)
  .S IEN=$E($PIECE(ORLST(I),U,2),3,999)
...
REMACCUM(ORY,ADUZ,LVL,TYP,SORT,CLASS) ; Accumulates ORTMP into ORY ;"  THIS IS COPIED FROM REMACCUM^ORQQPX, so the DUZ could be a parameter rather than assumed
 ...
 .S I=0,M=0,CLASS=$GET(CLASS)
 .F  S I=$ORDER(ORLST(I)) Q:'I  D
 ..S ORCLS=$PIECE(ORLST(I),U,1)
 ..I +CLASS S ADD=(ORCLS=+CLASS) I 1
 ..E  S ADD=$$ISA^USRLM(ADUZ,ORCLS,.ORERR)
 ..I +ADD D
 ...D GETWP^XPAR(.ORWP,"SYS",ORCLSPRM,ORCLS,.ORERR)
 ...S K=0
 ...F  S K=$ORDER(ORWP(K)) Q:'K  D
 ....S M=M+1
 ....S J=$PIECE(ORWP(K,0),";",1)
 ....S ORTMP(M)=J_U_$PIECE(ORWP(K,0),";",2)
 E  D GETLST^XPAR(.ORTMP,LVL,"ORQQPX COVER SHEET REMINDERS",TYP,.ORERR)
 S I=0,IDX=$ORDER(ORY(999999),-1)+1,ADD=(SORT="")
 F  S I=$ORDER(ORTMP(I)) Q:'I  D
 .S (FOUND,J)=0,P2=$PIECE(ORTMP(I),U,2)
 .S FLAG=$E(P2),IEN=$E(P2,2,999)
 .I ADD S DOADD=1
 .E  D
 ..S DOADD=0
 ..F  S J=$ORDER(ORY(J)) Q:'J  D  Q:FOUND
 ...S P2=$PIECE(ORY(J),U,2)
 ...S FIEN=$E(P2,2,999)
 ...I FIEN=IEN S FOUND=J,FFLAG=$E(P2)
 ..I FOUND D  I 1
 ...I FLAG="R",FFLAG'="L" K ORY(FOUND)
 ...I FLAG'=FFLAG,(FLAG_FFLAG)["L" S $E(P2)="L",$PIECE(ORY(FOUND),U,2)=P2
 ..E  I (FLAG'="R") S DOADD=1
 .I DOADD D
 ..S OUT(IDX)=ORTMP(I)
 ..S $PIECE(OUT(IDX),U)=$PIECE(OUT(IDX),U)_SORT
