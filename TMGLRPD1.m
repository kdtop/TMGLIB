TMGLRPD1 ;TMG/kst-Entry point for storing lab result PDFs ; 10/19/20, 3/24/21
...
STOREPDF(TMGHL7MSG,MSGINFO)  ;"store base-64-encoded PDF
  ...
  ;"====== Store metadata =========
  IF $DATA(^TMG(22745,TMGDFN)) GOTO L1
  ;"-- make top level record
  SET TMGFDA(22745,"+1,",.01)=TMGDFN
...
STRPDFDN ;
   QUIT TMGRESULT
   ;
GETRELPATH(TMGDFN) ;"get a file path, based on DFN.  Will create path on HFS if doesn't exist
  ;"Result: Returns file path, or -1^Message if error
  NEW PADLEN SET PADLEN=$SELECT($LENGTH(TMGDFN)>9:12,$LENGTH(TMGDFN)>6:9,1:6)
  NEW DFNSTR SET DFNSTR=$$RJ^XLFSTR(TMGDFN,PADLEN,"0")
  NEW PATH SET PATH=""
...
GETROOTPATH() ;
  ...
  ;
RPCHASPDF(OUT,TMGDFN,SDT,EDT)  ;"RPC FOR HAS LAB PDF for given date range?
  ...
  IF $DATA(^TMG(22745,TMGDFN))=0 GOTO RPCDN
  ...
  NEW IDX SET IDX=1
  ;"FOR  SET ADT=$ORDER(^TMG(22745,TMGDFN,1,"ATMGDT",ADT)) QUIT:(ADT'>0)!(ADT>EDT)  DO
  ;"THE ABOVE LINE WASN'T HANDLING THE ORDER CORRECTLY SINCE SOME DATES (WITHOUT TIME) GET ORDERED FIRST AS THEY ARE TREATED
  ;"    NUMERICALLY, THEN THE ONES WITH TIME ARE TREATED LIKE ALPHANUMERIC. THEREFORE IF A DATE EXISTS WITHOUT A TIME AND IS GREATER THAN
  ;"    THE END DATE... NO RESULTS WILL BE RETURNED          ELH     3/29/21
  FOR  SET ADT=$ORDER(^TMG(22745,TMGDFN,1,"ATMGDT",ADT)) QUIT:(ADT'>0)  DO      	  
  . IF (ADT<SDT)!(ADT>EDT) QUIT  ;"Added this as extra check because $O above wasn't handling the FMDate time correctly
  . NEW SUBIEN SET SUBIEN=0
  . FOR  SET SUBIEN=$ORDER(^TMG(22745,TMGDFN,1,"ATMGDT",ADT,SUBIEN)) QUIT:SUBIEN'>0  DO
  . . NEW FNAME SET FNAME=$GET(^TMG(22745,TMGDFN,1,SUBIEN,1))
  . . NEW RELPATH SET RELPATH=$GET(^TMG(22745,TMGDFN,1,SUBIEN,2))
...
DOWNLOADLABPDF(GREF,RELPATH,FNAME)  ; DOWNLOAD A LAB PDF
  ...
  NEW TMGDEBUG SET TMGDEBUG=0
  IF TMGDEBUG=0 DO
  . KILL ^TMG("TMP","RPC","DOWNLOADLABPDF^TMGLRPD1")
  . MERGE ^TMG("TMP","RPC","DOWNLOADLABPDF^TMGLRPD1","RELPATH")=RELPATH
  . MERGE ^TMG("TMP","RPC","DOWNLOADLABPDF^TMGLRPD1","FNAME")=FNAME
  ELSE  DO
  . KILL RELPATH,FNAME
  . MERGE RELPATH=^TMG("TMP","RPC","DOWNLOADLABPDF^TMGLRPD1","RELPATH")
  . MERGE FNAME=^TMG("TMP","RPC","DOWNLOADLABPDF^TMGLRPD1","FNAME")
  ;  
  SET FNAME=$GET(FNAME)
  SET GREF="^TMP(""DOWNLOADPDF^TMGLRPD1"","_$J_")"
  KILL @GREF
  IF $$SHOULDGARBLE^TMGMISC4() DO      ;"check for special mode to hide patient info during demos
  . SET RELPATH="000/000/"
  . SET FNAME="confidental.pdf"  
  NEW ROOTPATH SET ROOTPATH=$$GETROOTPATH() ;
  NEW FULLPATH SET FULLPATH=ROOTPATH_RELPATH
  ...
  QUIT
  ;
UPLDPDF(TMGRESULT,PDF,FNAME,TMGDFN,LABFMDT)  ;"RPC TO UPLOAD A LAB PDF
  ...
  ;  
  ;"====== Store metadata =========
  IF $DATA(^TMG(22745,TMGDFN)) GOTO L2
  ;"-- make top level record
  NEW LRDFN SET LRDFN=+$G(^DPT(TMGDFN,"LR"))
