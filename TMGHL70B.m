...
GETMAP(TMGENV,TEST,MODE,OUT) ;
  ...
  ;"        OUT("PREMAP","TEST")=TEST
  ;"        OUT("PREMAP","TESTID")=TESTID
  ;"        OUT("PREMAP","TESTNAME")=TESTNAME
  ;"        OUT("PREMAP","ALT TESTID")=ALTTESTID
  ;"        OUT("PREMAP","ALT TESTNAME")=ALTTESTNAME  
  ...
  SET OUT("PREMAP","TEST")=TEST
  SET OUT("PREMAP","TESTID")=TESTID
  SET OUT("PREMAP","TESTNAME")=TESTNAME
  SET OUT("PREMAP","ALT TESTID")=ALTTESTID
  SET OUT("PREMAP","ALT TESTNAME")=ALTTESTNAME
  ;"
GM0 ;
  KILL TEMPOUT
...
FIX60(TMGENV,TEST,INFO,IEN60,MODE) ;"60=LABORATORY TEST
  ...
  IF ALTNAME="",$PIECE(TEST,TMGU(2),9)]"" SET ALTNAME=$PIECE(TEST,TMGU(2),9)  ;"Ballad seems to be putting alt name in piec 9
  ;"IF TESTNAME="",TESTCODE'="" SET TESTNAME=TESTCODE
  NEW MAXLEN SET MAXLEN=60  ;"//kt 3/14/22
  NEW LONGNAME SET LONGNAME=""
  NEW OPTION SET OPTION("MAXLEN")=MAXLEN,OPTION("INDENTN")=INDENTN+2 
  IF ALTNAME]"" SET OPTION("ALTNAME",ALTNAME)=""
  SET CODE="$$EDITNAME^TMGHL70B("""_TESTNAME_""",.OPTION)"
  NEW EDITEDNAME SET EDITEDNAME=$$EXEC(.TMGENV,CODE)
  IF +EDITEDNAME=-1 SET TMGRESULT=EDITEDNAME GOTO F60DN  ;"abort or error
  IF EDITEDNAME'=TESTNAME DO
  . SET LONGNAME=TESTNAME
  . SET TESTNAME=EDITEDNAME
  . SET INFO("SHORTNAME")=TESTNAME
  . SET $PIECE(TEST,"^",2)=TESTNAME
  IF 0=1,$LENGTH(TESTNAME)>30 DO  GOTO:(+TESTNAME<0) F60DN  ;"DEPRECIATED 2/5/22
  . NEW TEMP SET TEMP=TESTNAME 
  . ;"//KT 2/4/22 ORIGINAL --> IF $DATA(INFO("SHORTNAME")) SET INFO("SHORTNAME")=TESTNAME QUIT
  . IF $DATA(INFO("SHORTNAME")) SET TESTNAME=INFO("SHORTNAME") QUIT  ;"//kt 2/4/22
  ...
  SET CODE="$$PCKADD60^TMGHL70C("""_ADDNAME_""",""DONE Adding LAB TEST"","""_(INDENTN+2)_""","""_$GET(MODE)_""")"
...
F60DN  ;
  QUIT TMGRESULT
  ;
EDITNAME(TESTNAME,OPTION) ;"Accept or edit display name of test 
  ;"Input: TESTNAME -- the name of the test.  This come from piece #2 of :
  ;"              <TestCode>^<Test Text>^<Coding System>^<Alternative Identifier>^<Alternative Text>^<Alternative coding system>
  ;"              this is provided upstream
  ;"       OPTION -- OPTIONAL.  Pass by reference.
  ;"          OPTION("INDENTN")=# -- Indention amount
  ;"          OPTION("MAXLEN") =# Max length. Default is 30
  ;"          OPTION("ALTNAME",<a name>)=""   <-- may provide multiple alternative names this way.  
  ;"Result: Returns TESTNAME (possibly edited, or unchanged) or -1 if aborted
  SET INDENTN=+$GET(OPTION("INDENTN"))
  SET TESTNAME=$$TRIM^XLFSTR($GET(TESTNAME))
  SET MAXLEN=+$GET(OPTION("MAXLEN")) IF MAXLEN'>0 SET MAXLEN=30
  ...
  NEW TESTLEN
EN0 ;
  IF +TMGRESULT=-1 GOTO ENDN
  KILL TMGUSERINPUT,TMGMNU
  SET TESTLEN=$LENGTH(TESTNAME)
  SET TMGMNU(-1,"INDENT")=INDENTN
  SET TMGMNU(0)="Pick option for altering TEST NAME:"
  SET TMGMNU(0,1)=" Current test name: "_TESTNAME   
  SET TMGMNU(0,2)=" Length status: "_$SELECT(TESTLEN>MAXLEN:"TOO LONG ("_TESTLEN_" chars)",1:"OK")   
  SET TMGMNU(0,3)=" NOTE: DON'T try to change the test, e.g. CBC to Potassium"
  SET TMGMNU(0,4)="       Only edit to make test name more clear"
  SET MNUNUM=1
  IF $LENGTH(TESTNAME)<(MAXLEN+1) DO
  . SET TMGMNU(MNUNUM)="USE '"_TESTNAME_"'"_$CHAR(9)_"USE^"_TESTNAME,MNUNUM=MNUNUM+1
  ELSE  DO
  . WRITE !,!,INDENTSTR,"TEST NAME TOO LONG: [",TESTNAME,"]",!
  SET TMGMNU(MNUNUM)="EDIT active name above"_$CHAR(9)_"EDIT",MNUNUM=MNUNUM+1
  IF $EXTRACT(TESTNAME,1,4)="BKR " DO
  . NEW ALTNAME SET ALTNAME=$$TRIM^XLFSTR($EXTRACT(TESTNAME,5,$LENGTH(TESTNAME)))
  . SET TMGMNU(MNUNUM)="SWAP to name: '"_ALTNAME_"'"_$CHAR(9)_"SWAP^"_ALTNAME,MNUNUM=MNUNUM+1
  NEW ALTNAME SET ALTNAME=""
  FOR  SET ALTNAME=$ORDER(OPTION("ALTNAME",ALTNAME)) QUIT:ALTNAME=""  DO
  . NEW TRIMNAME SET TRIMNAME=$$TRIM^XLFSTR(ALTNAME)
  . IF TRIMNAME=TESTNAME QUIT
  . SET TMGMNU(MNUNUM)="SWAP to name: "_TRIMNAME_$CHAR(9)_"SWAP^"_TRIMNAME,MNUNUM=MNUNUM+1
  WRITE !
  SET TMGUSERINPUT=$$MENU^TMGUSRI2(.TMGMNU,"1")
  KILL TMGMNU ;"Prevent from cluttering variable table during debug run
  IF TMGUSERINPUT["USE^" DO  GOTO ENDN
  . SET TMGRESULT=$PIECE(TMGUSERINPUT,"^",2)
  IF TMGUSERINPUT["SWAP^" DO  GOTO EN0
  . SET OPTION("ALTNAME",TESTNAME)=""
  . SET TESTNAME=$PIECE(TMGUSERINPUT,"^",2)
  IF TMGUSERINPUT="^" DO  GOTO EN0
  . SET TMGRESULT="-1^User aborted"  ;"default to failure
  IF TMGUSERINPUT="EDIT" DO  GOTO EN0
  . SET TESTNAME=$$TRIMNAME(TESTNAME,INDENTN,MAXLEN)
  GOTO EN0
ENDN  ;
  QUIT TMGRESULT
  ;
TRIMNAME(TESTNAME,INDENTN,MAXLEN) ;"Trim test name to X characters
  ;"Input: TESTNAME -- name to shorten
  ;"       INDENTN -- Indention amount
  ;"       MAXLEN -- OPTIONAL.  Max length. Default is 30
  ;"Result: New shortened test name, or -1^Message
  SET MAXLEN=+$GET(MAXLEN) IF MAXLEN'>0 SET MAXLEN=30
  NEW INDENTSTR SET INDENTSTR=""
  SET INDENTN=+$GET(INDENTN)
  IF INDENTN>0 SET INDENTSTR=$JUSTIFY(" ",INDENTN)
  SET TESTNAME=$GET(TESTNAME)
  NEW TMGRESULT,NEWNAME,%
TNM0  ;  
  IF $LENGTH(TESTNAME)'>MAXLEN GOTO TNM1
TNM0A ;  
  WRITE !,INDENTSTR,"'",TESTNAME,"' is too long. ("_$LENGTH(TESTNAME)_" chars)",!
  WRITE INDENTSTR,"Please shorten this name to ",MAXLEN," characters or less.",!,!
TNM1  ;
  ;"new method
  WRITE INDENTSTR,"Cursor keys enabled",!
  WRITE INDENTSTR SET TESTNAME=$$EDITBOX^TMGUSRI6(TESTNAME,MAXLEN,"_",0,0,MAXLEN)
  IF (TESTNAME["^")!(TESTNAME="") DO  GOTO TRIMNDN  
  . SET TMGRESULT="-1^User aborted" 
  IF $LENGTH(TESTNAME)>MAXLEN GOTO TNM0A
  SET TMGRESULT=TESTNAME
  GOTO TRIMNDN
  ;"old method below.....
  SET NEWNAME=$$READLEN^TMGUSRI4("Enter shorter test name (^ to abort): ",MAXLEN,.INDENTN)
...
ADD60SYN(IEN60,SYNONYM) ;
  ...
  NEW TMGRESULT SET TMGRESULT=1
  IF $LENGTH(SYNONYM)>75 DO
  . SET SYNONYM=$EXTRACT(SYNONYM,1,75)  ;"//kt 5/11/25
...
FIX61(TMGENV,NLT,INFO,IEN61) ;"61=TOPOGRAPHY FIELD (used as specimen)
  ...
  NEW INDENTN SET INDENTN=+$GET(TMGENV("INDENTN"))  
  NEW CODE SET CODE="$$ASKSPEC^TMGHL70C("""_TESTNAME_""","""_(INDENTN+2)_""")"
...
FIX64(TMGENV,NLT,INFO,IEN64) ;"64=WKLD CODE
  ...
  NEW INDENTN SET INDENTN=+$GET(TMGENV("INDENTN"))
  NEW TESTNAME SET TESTNAME=INFO("TESTNAME")
  NEW MAXLEN SET MAXLEN=60 ;"//kt was 30 before.  Changed to 60 3/14/22
  IF $LENGTH(TESTNAME)>MAXLEN DO  GOTO:(+TESTNAME<0) F64DN
  ...
  SET CODE="$$ASKWKLD^TMGHL70C("""_TESTNAME_""",.EXCLUDELIST,"""_(INDENTN+2)_""")"
...
FIX64061(TMGENV,NLT,INFO,IEN64D061) ;"64.061= LAB ELECTRONIC CODES (but used for SPECIMEN type)
  ...
  NEW INDENTN SET INDENTN=+$GET(TMGENV("INDENTN"))
  NEW CODE SET CODE="$$ASKSPEC2^TMGHL70C("""_TESTNAME_""",.INFO,"""_(INDENTN+2)_""")"
...
FIXSTORE(TMGENV,IEN62D41,INFO,STORAGE) ;
  ...
  . ELSE  DO
  . . SET CODE="$$TRIMNAME^TMGHL70B("""_TESTNAME_""","""_(INDENTN+2)_""")"
  . . SET TESTNAME=$$EXEC(.TMGENV,CODE)
  . IF +TESTNAME<0 SET TMGRESULT=TESTNAME QUIT
  . SET INFO("TESTNAME")=TESTNAME  
  NEW CODE SET CODE="$$LINKDN^TMGHL70C(.TMGENV,"""_TESTNAME_""","""_IEN60_""","""_(INDENTN+2)_""")"
