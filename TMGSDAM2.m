TMGSDAM2 ;TMG/kst/ENHANCED MAKE AN APPOINTMENT SDAPI ;1/9/09, 2/2/14, 3/24/21
 ...
 ;"=======================================================================
 ;"EN(TMGDFN,APPTDATE,SC,STYP,SDARRAY) -- MAKE AN APPOINTMENT
 ;
 ;"=======================================================================
 ;" API -- Public Functions.
 ;"=======================================================================
 ...
 ;"=======================================================================
 ;
EN(TMGDFN,APPTDATE,SC,STYP,SDARRAY)     ;
        ;"Purpose: MAKE AN APPOINTMENT
        ;"INPUT: TMGDFN -- PATIENT IEN (REQUIRED)
        ...
        IF '$D(^DPT(TMGDFN,0)) SET TMGRESULT="-101^INVALID PATIENT DFN" GOTO ENDONE
        ...
        SET ^DPT(TMGDFN,"S",0)="^2.98P^^"
        LOCK +^DPT(TMGDFN,"S",0):5
        ;"NEXT NODE 0
        SET PURVISIT=STYP
        ...
        SET ^DPT(TMGDFN,"S",APPTDATE,0)=TEMPS
        SET TEMPS=^DPT(TMGDFN,"S",APPTDATE,0)_"^^"_PURVISIT
        SET TEMPS=TEMPS_"^^^^^^^^^"_SDARRAY("APPT TYPE")_"^^^"_TIMEDD_"^^^^^"_0
        SET ^DPT(TMGDFN,"S",APPTDATE,0)=TEMPS
        SET TEMPS=^DPT(TMGDFN,"S",APPTDATE,0)_"^"_SDARRAY("SCHED_REQ_TYPE")_"^"
        SET TEMPS=TEMPS_SDARRAY("NEXT APPT IND")
        SET ^DPT(TMGDFN,"S",APPTDATE,0)=TEMPS
        ;"NEXT NODE 1
        IF SDARRAY("DESIRED DATE TIME OF APPT")'="" DO
        . SET ^DPT(TMGDFN,"S",APPTDATE,1)=SDARRAY("DESIRED DATE TIME OF APPT")_"^"_SDARRAY("FOLLOWUP VISIT INDICATOR")
        ELSE  DO
        . SET ^DPT(TMGDFN,"S",APPTDATE,1)=TIMEDD_"^"_SDARRAY("FOLLOWUP VISIT INDICATOR")
        LOCK -^DPT(TMGDFN,"S",0)
        ...
        SET ^SC(SC,"S",APPTDATE,1,SDY,0)=TMGDFN_"^"_SL_"^"_$GET(SDARRAY("X RAY DATA FREE TEXT"))_"^"_$GET(SDARRAY("OTHER DATA FREE TEXT"))_"^"_$GET(SDARRAY("OTHER WARD LOCATION"))
        ...
        DO MAKE^SDAMEVT(TMGDFN,SDT,SDCL,SDDA,SDMODE)
ENDONE  ;
        QUIT TMGRESULT
        ;
        ;
