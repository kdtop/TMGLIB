...
MEDIAHTM(TMGRESULT,FILEARR) ;
        ...
        ;"Result: none
        ;"
        ;"ELH - NOTE. IT APPEARS FROM THE USAGE, THAT THIS RPC WAS DESIGNED
        ;"      TO CREATE HTML, BUT WASN'T ACTUALLY FULLY IMPLEMENTED. I AM
        ;"      GOING TO FLESH THIS OUT IN A SLIGHTLY DIFFERENT WAY AND
        ;"      COMMENT THE ORIGINAL CODE BELOW    12/7/20
        ;"      
        SET TMGRESULT(0)=1
        NEW FILEIDX SET FILEIDX=0
        NEW IDX SET IDX=1
        FOR  SET FILEIDX=$O(FILEARR(FILEIDX)) QUIT:FILEIDX'>0  DO
        . ;"SET TMGRESULT(IDX)="<img height='+inttostr(Parent.height-40) width='+inttostr(Parent.width-40)+' src="'+FullFilePathName+'">'
        . SET TMGRESULT(IDX)="<img src="""_$G(FILEARR(FILEIDX))_""" width=100%>"
        . ;"SET TMGRESULT(IDX)="<img height=836 src="""_$G(FILEARR(FILEIDX))_"""width=640>"
        . SET IDX=IDX+1
        ;"SET TMGRESULT(0)=0
