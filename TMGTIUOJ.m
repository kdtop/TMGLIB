...
PRVPROBS(TMGDFN)  ;"
  ...
  SET TMGRESULT=$$ADTIUDTS(TMGRESULT,TMGDFN)  ;"added 6/10/22
...
PROBSLNK(TMGDFN)  ;"
  NEW TMGRESULT 
  SET TMGRESULT=$$GETTABLX^TMGTIUOJ(+$G(TMGDFN),"[FOLLOWUP ITEMS]")
  SET TMGRESULT=$$ADTIUDTS(TMGRESULT,TMGDFN)
  SET TMGRESULT=$$REPLSTR^TMGSTUT3(TMGRESULT,$C(13,10),"<br>")
  QUIT TMGRESULT
  ;"    
ADTIUDTS(TABLETEXT,TMGDFN)
  NEW TMGRESULT SET TMGRESULT=""
  NEW WORD
  NEW EXCLUDEWORDS
  SET EXCLUDEWORDS="^TO^TODAY^T^N^NO^NOW"
  SET TABLETEXT=$$REPLSTR^TMGSTUT3(TABLETEXT,"<BR>","<br>")
  FOR  SET WORD=$P(TABLETEXT," ",1) QUIT:TABLETEXT=""  DO
  . ;"was $C(13) now is <br>    10/10/22
  . NEW SUBWORD SET SUBWORD=""
  . FOR  SET SUBWORD=$P(WORD,"<br>",1) QUIT:SUBWORD=""  DO
  . . ;"IS SUBWORD A DATE?
  . . NEW FMDATE SET FMDATE=$$INTDATE^TMGDATE(SUBWORD)
  . . IF (FMDATE>0)&(EXCLUDEWORDS'[$$UP^XLFSTR(SUBWORD)) DO
  . . . ;"SET WORD="<B>"_WORD_"</B>"
  . . . SET SUBWORD=$$NOTEHREF(TMGDFN,FMDATE,SUBWORD)
  . . SET TMGRESULT=TMGRESULT_SUBWORD
  . . IF WORD["<br>" DO
  . . . SET TMGRESULT=TMGRESULT_"<br>"
  . . ELSE  DO
  . . . SET TMGRESULT=TMGRESULT_" "
  . . SET WORD=$P(WORD,"<br>",2,99999)
  . SET TABLETEXT=$P(TABLETEXT," ",2,99999)
  QUIT TMGRESULT
  ;"
NOTEHREF(TMGDFN,THISDATE,TEXT)
   NEW HREF,TIUTOUSE,DONE,FMDATE
   SET HREF=TEXT
   SET TIUTOUSE="",DONE=0,FMDATE=THISDATE
   FOR  SET FMDATE=$O(^TIU(8925,"ZTMGPTDT",TMGDFN,FMDATE)) QUIT:(DONE=1)!(FMDATE'>0)  DO
   . NEW TIUIEN SET TIUIEN=0
   . IF FMDATE'[THISDATE SET DONE=1 QUIT
   . FOR  SET TIUIEN=$O(^TIU(8925,"ZTMGPTDT",TMGDFN,FMDATE,TIUIEN)) QUIT:TIUIEN'>0  DO
   . . NEW DOCTYPE SET DOCTYPE=$P($G(^TIU(8925,TIUIEN,0)),"^",1)
   . . ;" allow Addendums for now IF DOCTYPE=81 QUIT
   . . IF TIUTOUSE'="" SET TIUTOUSE=TIUTOUSE_","  ;"DELIMITER BETWEEN NOTES, FOR SELECTION
   . . SET TIUTOUSE=TIUTOUSE_TIUIEN
   IF TIUTOUSE>0 SET HREF="<a href=""NoteSelect^"_TIUTOUSE_""">"_TEXT_"</a>"
   ;" tried contenteditable=""false""  above
   QUIT HREF
   ;"
...
GETLCOLN(TMGDFN) ;"Return date of last colonoscopy
  ...
  ;
GETAAA(TMGDFN) ;"Return date of last AAA
  QUIT $$GETAAA^TMGTBL01(.TMGDFN)
  ;  
GETLADIR(TMGDFN) ;"Return date of last advance directives        
