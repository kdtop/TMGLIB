...
FIXEKG(DFN) ;
        ...
        SET TEMPS=$$GETTABLX^TMGTIUOJ(DFN,"HYPERTENSION",.HTNARR)
        SET TEMPS=$$TRIM^XLFSTR($GET(HTNARR("KEY-VALUE","EKG")))
        IF "<NO DATA>."[$$UP^XLFSTR(TEMPS) SET TEMPS=""
        IF TEMPS'="" DO
        . SET EKGVAL=TEMPS
        . SET ONLYSTUDIESTABLE=0
        SET TEMPS=$$GETTABLX^TMGTIUOJ(DFN,"DIABETIC STUDIES",.DM2ARR)
        ...
        . SET ONLYSTUDIESTABLE=0
        SET TEMPS=$$GETTABLX^TMGTIUOJ(DFN,"STUDIES",.STUDIESARR)
