...
ERXNOTES(TMGDFN,MEDARRAY,ADDED) ;"FIND THE LAST NOTE WITH MED LIST AS WELL AS ERX DATA
  ...
  . ;"  NOTE:  I REMOVED THE DONE=1 IN THE QUIT CONDITION. THIS WAY IT WILL SCAN ALL DOCUMENTS FOR THE SAME DAY
  . ;"         EVEN IF 
  . FOR  SET TIUIEN=$ORDER(^TIU(8925,"ZTMGPTDT",TMGDFN,TIUDATE,TIUIEN)) QUIT:(TIUIEN'>0)  DO
...
CHK1ERX(TIUIEN,MEDARRAY,ADDED,FINALMED)  ;"GET MED LIST FROM 1 TIU NOTE
  ...
  ;" TAKING OUT TO TEST  -> *SOMETIMES WOULD CAUSE TEXT TO BE LOST  SET NOTETEXT=$$HTML2TXS^TMGHTM1(NOTETEXT)
  SET NOTETEXT=$$UP^XLFSTR(NOTETEXT)
  IF NOTETEXT'[$$BEGTAG QUIT
  DO NOTEMEDS(.TIUIEN,.NOTETEXT,.MEDARRAY,.ADDED)
  QUIT
...
NOTEMEDS(TIUIEN,NOTETEXT,MEDARRAY,ADDED)  ;"GET EPRESCRIBED MEDS OUT OF A STRING
  ...
  . . SET THISMED=$$HTML2TXS^TMGHTM1(THISMED)
