...
FINDGHP(OUTARRAY,PREFIX)  ;"DETERMINE IF ORDER HAS CMP, CBC, AND TSH. IF SO, REMOVE AND ONLY PRINT GHP
        NEW IDX SET IDX=0
        NEW CMP,CBC,TSH SET (CMP,CBC,TSH)=0
        FOR  SET IDX=$O(OUTARRAY(IDX)) QUIT:IDX'>0  DO
        . NEW LINE SET LINE=$G(OUTARRAY(IDX))
        . IF LINE["CMP" SET CMP=1
        . IF LINE["CBC-P" SET CBC=1
        . IF (LINE["TSH")&($$UP^XLFSTR(LINE)'["THYROID") SET TSH=1
        IF (CMP=1)&(CBC=1)&(TSH=1) DO
        . SET ^EDDIE("FINDGHP","FOUND")=1
        . NEW TEMPARRAY 
        . MERGE TEMPARRAY=OUTARRAY
        . KILL OUTARRAY
        . NEW OUTIDX SET OUTIDX=1
        . SET IDX=0
        ...
        ELSE  DO
        . SET ^EDDIE("FINDGHP","FOUND")=CMP_CBC_TSH
        QUIT
        ;"
