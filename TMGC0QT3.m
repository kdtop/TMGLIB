TMGC0QT3 ;TMG/kst/TMG CPT2 tests ;10/17/17
  ...
  ;"=======================================================================
  ;"EXCLUDE(DFN) -- whether patient should be excluded based on insurance
  ;"BPGRP(DFN,ADT,VITARR) --BP GROUP FOR GIVEN DATE
  ;"BESTGRP(DFN,YSDT,YEDT) -- Best group already reported in date range.
  ;
  ;"=======================================================================
  ;
BMI(OUTARRAY,BDATE,EDATE,TEXT)  ;"Function for BMI CPT codes. 
...
BMIVALUE(OUTARRAY,BDATE,EDATE,TEXT)  ;"Function for BMI Value codes
  ...
  . . . NEW DFN SET DFN=$PIECE($GET(^TIU(8925,TIUIEN,0)),"^",2)
  . . . NEW NAME SET NAME=$$GETNAME^TMGRPT2(DFN)
  . . . NEW BMI SET BMI=$$BMI^TMGTIUOJ(DFN)
  . . . SET BMI=+$GET(BMI)
  . . . IF BMI'>0 QUIT
  . . . KILL VADM
  . . . NEW AGE SET AGE=$P($$AGE^TIULO(DFN),"yr",1)
  . . . NEW MORBID  ;"ELH ADDED 3/15/21 
  . . . IF BMI>39.9 DO
  . . . . SET MORBID=1
  . . . ELSE  DO
  . . . . SET MORBID=0
  . . . IF AGE<21 DO
  . . . . NEW SEX SET SEX=$P($G(^DPT(DFN,0)),"^",2)
  . . . . SET BMI=$$BMIPTILE(BMI,AGE,SEX)
  . . . ELSE  DO
  . . . . SET BMI=$$BMICODE(BMI)
  . . . NEW BMIICD SET BMIICD=$P($P(BMI,"(",2),")",1)_" "
  . . . IF $$HASICD^TMGRPU1(DFN,BMIICD,SDT,EDT) QUIT
  . . . IF $$EXCLUDE^TMGC0QTU(DFN) QUIT
  . . . SET OUTARRAY(NAME,BMI,TIUDT)=""
  . . . IF MORBID=1 SET OUTARRAY(NAME,"BMI is over 40 (E66.01)",TIUDT)=""  ;"ELH ADDED 3/15/21
  . . . SET TMGRESULT=TMGRESULT+1
  QUIT TMGRESULT
  ;"
BMICODE(BMIVALUE)  ;"DETERMINE PROPER BMI CODE
  ...
  NEW BMIROUNDED
  IF BMIVALUE<20 DO
  . SET BMICODE="BMI is "_BMIVALUE_" (Z68.1)"
  ELSE  IF BMIVALUE<40 DO
  . SET BMIROUNDED=$P(BMIVALUE,".",1)
  . SET BMICODE="BMI is "_BMIVALUE_" (Z68."_BMIROUNDED_")"
  ELSE  DO
  . IF BMIVALUE<45 DO
  . . SET BMICODE="BMI is "_BMIVALUE_" (Z68.41)"
  . ELSE  IF BMIVALUE<50 DO
  . . SET BMICODE="BMI is "_BMIVALUE_" (Z68.42)"
  . ELSE  IF BMIVALUE<60 DO
  . . SET BMICODE="BMI is "_BMIVALUE_" (Z68.43)"
  . ELSE  IF BMIVALUE<70 DO
  . . SET BMICODE="BMI is "_BMIVALUE_" (Z68.44)"
  . ELSE  DO
  . . SET BMICODE="BMI is "_BMIVALUE_" (Z68.45)"
  QUIT BMICODE
  ;"
...
HTN(OUTARRAY,SDT,EDT,TEXT)  ;"Function for HTN CPT codes. 
  ...
  . . IF $DATA(EXCLUDE(DFN)) QUIT  
  . . IF $$EXCLUDE^TMGC0QTU(DFN) SET EXCLUDE(DFN)="" QUIT
  . . SET PTSEEN(DFN,TIUDT)=""
  ;"NOTE: I could break out below to a separate function if need to pass in a list 
  ;"      of patients from some other source...
  SET DFN=0
  FOR  SET DFN=$ORDER(PTSEEN(DFN)) QUIT:+DFN'>0  DO
  . NEW ISDM2 ;"SET ISDM2=$$.... FINISH THIS....
  . NEW AGE SET AGE=$$GET1^DIQ(2,DFN,".033")
  . NEW ADT SET ADT=0
  . FOR  SET ADT=$ORDER(PTSEEN(DFN,ADT)) QUIT:+ADT'>0  DO
  ...
  . . ;"
  . . ;"ORIGINAL BELOW
  . . ;"IF $DATA(VITALS(DFN))=0 DO VITARR^TMGTIUO3(.VITALS,DFN,.YSDT,.YEDT)
  . . ;"IF $DATA(VITALS(DFN))=0 QUIT
  . . ;"NEW CURGROUP SET CURGROUP=$$BPGRP(DFN,ADT,.VITALS) ;"GET CURRENT BP GROUP, LOWER IS BETTER
  . . ;"NEW BESTGROUP
  . . ;"IF $DATA(BESTGRPARR(DFN)) SET BESTGROUP=$GET(BESTGRPARR(DFN))
  . . ;"ELSE  DO
  . . ;". SET BESTGROUP=$$BESTGRP(DFN,YSDT,YEDT) ;"Best group already reported this year
  . . ;". SET BESTGRPARR(DFN)=BESTGROUP
  . . ;"IF +CURGROUP'<BESTGROUP QUIT  
  . . ;"
  . . ;"UPDATED
  . . NEW SDT,EDT SET SDT=$P(ADT,".",1)_".0001",EDT=$P(ADT,".",1)_".9999"
  . . IF $DATA(VITALS(DFN))=0 DO VITARR^TMGTIUO3(.VITALS,DFN,.SDT,.EDT)
  . . IF $DATA(VITALS(DFN))=0 QUIT
  . . NEW CURGROUP SET CURGROUP=$$BPGRP(DFN,ADT,.VITALS) ;"GET CURRENT BPGROUP, LOWER IS BETTER
  . . ;"  
  ...
  . . SET TEMP(DFN,+CURGROUP)=ADT_"^"_ATEXT
  . . SET TEMP(DFN,"DT",ADT)=+CURGROUP_"^"_ATEXT
  . . ;"SET OUTARRAY(NAME,ATEXT,TIUDT)=""
  NEW DFN SET DFN=0
  FOR  SET DFN=$ORDER(TEMP(DFN)) QUIT:+DFN'>0  DO
  . NEW NAME SET NAME=$$GETNAME^TMGRPT2(DFN)
  . ;"NEW GRP SET GRP=$ORDER(TEMP(DFN,""))
  . ;"NEW STR SET STR=$GET(TEMP(DFN,GRP))
  . ;"SET TIUDT=$PIECE(STR,"^",1)
  . NEW ADT SET ADT=$ORDER(TEMP(DFN,"DT",""),-1) ;"codes must be *most recent*, not best, entry
  . NEW STR SET STR=$GET(TEMP(DFN,"DT",ADT))
  . NEW ATEXT SET ATEXT=$PIECE(STR,"^",2,99)
  . SET OUTARRAY(NAME,ATEXT,ADT)=""
  . SET TMGRESULT=TMGRESULT+1
  QUIT TMGRESULT
  ;"
BPGRP(DFN,ADT,VITARR) ;"GET BP GROUP FOR GIVEN DATE
  ;"INPUT: DFN -- PATIENT
  ...
  NEW DT SET DT=$ORDER(VITARR(DFN,"V",1,ADT\1+1),-1) IF DT'>0 GOTO GCBDN  
  NEW SYS SET SYS=+$GET(VITARR(DFN,"V",1.1,DT)) IF SYS'>0 GOTO GCBDN
  NEW DIA SET DIA=+$GET(VITARR(DFN,"V",1.2,DT)) IF DIA'>0 GOTO GCBDN
  NEW CPT2A,CPT2B SET (CPT2A,CPT2B)=""
  IF SYS<130 DO
  . SET CPT2A="3074F"
...
BESTGRP(DFN,YSDT,YEDT) ;"Best group already reported in date range.
  ...
  NEW CPTARR DO ALDFNCPT^TMGRPU1(.CPTARR,DFN,.YSDT,.YEDT)
  NEW ARR
  NEW DT SET DT=0
  FOR  SET DT=$ORDER(CPTARR("DT",DT)) QUIT:+DT'>0  DO
  . NEW CPT,CPTSTR SET CPT="",CPTSTR=""
  . FOR  SET CPT=$ORDER(CPTARR("DT",DT,DFN,CPT)) QUIT:CPT=""  SET CPTSTR=CPTSTR_CPT_"^"
  . NEW TEMP SET TEMP=5
  . IF CPTSTR["3074F" DO
