...
TIMESTR(DFN)
   ...
   . ;"IF (INSIEN=12)!(INSNAME["BC/BS")!(INSNAME["BCBS")!(INSNAME["BLUE") DO
...
MDMHELP(DFN)  ;"
   NEW TMGRESULT SET TMGRESULT=$$TIMESTR(DFN)
   IF TMGRESULT'="" DO
   . SET TMGRESULT=TMGRESULT_$C(13,10)_$C(13,10)_"USE TELEPHONE CODES"
   ELSE  DO
...
FUREPORT  
   ...
   . . IF HLIGHT'="Y" QUIT
   . . SET DFN=$P($G(^TIU(8925,TIUIEN,0)),"^",2)
   ...
   FOR  SET DFN=$O(DFNARRAY(DFN)) QUIT:DFN'>0  DO
   . ;"WRITE "CHECKING ",$P($G(^DPT(DFN,0)),"^",1),!
   . NEW SCHEDULED SET SCHEDULED=0
   . NEW LASTDATE SET LASTDATE=0
   . NEW SCHDATE SET SCHDATE=0
   . FOR  SET SCHDATE=$O(^TMG(22723,DFN,1,"B",SCHDATE)) QUIT:SCHDATE'>0  DO
   . . NEW SCHIEN SET SCHIEN=$O(^TMG(22723,DFN,1,"B",SCHDATE,0))
   . . NEW STATUS SET STATUS=$P($G(^TMG(22723,DFN,1,SCHIEN,0)),"^",7)
   ...
   . IF SCHEDULED=0 DO
   . . ;"WRITE "**** NOT SCHEDULED!!!! ****",!,!
   . . SET REPORTARR($P($G(^DPT(DFN,0)),"^",1),DFN)="LAST SEEN ON: "_$$EXTDATE^TMGDATE(LASTDATE,1)
   . . ;"add to list (LIST 11 IN 100.21)
   . . ;NEW TMGFDA,TMGMSG,TMGIENS
   . . ;SET TMGIENS="+1,11,"
   . . ;SET TMGFDA(100.2101,TMGIENS,.01)=DFN
   ...
   . NEW DFN SET DFN=0
   . FOR  SET DFN=$O(REPORTARR(NAME,DFN)) QUIT:DFN'>0  DO
   . . NEW DOB SET DOB=$P($G(^DPT(DFN,0)),"^",3)
   . . WRITE "[   ] ",$P($G(^DPT(DFN,0)),"^",1)," ("
   . . WRITE $$EXTDATE^TMGDATE(DOB),") - "_$P($G(^DPT(DFN,"TMG")),"^",2),!
   . . WRITE "       HOME: ",$P($G(^DPT(DFN,.13)),"^",1)," CELL: ",$P($G(^DPT(DFN,.13)),"^",4),!
   . . WRITE "       ",$G(REPORTARR(DFN)),!,!
   DO ^%ZISC  ;" Close the output device
   QUIT
   ;"