...
PTINQ(TMGDFN)  ;"Replacement for DGINQB^ORCXPND1 patient inquiry.
  ...
  . IF SHOULDGARBLE QUIT  ;"check for special mode to hide patient info during demos
...
APPTRECS(SDT,EDT,RANGE)  ;"
 ...
 ;"NOPRINT  
  ...
  . . IF REASON="TH-FU HOSP" SET STATUS="NEED HOSPITAL RECORDS"  
...
HL7QUERY(ROOT,ORDFN,ID,ORALPHA,OROMEGA,ORDTRNG,REMOTE,ORMAX,ORFHIE) ; --Query to Health Summary Reports
 ...
 . . IF $GET(MSH)="" QUIT
 . . NEW TMGU DO MSH2TMGU^TMGHL7X2(MSH,.TMGU)
 . . SET ADTTYPE=$P(MSH,TMGU(1),9)   
 . . IF ADTTYPE'["ADT" QUIT
 . . SET ADTTYPE=$P(ADTTYPE,TMGU(2),2)
 ...
 . . . NEW LINE SET LINE=$G(HL7MSGARR(HL7IDX))
 . . . NEW SEGTYPE SET SEGTYPE=$PIECE(LINE,TMGU(1),1)
 . . . IF SHOULDGARBLE,"PID,ZPD,NK1,NK2,GT1,IN1,IN2,ZG1"[SEGTYPE DO
 . . . . SET LINE=SEGTYPE_TMGU(1)_"-- Redacted for Privacy --" 
 . . . S IDX=IDX+1,^TMP("HL7DATA",$J,IDX)="2^"_LINE
 . . S IDX=IDX+1,^TMP("HL7DATA",$J,IDX)="2^=========================================================================================="
...
TEST
  ...
  QUIT
  ;"
NOLABS(TMGDFN)  ;"THIS WILL CREATE A MESSAGE IF THE PATIENT HAS
  ;" AN UNFULFILLED LAB ORDER AND NO LAB RESULTS IN THE LAST 14 DAYS
  NEW TMGRESULT SET TMGRESULT=""
  NEW LABDATES DO GETLABDT(TMGDFN,.LABDATES,14)
  ;" IF THEY HAVE LAB RESULTS QUIT
  IF $D(LABDATES) QUIT TMGRESULT
...
GETAORD(TMGDFN,RESULTARR)
  ...
  QUIT
  ;"
GETLABDT(TMGDFN,RESULTARR,CUTOFF)
  SET CUTOFF=+$G(CUTOFF)
  IF CUTOFF'>0 SET CUTOFF=30
  NEW LRDFN SET LRDFN=+$P($G(^DPT(TMGDFN,"LR")),"^",1)
  IF LRDFN'>0 GOTO GLDDN
  NEW RTODAY SET RTODAY=$$LABDTNOW()
  NEW CUTOFFDT SET CUTOFFDT=$$LCUTDT(CUTOFF)
...
TIMESTR(TMGDFN)
   ...
   . IF $$UP^XLFSTR(INSNAME)["AETNA" DO
   . . SET WARN=$C(13,10)_"  !! WILL NOT PAY FOR AUDIO ONLY VISITS!!"_$C(13,10)
   . ELSE  IF WARNSTR[INSSTR DO
...
NOTEWARN(TMGRESULT,TMGDFN,NOTETYPE)  ;"
   ...
   ;"
   ;"CHECK INSURANCE
   NEW INSIDX,INSIEN,INSNAME SET INSIDX=0,INSNAME=""
   NEW WARN,WARNSTR
   SET WARN="",WARNSTR="^17^100^22^4^"
   FOR  SET INSIDX=$O(^DPT(TMGDFN,.312,INSIDX)) QUIT:INSIDX'>0  DO
   . SET INSIEN=$P($G(^DPT(TMGDFN,.312,INSIDX,0)),"^",1)
   . NEW INSSTR SET INSSTR="^"_INSIEN_"^"
   . IF WARNSTR[INSSTR DO
   . . SET INSNAME=$P($G(^DIC(36,INSIEN,0)),"^",1)
   . . SET WARN=WARN_"!!THIS INSURANCE MAY NOT PAY FOR AUDIO ONLY VISITS!!"
   IF WARN'="" SET TMGRESULT=WARN
   ;"
   ;"CHECK FOR PREVIOUS VISIT
   NEW BDATE SET BDATE=$$ADDDAYS^TMGDATE("-7",$$TODAY^TMGDATE())
   ...
   . . IF OFFNOTE'="" DO
   . . . IF TMGRESULT'="" SET TMGRESULT=TMGRESULT_"@@BR@@@@BR@@"
   . . . SET TMGRESULT=TMGRESULT_"WARNING: PATIENT HAD A VISIT ON "_$$EXTDATE^TMGDATE(BDATE,1)_".@@BR@@AN AUDIO ONLY VISIT MAY NOT PAY."
   IF TMGRESULT="" DO
   . SET TMGRESULT="1^OKAY"
   ELSE  DO
   . IF INSNAME="" DO
   . . SET TMGRESULT="-1^"_TMGRESULT_"@@BR@@CONTINUE?"
   . ELSE  DO
   . . SET TMGRESULT="-1^PATIENT HAS "_INSNAME_"@@BR@@  "_TMGRESULT_"@@BR@@CONTINUE?"
   QUIT 
   ;"
MDMHELP(TMGDFN,NEWPAT)  ;"
   ...
   ELSE  IF NEWPAT=1 DO
   . IF TIMESPENT>59 SET WARNING="NOTE: Time spent is "_TIMESPENT_" mins. A "_CODE_" can be billed based on time."
   SET TMGRESULT=$$TIMESTR(TMGDFN)
   IF TMGRESULT'="" DO
...
LSTBPBAD  ;"LAST BP FOR THIS YEAR WAS NOT IN RANGE FOR MEDICARE ADV PATIENTS
   ...
   NEW THISYEAR SET THISYEAR=$E(BDT,1,3)+1700
   NEW %ZIS
   SET %ZIS("A")="Enter Output Device: "
   SET IOP="S121-LAUGHLIN-LASER"
   DO ^%ZIS  ;"standard device call
   ...
   WRITE "       WITH NO MORE APPOINTMENTS THIS YEAR.",!
   WRITE "*************************************************(FROM TMGRPT3.m)",!,!   
   ;"W BDT,!
   FOR  SET TMGDFN=$O(^DPT(TMGDFN)) QUIT:TMGDFN'>0  DO
   . NEW AGE K VADM SET AGE=+$$AGE^TIULO(TMGDFN)
   ...
   . IF NEXTAPPT[THISYEAR QUIT  ;"THIS YEAR
   . IF NEXTAPPT["NO UPCOMING" DO 
   . . SET NEXTAPPT="NONE" 
   . ELSE  DO
   . . SET NEXTAPPT=$$EXTDATE^TMGDATE($P($P(NEXTAPPT,"APPTS: ",2),"@",1),1)
   ...
   . SET OUTARR($P($G(^DPT(TMGDFN,0)),"^",1))=TMGDFN_"^"_SBP_"/"_DBP_"^"_$$EXTDATE^TMGDATE(LDATE,1)_"^"_NEXTAPPT
   NEW NAME SET NAME=""
   WRITE "NAME",?30,"LAST BP",?40,"LAST DATE",?55,"PHONE NUMBER",?70,"NEXT APPT",!,!
   FOR  SET NAME=$O(OUTARR(NAME)) QUIT:NAME=""  DO
   . NEW BP,DATE,PHONE,NEXTAPPT
   ...
   . IF PHONE="" SET PHONE=$P($G(^DPT(TMGDFN,.13)),"^",4)
   . SET PHONE=$$FMTPHONE(PHONE)
   . SET NEXTAPPT=$P($G(OUTARR(NAME)),"^",4)
   . WRITE "[  ] ",NAME,?30,BP,?40,DATE,?55,PHONE,?70,NEXTAPPT,!,!
...
IMMGRAN
        NEW TMGDFN SET TMGDFN=0
        NEW BDT SET BDT=3220925  ;"CAN BE ALTERED TO BE MORE ROBUST
        ;"NEW BDT SET BDT=3240931  ;"CAN BE ALTERED TO BE MORE ROBUST
        NEW TOTALCNT,NORMALCNT,ABNORMALCNT,DATAARR,YEAR,MONTH
        SET (TOTALCNT,NORMALCNT,ABNORMALCNT)=0
        FOR  SET TMGDFN=$O(^DPT(TMGDFN)) QUIT:TMGDFN'>0  DO
        ...
        . . IF '$DATA(^LR(LRDFN,"CH",DATEIDX,69413)) QUIT
        . . NEW DATASTR SET DATASTR=$G(^LR(LRDFN,"CH",DATEIDX,69413))
        . . NEW DATE SET DATE=$P(DATASTR,"^",6)
        . . IF DATE<BDT QUIT
        . . SET TOTALCNT=TOTALCNT+1
        . . NEW FLAG SET FLAG=$P(DATASTR,"^",2)
        . . SET YEAR=$E(DATE,1,3)+1700,MONTH=+$E(DATE,4,5)        
        . . IF FLAG="" DO
        . . . SET NORMALCNT=NORMALCNT+1
        . . . SET DATAARR(YEAR,MONTH,"NORMAL")=+$G(DATAARR(YEAR,MONTH,"NORMAL"))+1
        . . ELSE  DO
        . . . WRITE $P($G(^DPT(TMGDFN,0)),"^",1),!
        . . . WRITE "  ",DATASTR,!
        . . . SET ABNORMALCNT=ABNORMALCNT+1
        . . . SET DATAARR(YEAR,MONTH,"ABNORMAL")=+$G(DATAARR(YEAR,MONTH,"ABNORMAL"))+1  
        . . SET DATAARR(YEAR,MONTH,"TOTAL")=+$G(DATAARR(YEAR,MONTH,"TOTAL"))+1
        . . ;"WRITE $G(^LR(LRDFN,"CH",DATEIDX,69413)),!
        ;"ZWR DATAARR
        SET YEAR=2025
        FOR  SET YEAR=$O(DATAARR(YEAR),-1) QUIT:YEAR'>0  DO
        . WRITE "==== YEAR: ",YEAR," ====",!
        . SET MONTH=14
        . FOR  SET MONTH=$O(DATAARR(YEAR,MONTH),-1) QUIT:MONTH'>0  DO
        . . NEW OUTMONTH SET OUTMONTH=MONTH
        . . IF $L(OUTMONTH)=1 SET OUTMONTH="0"_OUTMONTH
        . . WRITE " *MONTH: ",OUTMONTH," -> "
        . . NEW NORMAL SET NORMAL=+$G(DATAARR(YEAR,MONTH,"NORMAL"))
        . . IF $L(NORMAL)=2 SET NORMAL="0"_NORMAL
        . . WRITE "NORMAL: ",NORMAL
        . . NEW ABNORMAL SET ABNORMAL=+$G(DATAARR(YEAR,MONTH,"ABNORMAL"))
        . . IF $L(ABNORMAL)=1 SET ABNORMAL="0"_ABNORMAL
        . . WRITE " ABNORMAL: ",ABNORMAL
        . . NEW TOTAL SET TOTAL=+$G(DATAARR(YEAR,MONTH,"TOTAL"))
        . . NEW PERCENT SET PERCENT=(+$G(DATAARR(YEAR,MONTH,"ABNORMAL"))/TOTAL)*100
        . . WRITE " -- ",$J(PERCENT,"",1),"% IS ABNORMAL",!
        WRITE "TOTAL LAB COUNT: ",TOTALCNT,!
        WRITE "TOTAL ABNORMAL COUNT: ",ABNORMALCNT,!
        WRITE "TOTAL NORMAL COUNT: ",NORMALCNT,!
        QUIT
        ;"
IMGQUERY(ROOT,DFN,ORID,ALPHA,OMEGA,DTRANGE,REMOTE,ORMAX,ORFHIE) ; -- return imaging report
 D RPT^ORWRA(.ROOT,.DFN,.ORID,.ALPHA,.OMEGA,.DTRANGE,.REMOTE,.ORMAX,.ORFHIE)
 Q
 ;"
CPEREPORT
  NEW SDT,EDT,APPTARRAY
  NEW OUTARRAY
  SET SDT=$$ADDDAYS^TMGDATE(1),EDT=$$ADDDAYS^TMGDATE(365)
  ;"
  DO APPT4DT^TMGSMS05(SDT,EDT,.APPTARRAY,1)
  ;"
  ;"NEW TESTINS SET TESTINS="^AARP / Secure Horizon^BC/BS ADVANTAGE^HUMANA GOLD^UHC-LIFE1^"
  NEW TESTINS SET TESTINS="^AARP / Secure Horizon^UHC-LIFE1^"
  ;"
  NEW APPTDATE SET APPTDATE=0
  FOR  SET APPTDATE=$O(APPTARRAY(APPTDATE)) QUIT:APPTDATE'>0  DO
  . NEW TMGDFN SET TMGDFN=0
  . FOR  SET TMGDFN=$O(APPTARRAY(APPTDATE,TMGDFN)) QUIT:TMGDFN'>0  DO
  . . NEW REASON SET REASON=$G(APPTARRAY(APPTDATE,TMGDFN,"REASON"))
  . . NEW PTNAME SET PTNAME=$G(APPTARRAY(APPTDATE,TMGDFN,"NAME"))
  . . NEW INSURANCE SET INSURANCE=$$GETINS(TMGDFN)
  . . IF REASON="PHYSICAL" DO
  . . . NEW TESTLINE SET TESTLINE="^"_INSURANCE_"^"
  . . . IF TESTINS[TESTLINE DO
  . . . . SET OUTARRAY("THE FOLLOWING ARE SCHEDULED FOR CPE BUT HAVE MEDICARE ADVANTAGE. CONSIDER CHANGING TO AWV.",PTNAME," CPE APPOINTMENT: "_$$EXTDATE^TMGDATE(APPTDATE,1)_" BUT HAS "_INSURANCE_".")=""
  . . . . ;"SET OUTARRAY("ADV-AWV",PTNAME,"(CONSIDER AWV)")=""
  . . . IF INSURANCE="MEDICARE" DO
  . . . . NEW LASTCPE,TEST,DATE SET LASTCPE=$$ADVCPEDN^TMGPXR01(TMGDFN,.TEST,.DATE)
  . . . . SET LASTCPE=$$INTDATE^TMGDATE(LASTCPE)
  . . . . NEW DAYSDIFF SET DAYSDIFF=$$DAYSDIFF^TMGDATE(APPTDATE,LASTCPE)
  . . . . IF DAYSDIFF<366 DO
  . . . . . SET OUTARRAY("THE FOLLOWING HAVE MEDICARE AND THIS APPOINTMENT MAY BE TOO SOON. CONSIDER RESCHEDULING.",PTNAME," CPE APPOINTMENT: "_$$EXTDATE^TMGDATE(APPTDATE,1)_". LAST ONE WAS ON "_$$EXTDATE^TMGDATE(LASTCPE,1)_" ("_DAYSDIFF_" DAYS BETWEEN).")=""
  . . . . ELSE  DO
  . . . . . SET OUTARRAY("THE FOLLOWING ARE SCHEDULED FOR CPE BUT HAVE MEDICARE. CONSIDER CHANGING TO AWV.",PTNAME," CPE APPOINTMENT: "_$$EXTDATE^TMGDATE(APPTDATE,1)_" BUT HAS "_INSURANCE_".")=""
  . . . . . ;"SET OUTARRAY("MEDICARE",PTNAME,"(MEDICARE REQUIRES 365 DAYS BETWEEN)")=""
  . . IF REASON="AWV" DO
  . . . IF INSURANCE="MEDICARE" DO
  . . . . NEW LASTCPE,TEST,DATE SET LASTCPE=$$ADVCPEDN^TMGPXR01(TMGDFN,.TEST,.DATE)
  . . . . SET LASTCPE=$$INTDATE^TMGDATE(LASTCPE)
  . . . . NEW DAYSDIFF SET DAYSDIFF=$$DAYSDIFF^TMGDATE(APPTDATE,LASTCPE)
  . . . . IF DAYSDIFF<366 DO
  . . . . . SET OUTARRAY("THE FOLLOWING HAVE MEDICARE AND THIS APPOINTMENT MAY BE TOO SOON. CONSIDER RESCHEDULING.",PTNAME," AWV APPOINTMENT: "_$$EXTDATE^TMGDATE(APPTDATE,1)_". LAST ONE WAS ON "_$$EXTDATE^TMGDATE(LASTCPE,1)_" ("_DAYSDIFF_" DAYS BETWEEN).")=""
  . . . . . ;"SET OUTARRAY("MEDICARE",PTNAME,"(MEDICARE REQUIRES 365 DAYS BETWEEN)")=""
  ;"
  ;"  ADDING ANOTHER CHECK FOR MULTIPLE APPOINTMENTS
  NEW TMGNAME SET TMGNAME=""
  FOR  SET TMGNAME=$O(APPTARRAY("B",TMGNAME)) QUIT:TMGNAME=""  DO
  . NEW COUNT SET COUNT=0
  . NEW APPTARR
  . NEW APPTDATE SET APPTDATE=0
  . FOR  SET APPTDATE=$O(APPTARRAY("B",TMGNAME,APPTDATE)) QUIT:APPTDATE=""  DO
  . . SET COUNT=COUNT+1
  . . SET APPTARR(APPTDATE)=""
  . IF COUNT>1 DO
  . . NEW APPTSTR,APPTDATE
  . . SET APPTSTR="",APPTDATE=0
  . . FOR  SET APPTDATE=$O(APPTARR(APPTDATE)) QUIT:APPTDATE=""  DO
  . . . IF APPTSTR'="" SET APPTSTR=APPTSTR_", "
  . . . SET APPTSTR=APPTSTR_$$EXTDATE^TMGDATE(APPTDATE,1)
  . . SET OUTARRAY("THESE PATIENTS HAVE MULTIPLE APPOINTMENTS",TMGNAME,APPTSTR)=""
  ;"
  NEW %ZIS
  SET %ZIS("A")="Enter Output Device: "
  SET IOP="S121-LAUGHLIN-LASER"
  ...
  WRITE !
  WRITE "****************************************************************",!
  WRITE "               Upcoming appointment suggestions",!
  WRITE "                Appt Dates: ",$$EXTDATE^TMGDATE(SDT)," - ",$$EXTDATE^TMGDATE(EDT),!
  WRITE "****************************************************************",!
  WRITE "  Deliver to: Lindsey                          (From TMGRPT3.m)",!!
  NEW REASON SET REASON=""
  FOR  SET REASON=$O(OUTARRAY(REASON)) QUIT:REASON=""  DO
  . WRITE "=====================================================================",!
  . WRITE REASON,!
  . WRITE "=====================================================================",!,!
  . NEW PTNAME SET PTNAME=""
  . FOR  SET PTNAME=$O(OUTARRAY(REASON,PTNAME)) QUIT:PTNAME=""  DO
  . . WRITE "** ",PTNAME," **",!
  . . NEW LINE SET LINE=""
  . . FOR  SET LINE=$O(OUTARRAY(REASON,PTNAME,LINE)) QUIT:LINE=""  DO
  . . . WRITE LINE,!
  . . WRITE !
  . WRITE !
  DO ^%ZISC  ;" Close the output device
  QUIT
  ;"
GETINS(TMGDFN)  ;"RETURN THE NAME OF THE PATIENT'S PRIMARY INSURANCE
  NEW INSIDX,INSIEN SET INSIDX=0
  NEW TMGRESULT SET TMGRESULT=""
  FOR  SET INSIDX=$ORDER(^DPT(TMGDFN,.312,INSIDX)) QUIT:INSIDX'>0  DO
  . NEW COB SET COB=+$P($G(^DPT(TMGDFN,.312,INSIDX,0)),"^",20)
  . IF COB'=1 QUIT
  . SET INSIEN=$P($GET(^DPT(TMGDFN,.312,INSIDX,0)),"^",1)
  . SET TMGRESULT=$PIECE($GET(^DIC(36,INSIEN,0)),"^",1)
  QUIT TMGRESULT
  ;"
