...
NOTEWARN(TMGRESULT,TMGDFN,NOTETYPE)  ;"
   ...
   NEW INSIDX,INSIEN SET INSIDX=0
   NEW WARN,WARNSTR
   SET WARN="",WARNSTR="^17^100^"
   FOR  SET INSIDX=$O(^DPT(TMGDFN,.312,INSIDX)) QUIT:INSIDX'>0  DO
   . SET INSIEN=$P($G(^DPT(TMGDFN,.312,INSIDX,0)),"^",1)
   . NEW INSSTR SET INSSTR="^"_INSIEN_"^"
   . IF WARNSTR[INSSTR DO
   . . NEW INSNAME SET INSNAME=$P($G(^DIC(36,INSIEN,0)),"^",1)
   . . SET WARN=WARN_"PATIENT HAS "_INSNAME_"@@BR@@  !!THIS INSURANCE MAY NOT PAY FOR AUDIO ONLY VISITS!!"
   IF WARN'="" DO  QUIT
   . SET TMGRESULT="-1^"_WARN
   ;"
   ...
   . . IF OFFNOTE'="" DO
   . . . SET TMGRESULT="-1^WARNING: PATIENT HAD A VISIT ON "_$$EXTDATE^TMGDATE(BDATE,1)_".@@BR@@AN AUDIO ONLY VISIT MAY NOT PAY.@@BR@@CONTINUE?"
   QUIT 
   ;"
