TMGSEQL1 ;TMG/kst/Interface with SequelSystems PMS ;7/10/12, 2/2/14, 4/19/18, 3/24/21
...
RUNNOW  ;"RUN NOW
  ...
  ;"
  NEW TMGTEST SET TMGTEST=1
  IF TMGTEST=1 DO IMPRTINS
  QUIT
  ;
IMPRTINS
  ;"Purpose: Import Insurances from 
  ;"Input: none
  ;"Output: none
  ;"Results; none
  NEW GREF
  SET GREF=$NAME(^TMP("TMG",$J,"SEQUELIMPORT","DATA",4)) KILL @GREF
  SET RESULT=$$FTG^%ZISH("/mnt/WinServer/","demographics4.csv",$NAME(@GREF@(1)),6) IF RESULT=0 GOTO IIDN
  NEW PATARRAY,IDX,TITLE,TITLEARR,INSARRAY
  ;"GET ALL TITLES
  SET TITLE="",IDX=0
  FOR  SET IDX=IDX+1 QUIT:$P($G(@GREF@(1)),",",IDX)=""  DO
  . SET TITLEARR(IDX)=$P($G(@GREF@(1)),",",IDX)
  ;"LOAD ALL PATIENT DATA
  SET IDX=1
  FOR  SET IDX=$O(@GREF@(IDX)) QUIT:IDX'>0  DO
  . NEW TITLEIDX,INSIDX
  . SET TITLEIDX=0,INSIDX=0
  . FOR  SET TITLEIDX=$O(TITLEARR(TITLEIDX)) QUIT:TITLEIDX'>0  DO
  . . SET INSARRAY(IDX,$G(TITLEARR(TITLEIDX)))=$P($G(@GREF@(IDX)),",",TITLEIDX)
  KILL @GREF
  ;"ZWR INSARRAY
  ;"PARSE DATA
  SET IDX=0
  FOR  SET IDX=$O(INSARRAY(IDX)) QUIT:IDX'>0  DO
  . NEW COB SET COB=$G(INSARRAY(IDX,"plan_"))
  . IF COB'="1" QUIT
  . NEW ACCTNUM,FNAME,LNAME,SSN,PNAME,PID
  . SET ACCTNUM=$G(INSARRAY(IDX,"account_num")),FNAME=$G(INSARRAY(IDX,"pat_first_name"))
  . SET LNAME=$G(INSARRAY(IDX,"pat_last_name")),SSN=$G(INSARRAY(IDX,"pat_ssn"))
  . SET PNAME=$G(INSARRAY(IDX,"plan_short_name")),PID=$G(INSARRAY(IDX,"plan_id_num"))
  . WRITE ACCTNUM,"-",LNAME,"-",FNAME,"-",SSN,"-",PNAME,"-",PID,!
  . NEW TMGDFN 
  . DO FINDPAT^TMGGDFN(.TMGDFN,ACCTNUM,LNAME_","_FNAME,"")
  . IF $$ACTIVEPT^TMGPXR03(TMGDFN)'=1 QUIT
  . W "----------> ",TMGDFN,!
  . DO DELINS^TMGSEQL8(TMGDFN)
  . DO ENSURINS^TMGSEQL8(PNAME,PID,COB,TMGDFN)
IIDN  
  QUIT
  ;"
AUTOIN  ;
...
IMPORTFILE(FILEPATH,FILENAME,F2NAME,F3NAME,ERRARRAY,CHGLOG,PROGFN,DELFILES,USERID) ;
  ...
  ;"//kt was --> SET GREF=$NAME(^TMP("TMG","SEQUELIMPORT","DATA",1,$J)) KILL @GREF  //kt all ref's (below) also changed 
  SET GREF=$NAME(^TMP("TMG",$J,"SEQUELIMPORT","DATA",1)) KILL @GREF 
  SET RESULT=$$FTG^%ZISH(FILEPATH,FILENAME,$NAME(@GREF@(1)),6) IF RESULT=0 GOTO IFDONE  
  SET G2REF=$NAME(^TMP("TMG",$J,"SEQUELIMPORT","DATA",2)) KILL @G2REF
  SET RESULT=$$FTG^%ZISH(FILEPATH,F2NAME,$NAME(@G2REF@(1)),6) IF RESULT=0 GOTO IFDONE  
  SET G3REF=$NAME(^TMP("TMG",$J,"SEQUELIMPORT","DATA",3)) KILL @G3REF
