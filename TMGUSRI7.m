...
TEST2(USESCRN) ;"Draw menu and interact with user.  
  ...
  DO SETUPMENU^TMGIDE2C(.MENU,1,20) ;
  NEW RUNOPT
...
DRAWMENU(IDX,MENU)  ;"Draw one menu box.   
  ...
  ELSE  DO
  . DO SETCOLORS("BORDER",.COLARR)
  . NEW BORDERCOL SET BORDERCOL=$GET(COLARR("BORDER"))
  . SET FG=$PIECE(BORDERCOL,"^",1),BG=$PIECE(BORDERCOL,"^",2)
  . IF FG="" SET FG=-1
  . IF BG="" SET BG=-1
  DO DRAWBOX^TMGTERM2(X,Y,WT+2,HT+2,FG,BG,.INFO)  
...
DRAWTESTSCRN(WT,HT) ;
  ...
  DO VTATRIB^TMGTERM(0) ;"clear colors for box
  ;"WRITE WT,",",HT,!
  DO DRAWBOX^TMGTERM2(1,1,WT,HT,-1,-1,.INFO)
  NEW X,Y
  FOR Y=2:1:HT-1 DO
  . FOR X=2:1:WT-1 DO
  . . DO CUP^TMGTERM(X,Y)
