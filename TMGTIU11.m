TMGTIU11 ;TMG/kst-TIU PROBLEM LINK STUFF ; 06/22/15, 3/24/21
...
PROBLINK(OUT,IN)  ;"Read or ADD problem links.  Code for RPC: TMG CPRS PROBLEM LINK
  ...
  NEW TMGDFN SET TMGDFN=+$GET(IN("PATIENT"))
  IF TMGDFN'>0 SET RESULT="-1^DFN NOT PROVIDED" GOTO PLDN
  ...
  . DO READSEC(TMGDFN,PROBIEN,"ATMGPROB",.COUNT,.OUT)
  FOR  SET DOCIEN=$ORDER(IN("READ","DOC",DOCIEN)) QUIT:DOCIEN'>0  DO
  . DO READSEC(TMGDFN,DOCIEN,"ATMGDOC",.COUNT,.OUT)
  FOR  SET POVIEN=$ORDER(IN("READ","POV",POVIEN)) QUIT:POVIEN'>0  DO
  . DO READSEC(TMGDFN,POVIEN,"ATMGPOV",.COUNT,.OUT)
...
PLDN ;
  ...
  QUIT
  ;"
READSEC(TMGDFN,IEN,XREF,COUNT,OUT)  ;
  ...
  FOR  SET PLIEN=$ORDER(^TIU(8925.9,XREF,TMGDFN,IEN,PLIEN)) QUIT:PLIEN'>0  DO
  . NEW ZN SET ZN=$GET(^TIU(8925.9,PLIEN,0))
  . NEW IEN8925 SET IEN8925=+ZN
  . NEW PTR2PAT SET PTR2PAT=+$PIECE($GET(^TIU(8925,IEN8925,0)),"^",2)
  . IF ($DATA(^TIU(8925,IEN8925))'>0)!(IEN8925=0)!(PTR2PAT'=TMGDFN) DO  QUIT  ;"Linked document may be invalid
  . . NEW DIK,DA SET DIK="^TIU(8925.9,",DA=PLIEN
  . . DO ^DIK   ;"kill record linking to missing document.
  . . KILL ^TIU(8925.9,XREF,TMGDFN,IEN,PLIEN)  ;"for some reason ^DIK didn't kill this XRef entry 
  . SET COUNT=COUNT+1
