TMGHL77 ;TMG/kst-HL7 transformation engine processing ;4/11/2019
 ...
 ;"~--~--~--~--~--~--~--~--~--~--~--~--~--~--~--~--~--~--~--~--~--~--~--~--
 ;
 ;"NOTE: this is code for working with labs from **[SOFHA lab]**
 ...
 ;
TEST  ;"Pick file and manually send through filing process.
        DO TEST^TMGHL71("/mnt/WinServer/SOFHA_HL7")
        QUIT
        ;
BATCH   ;"Launch processing through all files in folder.
        DO HLDIRIN^TMGHL71("/mnt/WinServer/SOFHA_HL7",1000,10)
        QUIT
        ;
        ;"---------------------------------------------------------------
        ;"===============================================================
...
OBX3    ;"Purpose: To transform the OBX segment, field 3 -- Observation Identifier
        ...
        ELSE  IF $PIECE(TMGVALUE,TMGU(2),1)="" DO   ;"real life example: ^^^^^^^^BKR ANION GAP
        . NEW INFO MERGE INFO=TMGHL7MSG(TMGSEGN,3)
        . IF $DATA(INFO)<10 QUIT
        . NEW PCE SET PCE=$ORDER(INFO(0)) QUIT:PCE'>0
        . SET TMGVALUE=$GET(INFO(PCE)) QUIT:TMGVALUE=""
        . NEW CODE SET CODE="TMG_"_$TRANSLATE(TMGVALUE," ","_")
        . SET TMGVALUE=CODE_TMGU(2)_TMGVALUE
        . DO SETPCE^TMGHL7X2(TMGVALUE,.TMGHL7MSG,.TMGU,TMGSEGN,3)
        DO OBX3^TMGHL72
