TMGRX005 ;TMG/kst/Patient medication code; 08/23/17
 ...
 ;"REGDOSE(ARR,IEN22733) -- REGISTER DOSE
...
CKR2 ;  
  ...
  SET NEEDDOSE=($ORDER(ARR("DOSE","IENS",""))="")
  IF ('NEEDRX)&('NEEDDOSE) GOTO CKR3
CKR2B  ;  
  ...
  IF (NEEDRX=0),NEEDDOSE WRITE "Dose doesn't appear to be registered, or is missing from input",!
CKRM ;       
  ...
  ELSE  IF NEEDDOSE DO
  . SET IDX=IDX+1,MENU(IDX)="Register dose for "_MEDNAME_$CHAR(9)_"REGDOSE"
  IF IEN22733>0 SET IDX=IDX+1,MENU(IDX)="Show "_MEDNAME_"'s registered doses"_$CHAR(9)_"SHOWDOSES"
  IF IEN22733>0 SET IDX=IDX+1,MENU(IDX)="Edit "_MEDNAME_"'s registered doses"_$CHAR(9)_"EDITDOSES"    
  IF IEN22733>0 SET IDX=IDX+1,MENU(IDX)="Edit "_MEDNAME_" record"_$CHAR(9)_"EDIT^"_IEN22733
  IF IEN22733>0 SET IDX=IDX+1,MENU(IDX)="Show "_MEDNAME_"'s modifiers"_$CHAR(9)_"SHOWMODIFIERS"
  IF IEN22733>0 SET IDX=IDX+1,MENU(IDX)="Add modifier for "_MEDNAME_$CHAR(9)_"ADDMODIFIER"
  ...
  IF USRPICK="REGDOSE" DO  GOTO:(RESULT'>0) CKRM GOTO CKR1
  . SET RESULT=$$REGDOSE(.ARR,IEN22733)  ;"REGISTER DOSE
  ...
  . DO EDITRX(+Y) 
  IF USRPICK="SHOWDOSES" DO  GOTO CKR2
  . DO SHOWDOSES(IEN22733)
  IF USRPICK="EDITDOSES" DO  GOTO CKR2
  . DO EDTDOSES(IEN22733)  ;"EDIT CURRENT REGISTERED DOSES FOR MEDICATION  
...
REGDOSE(ARR,IEN22733)  ;"REGISTER DOSE
  ...
  IF DOSE="" DO  GOTO:DOSE="" REGDDN
  . WRITE "Enter dose (e.g. '20'), WITHOUT units (e.g. NOT '20 mg'). (^ to abort)",!
  . WRITE "DOSE: " READ DOSE:DTIME WRITE !
  IF DOSE["^" QUIT  ;"leaves result as 0 (abort)
  SET ARR("REG DOSE")=DOSE
  ;"----------------------------------------
  SET IEN50D607=+$GET(ARR("UNITS","IEN50.607")) 
  IF IEN50D607>0 GOTO REGD2
  SET %=1 WRITE "Use MG for units of dose" DO YN^DICN WRITE !
...
REGD2 ;"--------------------------------------
  ...
  ;"-------------------------------------------
  WRITE "Pick VA drug & dose that matches:",! 
...
REGD3 ;"------------------------------------------- 
  ...
  . . SET ABORT=1
  KILL TMGFDA,TMGIEN,TMGMSG
  SET TMGFDA(22733.03,"+1,"_SUBIEN_","_IEN22733_",",.01)=DOSE  ;"SUB-FIELD 1, 'STRENGTH'
...
SCRNVAP(ARR)  ;"DIC screening routine 
  ...
  NEW DOSE SET DOSE=$GET(ARR("REG DOSE"))
...
PFRI1  ;  
  ...
  . WRITE "Sorry, this record does not have any registered dose forms to pick.",!
  SET USRPICK=$$MENU^TMGUSRI2(.MENU,"^")  
...
PFDN  ;  
  QUIT IEN50D606
  ;
SHOWDOSES(IEN22733)  ;"SHOW CURRENTLY REGISTERED DOSES FOR MEDICATION
  ...
  . . . WRITE "     Alt dose: ",$PIECE(ZN,"^",1),!
  DO PRESS2GO^TMGUSRI2
  QUIT  
  ;
EDTDOSES(IEN22733)  ;"EDIT CURRENT REGISTERED DOSES FOR MEDICATION
...
EDS1  ;  
  KILL MENU SET IDX=0,TABIEN=0
  SET MENU(IDX)="Pick DOSE for "_$PIECE($GET(^TMG(22733,IEN22733,0)),"^",1)      
  NEW STRIEN SET STRIEN=0
  FOR  SET STRIEN=$ORDER(^TMG(22733,IEN22733,2,SUBIEN,1,STRIEN)) QUIT:STRIEN'>0  DO
  . NEW ZN SET ZN=$GET(^TMG(22733,IEN22733,2,SUBIEN,1,STRIEN,0)) QUIT:ZN=""
  . NEW STRENGTH SET STRENGTH=$PIECE(ZN,"^",1)
  ...
  . SET IDX=IDX+1,MENU(IDX)="Dose: "_SHOW_$CHAR(9)_STRIEN    
  IF IDX=0 DO  GOTO EDSDN
  . WRITE "Sorry, this drug FORM does not have any registered doses to pick.",!
  SET USRPICK=$$MENU^TMGUSRI2(.MENU,"^")  
