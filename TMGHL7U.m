TMGHL7U ;TMG/kst-HL7 transformation utility functions ;10/20/15, 8/11/16
...
SU2     KILL TMGENV
        ...
        ;
        SET HL7INST=$PIECE($GET(TMGMSG($ORDER(TMGMSG(0)))),FS,4)
        SET HL7APP=$PIECE($GET(TMGMSG($ORDER(TMGMSG(0)))),FS,3)
...
CMPLTORD(ORDERNUM,TMGHL7MSG) ;"
       ...
       IF ORDDFN'>0 SET TMGRESULT="-1^NO DFN WAS FOUND IN ORDER" GOTO CODN
       NEW PIDIDX SET PIDIDX=+$ORDER(TMGHL7MSG("B","PID",0))
       IF PIDIDX'>0 SET TMGRESULT="-1^PID COULD NOT BE LOCATED" GOTO CODN
       NEW DFN SET DFN=+$GET(TMGHL7MSG(PIDIDX,4))
       IF DFN=ORDDFN GOTO CHNGE
       SET SSN=+$GET(TMGHL7MSG(PIDIDX,19))
       IF SSN'>0 SET TMGRESULT="-1^COULD NOT FIND SSN IN HL7 ARRAY" GOTO CODN
       NEW ORDSSN SET ORDSSN=+$PIECE($GET(^DPT(ORDDFN,0)),"^",9)
       IF ORDSSN'>0 SET TMGRESULT="-1^NO SSN FOUND FOR PATIENT IN ORDER" GOTO CODN 
       IF ORDSSN'=SSN SET TMGRESULT="-1^SSN ON HL7 DOES NOT MATCH ORDER SSN" GOTO CODN
