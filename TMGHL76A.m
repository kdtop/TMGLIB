...
FILEADT(TMGENV,TMGHL7MSG)  ;"File the ADT report.
        ...
        NEW DFN SET DFN=$G(TMGHL7MSG(PIDIDX,4))
        ...
        . IF (HASCOVIDDX=1)&(ADTEVENT="A03") DO COVDNOTE(DFN,ADTDATE,PCPDUZ)
FADTDN  ;
        QUIT TMGRESULT   
        ;
MAKENOTE(DFN,TMGDUZ,DOS,NOTETITLE,ARR)
        ...
        IF $$NOTEEXISTS(DFN,TMGDUZ,DOS,NOTETITLE,.TIUIEN)=1 DO
...
COVDNOTE(DFN,DOS,TMGDUZ)
        ...
        . NEW ALRTRESULT
        . DO INFRMALT^TMGXQAL(.ALRTRESULT,150,"ERROR CREATING BLANK NOTE FOR: "_DFN)
        ;
        ;" --- COMPOSE THE NOTE ---
        NEW IDX SET IDX=1
        SET NOTETEXT("TEXT",IDX,0)="<!DOCTYPE HTML PUBLIC ""-//WC3//DTD HTML 3.2//EN"">  <HTML><BODY>",IDX=IDX+1    ;"HTML HEADER
...
CNDN   ;
        QUIT
        ;"        
NOTEEXISTS(DFN,DUZ,DOS,NOTETITLE,RTNTIUIEN)  ;"
        ...
        FOR  SET DATE=$O(^TIU(8925,"ZTMGPTDT",DFN,DATE)) QUIT:(DATE'>0)!(DATE'[$P(DOS,".",1))  DO
        . NEW TIUIEN SET TIUIEN=0
        . FOR  SET TIUIEN=$O(^TIU(8925,"ZTMGPTDT",DFN,DATE,TIUIEN)) QUIT:TIUIEN'>0  DO
...
TYPES   ;"//----- ADT TYPES ----      
        ...
        ;
TESTADD
        NEW DFN,DOS,NOTETITLE,TIUIEN,TMGDUZ,USERNAME
        ;"
        SET DFN=75492,DOS="3200424.1500",NOTETITLE="ACUTE MEDICAL ISSUE VISIT",USERNAME="HAGOOD,EDDIE L",TMGDUZ=150
        IF $$NOTEEXISTS(DFN,TMGDUZ,DOS,NOTETITLE,.TIUIEN)=1 DO
