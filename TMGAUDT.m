TMGAUDT ;TMG/kst/Entry points for Audit API ; 10/23/13, 2/2/14
...
GETAUDT1(OUT,USERIEN,SDT,EDT,MODE) ;"PER USER AUDIT
        ...
        . . . NEW DFN SET DFN=0
        . . . FOR  SET DFN=$ORDER(TEMPARR(FILE,DUZ,DT,DFN)) QUIT:(+DFN'>0)  DO
        . . . . NEW NAME SET NAME=$GET(TEMPARR(FILE,DUZ,DT,DFN))
        . . . . NEW ADT SET ADT=DT IF ADT'["." SET ADT=ADT_".999999"
        . . . . SET SORTARR(DT\1,ADT,DFN)=NAME
        . . . . SET NAMESORTARR(NAME,DT\1,ADT)=DFN
        ...
        . . . NEW DFN SET DFN=$GET(NAMESORTARR(NAME,DAY,DT))
        . . . NEW DTSTR SET DTSTR=$$LJ^XLFSTR($$FMTE^XLFDT(DT,"7MZ"),16)
        . . . NEW STR SET STR=DFN_"^"_NAME_" -- "_DTSTR
        ...
        . . . NEW DFN SET DFN=0
        . . . FOR  SET DFN=$ORDER(SORTARR(DAY,DT,DFN)) QUIT:+DFN'>0  DO
        . . . . IF $DATA(SHOWN(DFN,DAY)) QUIT
        . . . . NEW ADT SET ADT=DT IF ADT[".9999" DO
        . . . . . SET ADT=ADT\1
        . . . . NEW NAME SET NAME=$GET(SORTARR(DAY,DT,DFN))
        . . . . NEW DTSTR SET DTSTR=$$LJ^XLFSTR($$FMTE^XLFDT(ADT,"7MZ"),16)
        . . . . NEW STR SET STR=DFN_"^"_DTSTR_" "_NAME
        . . . . SET IDX=IDX+1,OUT(IDX)=STR
        . . . . SET SHOWN(DFN,DAY)=1
GA1DN   SET OUT(0)=TMGRESULT
        QUIT
        ;
GETAUDT2(OUT,DFN,MODE,SDT,EDT) ;"PER PATIENT AUDIT
        ;"Purpose: return an audit log for 1 user, for 1 file, for given date range
        ;"Input: OUT -- PASS BY REFERENCE, an OUT parameter.
        ;"       DFN -- IEN IN file 2 (PATIENT)
        ;"       MODE -- OPTIONAL. DEFAULT=1
        ...
        KILL OUT
        SET OUT(0)="1^Success"
        IF DFN="" DO  GOTO GAU2DN
        . SET OUT(0)="-1^No patient defined"
        DO PREPDATS(.SDT,.EDT) ;"PREP DATES
        SET MODE=+$GET(MODE) IF +MODE'>0 SET MODE=1       
        NEW TEMPARR,FLDS SET FLDS=".001;.01;1"
        NEW NAMEDATEARR,NAMERPCARR,DATEARR
        NEW TMGRESULT SET TMGRESULT=$$GRPCPAUL(.TEMPARR,DFN,FLDS,"EN",SDT,EDT) ;"GET AUDIT LIST PER PATIENT FROM FILE 8994.81 (AUDIT LOG FOR RPCS)
        IF +TMGRESULT<0 DO  GOTO GAU2DN
        . SET OUT(0)=TMGRESULT
        NEW ADT SET ADT=0
        FOR  SET ADT=$ORDER(TEMPARR(8994.81,DFN,ADT)) QUIT:ADT'>0  DO
        . NEW USER,RPC,DATE,FORMAT
        . SET FORMAT=$ORDER(TEMPARR(8994.81,DFN,ADT,1,"A"))
        . SET USER=$GET(TEMPARR(8994.81,DFN,ADT,1,FORMAT))
        . SET RPC=$GET(TEMPARR(8994.81,DFN,ADT,.01,FORMAT))
        . SET DATE=$GET(TEMPARR(8994.81,DFN,ADT,.001,FORMAT))
        ...
        SET FLDS=".02;.04;4.1"
        KILL TEMPARR
        DO GETADTLS(.TEMPARR,2,DFN,FLDS,"IEN",SDT,EDT)
        NEW AUIEN SET AUIEN=0
        FOR  SET AUIEN=$ORDER(TEMPARR(2,DFN,AUIEN)) QUIT:AUIEN'>0  DO
        . NEW ADUZ,USER,OPTION,DATE,FORMAT,ADT
        . SET ADUZ=$GET(TEMPARR(2,DFN,AUIEN,.04,"I"))
        . SET USER=$GET(TEMPARR(2,DFN,AUIEN,.04,"E"))
        . SET OPTION=$GET(TEMPARR(2,DFN,AUIEN,4.1,"E"))
        . SET ADT=$GET(TEMPARR(2,DFN,AUIEN,.02,"I"))
        . SET DATE=$GET(TEMPARR(2,DFN,AUIEN,.02,"E"))
...
GRPCUAUL(OUT,TMGDUZ,SDT,EDT) ;"GET AUDIT LIST, PER USER, FROM FILE 8994.81 (LOGGABLE RPC)
        ...
        . NEW DFN SET DFN=0
        . FOR  SET DFN=$ORDER(^XUSEC(8994,"USR",TMGDUZ,ADT,DFN)) QUIT:(+DFN'>0)  DO
        . . SET OUT(8994.81,TMGDUZ,ADT,DFN)=$PIECE($GET(^DPT(DFN,0)),"^",1)
GRUAUDN QUIT TMGRESULT
        ;                
GRPCPAUL(OUT,DFN,FLDS,FLAGS,SDT,EDT) ;"GET AUDIT LIST, PER PT, FROM FILE 8994.81 (AUDIT LOG FOR RPCS)
        ...
        ;"       DFN -- Patient record number in PATIENT file to get RPC audit trail on
        ...
        SET FILE=+$GET(FILE)
        NEW TMGRESULT SET TMGRESULT="1^OK"
        SET DFN=+$GET(DFN) IF DFN'>0 DO  GOTO GRADTDN
        . SET TMGRESULT="-1^Invalid DFN parameter."
        SET FLDS=$GET(FLDS,"*")   
        DO PREPDATS(.SDT,.EDT) ;"PREP DATES
        NEW ADT SET ADT=(SDT-0.00000001)
        FOR  SET ADT=$ORDER(^XUSEC(8994,"APAT",DFN,ADT)) QUIT:(+ADT'>0)!(ADT>EDT)!(+TMGRESULT=-1)  DO
        . NEW TMGARR,TMGMSG
        . DO GETS^DIQ(8994.81,ADT_",",.FLDS,.FLAGS,"TMGARR","TMGMSG")
        . IF $DATA(TMGMSG) DO  QUIT  
        . . SET TMGRESULT="-1^"_$$GETERRST^TMGDEBU2(.TMGMSG)                             
        . MERGE OUT(8994.81,DFN)=TMGARR(8994.81)
...
GACCES1(OUT,TMGDUZ,SDT,EDT) ;"-- GET BRIEF ACCESS LIST FROM FILE 1.1 (AUDIT)
        ...
        . NEW DFN SET DFN=+$GET(TEMPARR(1.1,IEN,2.9,"E"))
        . SET OUT(1.1,TMGDUZ,ADT,DFN)=$PIECE($GET(^DPT(DFN,0)),"^",1)
...
TEST6(MODE,DFN)   ;
        ...
        IF $DATA(DFN)'>0 NEW DFN SET DFN=69928
        DO GETAUDT2(.TEMPARR,DFN,MODE,SDT,EDT) ;"GET AUDIT LIST, PER USER
        IF $DATA(TEMPARR) DO ZWRITE^TMGZWR("TEMPARR")
        QUIT
