...
SETUPOPTION(OPTION)  ;
  ...
  NEW CLRRED SET CLRRED="LightCoral"
  ...
  SET OPTION("COLORS","HEADER")=$$COLOR24PAIR^TMGUSRI8("Red",CLRAPPBACK,.TMPWCLR)
  SET OPTION("COLORS","FOOTER")=$$COLOR24PAIR^TMGUSRI8(CLRBLACK,CLRAPPBACK,.TMPWCLR)
...
LOADDATA(DATA,ARRAYREF,OPTION) ;
  ...
  . . . . SET LINE=$$MARKUPLINE(LINE)  ;"TO DO.  implement color writing in Scroller.... or use ShowLine^TMGIDE6
...
HNDONB4FTRS(TMGPSCRLARR,OPTION,INFO)  ;"HANDLE ON BEFORE FOOTERS -- Part of CONSOLE^TMGZJOB
  ...
  QUIT
  ;
SELECTEDPID()  ;"Get currently selected job number
  NEW HL SET HL=HIGHLINE(1)  
  NEW PID SET PID=$ORDER(@TMGPSCRLARR@(HL,0))
  QUIT PID
  ;
...
HNDFECOLOR(TEXT,ATAG,POS) ;"CallbackFN FOR EACH ADJUSTCOLORS color tag
  ...
  NEW ACOLOR MERGE ACOLOR=COLORS("COLORS",ATAG)
  NEW FG SET FG=$GET(ACOLOR("FG"))
  NEW BG SET BG=$GET(ACOLOR("BG"))
  SET SELECTED=+$GET(SELECTED)
  SET HIGHLIGHT=+$GET(HIGHLIGHT)
  IF SELECTED=1 DO
  . SET BG=$GET(COLORS("COLORS","SELECTED","BG"))
  . IF $$ISCLRVEC24^TMGTERM(BG)=0,+BG=BG DO
  . . SET BG=$$MAPIDXTO24BIT^TMGUSRI8(BG,1)
  IF $GET(HIGHLIGHT)=1 DO
  . NEW PCT SET PCT="0.05"
  . SET FG=$$LIGHTERCLR^TMGUSRI8(FG,PCT)
  . IF SELECTED DO
  . . SET BG=$$LIGHTERCLR^TMGUSRI8(BG,PCT)  
  . ELSE  DO
  . . SET BG=$GET(COLORS("COLORS","HIGH","BG"))
  NEW PARTA,PARTB
...
HNDONKP(TMGPSCRLARR,OPTION,INFO)  ;"Part of CONSOLE^TMGZJOB
  ...
  . NEW ACTIVECOL SET ACTIVECOL=+$GET(OPTION("COLUMNS","ACTIVE"),1)
  . IF ACTIVECOL=2 DO
  . . NEW PID SET PID=$$SELECTEDPID()  ;"Get currently selected job number
