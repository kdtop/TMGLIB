...
GTSCH4BC(TMGRESULT,DATE)  ;"
 ...
 QUIT
 ;" 
BILITEMS(DATE)  ;"THIS WILL GET ALL THE BILLABLE ITEMS AND COMPARE TO WHAT HAS ALREADY BEEN POSTED
 NEW DATAFILE,LOGFILE
 SET DATAFILE=$$RANDOMF_$$DATAFILETYPE
 SET LOGFILE=$$RANDOMF_$$LOGFILETYPE
 ;"
 SET TMGRESULT=$$SQLREQUEST($$BITEMSQL(DATE),$$WINDIR(),DATAFILE,0,LOGFILE)
 IF +TMGRESULT'=1 DO  QUIT
 . NEW ALRTRESULT
 . DO LOGLINE(LOGFILE,"!ERROR WITH SQL QUERY! "_$P(TMGRESULT,"^",2),0,0)
 . ;"DO MAKEALERT("ERROR WITH ENCOUNTER SQL: "_$P(TMGRESULT,"^",2))
 ...
 ;"GET BILLABLE ITEMS
 NEW BILLITEMS DO BILLRPC^TMGRPT2(.BILLITEMS,"T","T",4)
 ;"
 ;"ZWR BILLITEMS
 NEW ACCT SET ACCT=0
...
FUBUCKET()  ;"GET FOLLOWUP BUCKET ITEMS AND PRINT REPORT
 ...
 . IF CPT["90677" SET PREVNARVISIT(PATIENT,VISITNUM,CPT)=BALANCE
 . ELSE  IF DAYSDIFF>120 SET OLDVISIT(PATIENT,VISITNUM,CPT)=BALANCE
 . ELSE  SET OUTARR(PATIENT,VISITNUM,CPT)=BALANCE
 ;"
 ;" NOW ADJUST PREVNAR ARRAYS
 NEW PATIENT SET PATIENT=""
 FOR  SET PATIENT=$O(PREVNARVISIT(PATIENT)) QUIT:PATIENT=""  DO
 . NEW VISITNUM SET VISITNUM=""
 . FOR  SET VISITNUM=$O(PREVNARVISIT(PATIENT,VISITNUM)) QUIT:VISITNUM=""  DO
 . . IF $D(OLDVISIT(PATIENT,VISITNUM)) DO
 . . . MERGE PREVNARVISIT(PATIENT,VISITNUM)=OLDVISIT(PATIENT,VISITNUM)
 . . . KILL OLDVISIT(PATIENT,VISITNUM)
 ;"
 NEW %ZIS,IOP
 ...
 WRITE !
 WRITE "************************************************************",!
 WRITE "           Sequel Followup Bucket High Priority Report",!
 ...
 WRITE !,"**** VISITS OLDER THAN 120 DAYS ****",!
...
WRITEARR(OUTARR)  ;"OUTPUT FOLLOWUP BUCKET ARRAY
 NEW PATIENT SET PATIENT=""
 FOR  SET PATIENT=$O(OUTARR(PATIENT)) QUIT:PATIENT=""  DO
 . WRITE "  ==== ",PATIENT," ====",!
 . NEW VISITNUM SET VISITNUM=""
 . FOR  SET VISITNUM=$O(OUTARR(PATIENT,VISITNUM)) QUIT:VISITNUM=""  DO
 . . WRITE "    VISIT NUMBER -> ",VISITNUM," !! ",$G(IMPORTANT(VISITNUM)),!
 . . NEW CPT SET CPT=""
 . . FOR  SET CPT=$O(OUTARR(PATIENT,VISITNUM,CPT)) QUIT:CPT=""  DO
 . . . WRITE "       ",CPT," WITH BALANCE OF $",$G(OUTARR(PATIENT,VISITNUM,CPT)),!
 . . WRITE !
 . WRITE !
 QUIT
 ;"
