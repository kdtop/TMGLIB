...
ACTIVEPT(TMGDFN,WINDOW) ;"ACTIVE PATIENT   **NAME ALERT** --There is also an ACTIVEPT^TMGPXR01() for a different purpose.
        ...
        NEW INACTTITLE SET INACTTITLE=171  ;"<--- HARD CODED title IEN  ADDED 7/20/23
        ...
        . . . IF (TITLEIEN=XFERTITLE)!(TITLEIEN=INACTTITLE) DO  ;"TRANSFER DATE STORED FOR TESTING ELH
        . . . . SET XFER=1
        . . . . IF ONEDATE>XFERDT DO
        . . . . . SET XFERDT=ONEDATE
...
GETSCHED(TMGRESULT,BEGDT,ENDDT,STATUSES,EXCLUDE)  ;"
        ...
        . . . . SET TMGRESULT(TMGDFN,DTIDX)=REASON_"^"_WITH_"^"_COMMENT
...
GTLABSOR(TMGRESULT,TMGDFN)  ;
        ...
        QUIT
        ;"
COMORDHF(TMGRESULT,TMGDFN,ORDERTEXT)  
        ;"THIS IS CALLED FROM ORDER SIGNING. IT WILL MOVE ANY TESTS FROM
...
COLODUE(TMGDFN,TEST,DATE,DATA,TEXT)  ;"IS COLONOSCOPY DUE FOR PATIENT
        ...
        ;"
FRAILDUE(TMGDFN,TEST,DATE,DATA,TEXT)  ;"IS FRAILITY DUE FOR PATIENT
        ;"5/18/23 NOTE: PREVIOUSLY DUE WAS DECIDED IF PATIENT IF ONE HF WAS
        ;"  FOUND SINCE THE BEGINNING OF THE YEAR. NOW WE NEED 2, SO I ADDED A
        ;"  COUNT AND WE WILL TEST BY THAT
        SET TEST=1,DATE=$$TODAY^TMGDATE() ;"TURN ON BY DEFAULT
        ...
        . . IF BDATE<THISDATE SET COUNT=COUNT+1
        ;"USE COUNT INSTEAD PER ABOVE NOTE    IF BDATE<MRD DO  
        IF COUNT>2 DO
        . SET TEST=0
        . SET DATE=0
        QUIT
