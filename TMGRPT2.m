...
CPTIIRPT
  ...
  . NEW ACEARB,MEDSTR
  . SET ACEARB=$$CHKMEDS(TMGDFN,49,.MEDSTR,"4010F")
  . IF ACEARB=1 SET DATAARR(NAME,"ACE/ARB MEDICATION")=MEDSTR_" (JUST CHECK FOR NOW)"
  . ;"
  . ;"Get AtHome BPs
  ;"KILL DATAARR
...
TNCARRPT(DATE)  ;"
  ...
  . . . SET PTCHECKARR(TMGDFN)=$P($G(^DPT(TMGDFN,0)),"^",1)_" has "_$P($G(^DIC(36,INSIEN,0)),"^",1)
  IF $D(PTCHECKARR) DO
  . WRITE !
  . WRITE "************************************************************",!
...
GETCPT(TMGDFN,KEY,VALUE,TMGRESULT,DATE) 
  ...
  IF +$$HASCPT^TMGRPU1(TMGDFN,CPT,$$FIRSTYR^TMGDATE,9999999)=1 SET REPORTED=1
  ...
  ELSE  DO
  . IF +$$HASCPT^TMGRPU1(TMGDFN,CPT,$$FIRSTYR^TMGDATE,9999999)=1 SET TMGRESULT=TMGRESULT_" (ALREADY REPORTED)"
  QUIT REPORTED
  ;"  
