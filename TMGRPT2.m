...
GETITEMS(TMGRESULT,GROUP,BDATE,EDATE,FILTER)  ;"
  ...
  NEW OPTION
  IF (FILTER=0)!(FILTER=1) SET OPTION("CPT")=1
  IF (FILTER=0)!(FILTER=2) SET OPTION("CPT-II")=1 
  IF (FILTER=0)!(FILTER=3) SET OPTION("ICD")=1
  IF FILTER=0 DO
  . SET OPTION("IMM")=1
  . SET OPTION("HF")=1
  . SET OPTION("TIU")=1
...
TEST
  ...
  . SET FOUND=$$CHKNOTE(TMGDFN,1400,BEGINDATE,.DATESTR,"1111F",0)
  . IF FOUND=1 DO
