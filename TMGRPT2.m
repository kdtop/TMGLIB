...
NEXTAPPT(TMGRESULT,DFN,TODAY)  ;"
  ...
  NEW DONE SET DONE=0
  FOR  SET DATE=$ORDER(^TMG(22723,DFN,1,"B",DATE)) QUIT:(DATE'>0)!(DONE=1)  DO
  . NEW IDX SET IDX=$ORDER(^TMG(22723,DFN,1,"B",DATE,0))
  . ;"10/2/20 - ONLY EXCLUDE CANCELLED IF $P($G(^TMG(22723,DFN,1,IDX,0)),"^",7)'="A" QUIT
  . IF $P($G(^TMG(22723,DFN,1,IDX,0)),"^",7)="C" QUIT
...
OVERDUE(STATUS,DUE)
  ...
  QUIT RESULT
  ;"
TOPICS(ROOT,DFN,ID,ALPHA,OMEGA,DTRANGE,REMOTE,MAX,ORFHIE) ;"TOPICS report
  ;"Purpose: Entry point, as called from CPRS REPORT system
  ;"Input: ROOT -- Pass by NAME.  This is where output goes
  ;"       DFN -- Patient DFN ; ICN for foriegn sites
  ;"       ID --
  ...
  DO TOPICS^TMGTIUT3(.TOPICARR,"LIST",DFN,"HPI")
  SET IDX=0
  ...
  . ;"DO TOPICS^TMGTIUT3(.TOPICARR,"SUM1",DFN,"HPI")  <-  
...
AWVREMS(REMLIST)  ;"list of reminders to include on AWV report
  ...
  SET REMLIST(224)="MAMMOGRAM/BREAST IMAGING^Annually"
  SET REMLIST(293)="TOBACCO CESSATION^Annually"
