...
GETITEMS(TMGRESULT,GROUP,BDATE,EDATE,FILTER)  ;"
  ...
  . . . . SET TMGRESULT(TMPPAT,"Bill with ICD "_$P(ICDSTR," --",1),THISDATE)=""  
  ;"Handle Routines
...
NEXTAPPT(TMGRESULT,DFN)  ;"
  ...
  . NEW WITHDR SET WITHDR=$P($G(^TMG(22723,DFN,1,IDX,0)),"^",3)
...
CPTIIRPT
  ...
  . IF $$HASNOTES(DFN,BEGINDATE)=0 QUIT
  ...
  . . . . . NEW CPTICD,REPORTED
  . . . . . SET REPORTED=$$GETCPT(DFN,KEY,VALUE,.CPTICD,FMDATE)
...
TNCARRPT(DATE)  ;"
  ...
  NEW DFN SET DFN=0
  NEW PTCHECKARR
  FOR  SET DFN=$O(SCHARRAY(DFN)) QUIT:DFN'>0  DO
  . NEW INSIEN SET INSIEN=0
  . FOR  SET INSIEN=$O(^DPT(DFN,.312,"B",INSIEN)) QUIT:INSIEN'>0  DO
  . . IF $D(INSARRAY(INSIEN)) DO
  . . . SET PTCHECKARR(DFN)=$P($G(^DPT(DFN,0)),"^",1)_" has "_$P($G(^DIC(36,INSIEN,0)),"^",1)
  IF $D(PTCHECKARR) DO
  . WRITE !
  . WRITE "************************************************************",!
  . WRITE "              PATIENTS WITH INSURANCES TO BE VERIFIED",!
  . WRITE "              APPOINTMENT DATE: ",$$EXTDATE^TMGDATE(DATE),!
  . WRITE "************************************************************",!
  . WRITE "                                            (From TMGRPT2.m)",!!
  . SET DFN=0
  . FOR  SET DFN=$O(PTCHECKARR(DFN)) QUIT:DFN'>0  DO
  . . WRITE $G(PTCHECKARR(DFN)),!,!
...
HOMEBPS(BDATE,DATAARR)  
  ...
  . . . SET SYSSTR="3077F"
  . . IF SYSSTR'="" DO
  . . . IF $$HASCPT^TMGRPU1(DFN,SYSSTR,$P(DATE,".",1),$P(DATE,".",1)) SET SYSSTR=SYSSTR_" (ALREADY REPORTED)"
  ...
  . . . SET DIASTR="3079F"
  . . IF DIASTR'="" DO
  . . . IF $$HASCPT^TMGRPU1(DFN,DIASTR,$P(DATE,".",1),$P(DATE,".",1)) SET DIASTR=DIASTR_" (ALREADY REPORTED)"
...
NOTETEXT(TIUIEN)  ;"RETURNS TEXT OF A NOTE IN A STRING
  ...
  QUIT STRING
  ;"
GETCPT(DFN,KEY,VALUE,TMGRESULT,DATE) 
  ...
  IF +$$HASCPT^TMGRPU1(DFN,CPT,$$FIRSTYR^TMGDATE,9999999)=1 SET REPORTED=1
  ...
  . SET REPORTED=0
  . IF +$$HASCPT^TMGRPU1(DFN,CPT,$P(FMDATE,".",1)_".0000",$P(FMDATE,".",1)_".999999")=1 SET TMGRESULT=TMGRESULT_" (ALREADY REPORTED)",REPORTED=1
  ELSE  DO
  . IF +$$HASCPT^TMGRPU1(DFN,CPT,$$FIRSTYR^TMGDATE,9999999)=1 SET TMGRESULT=TMGRESULT_" (ALREADY REPORTED)"
  QUIT REPORTED
  ;"  
...
SMOKER(OUTARR)
  ...
  QUIT
  ;"
HASNOTES(DFN,BEGINDATE)  ;"This function returns 0 (if none) or date of note
                         ;"the patient had after BEGINDATE
  NEW TMGRESULT SET TMGRESULT=0
  ;"NEW DATE SET DATE=$P(BEGINDATE,".",1)
  NEW DATE SET DATE=$E(BEGINDATE,1,3)_"0101"
  SET DATE=$O(^TIU(8925,"ZTMGPTDT",DFN,DATE))
  IF DATE>0 DO
  . SET TMGRESULT=1
  QUIT TMGRESULT
  ;"
...
MISOVCHG(BDATE,EDATE)  ;"Print appts that don't have corresponding OV
  ...
  . . . IF $$HASCPT2^TMGRPU1(DFN,99,THISDATE_".000000",THISDATE_".999999")=0 DO
  . . . . NEW NOTEDATE SET NOTEDATE=THISDATE
  . . . . FOR  SET NOTEDATE=$O(^TIU(8925,"ZTMGPTDT",DFN,NOTEDATE)) QUIT:(NOTEDATE=0)!(NOTEDATE'[THISDATE)  DO
  . . . . . NEW NOTEIEN SET NOTEIEN=$O(^TIU(8925,"ZTMGPTDT",DFN,NOTEDATE,0))
  . . . . . NEW NOTETITLE SET NOTETITLE=$P($G(^TIU(8925,NOTEIEN,0)),"^",1)
  . . . . . NEW OVTITLE SET OVTITLE=$P($G(^TIU(8925.1,NOTETITLE,"TMGH")),"^",1)
  . . . . . ;"WRITE OVTITLE,!
  . . . . . IF OVTITLE'="Y" QUIT
  . . . . . ;"WRITE DFN," - ",NOTEDATE,"  -  ",NOTETITLE,!
  . . . . . WRITE $P($G(^DPT(DFN,0)),"^",1)_" MISSING OV FOR "_$$EXTDATE^TMGDATE(DATE),!  
  QUIT
  ;"
TMGAWV(ROOT,DFN,ID,ALPHA,OMEGA,DTRANGE,REMOTE,MAX,ORFHIE) ;"AWV report
  ;"Purpose: Entry point, as called from CPRS REPORT system
  ;"Input: ROOT -- Pass by NAME.  This is where output goes
  ;"       DFN -- Patient DFN ; ICN for foriegn sites
  ;"       ID --
  ...
  . NEW REMRESULT
  . SET REMRESULT=$$DOREM^TMGPXR03(DFN,REMIEN,5,$$TODAY^TMGDATE,1)
  . NEW STATUS,DUE,DONE
...
AWVREMS(REMLIST)  ;"list of reminders to include on AWV report
  SET REMLIST(224)="MAMMOGRAM/BREAST IMAGING"
  SET REMLIST(241)="PREVNAR-13"
  SET REMLIST(257)="PNEUMOCOCCAL-23"
  SET REMLIST(220)="FECAL OCCULT BLOOD TEST"
  SET REMLIST(232)="ADVANCE CARE PLANNING"
  ;" Don't use for now -> SET REMLIST(261)="CARDIOVASCULAR DISEASE SCREENING" 
  SET REMLIST(228)="BONE DENSITY"
  SET REMLIST(223)="COLONOSCOPY"
  SET REMLIST(242)="DIABETIC EYE EXAM"
  SET REMLIST(243)="DIABETIC FOOT EXAM"
  SET REMLIST(267)="STD SCREENING"
  SET REMLIST(267)="GLAUCOMA SCREENING"
  SET REMLIST(220)="FECAL OCCULT BLOOD TEST"
  SET REMLIST(254)="HEMOGLOBIN A1C"
  SET REMLIST(127)="HEPATITIS C"
  SET REMLIST(211)="HIV SCREENING" 
  SET REMLIST(281)="PAP SMEAR"  
  SET REMLIST(220)="FECAL OCCULT BLOOD TEST"
  SET REMLIST(225)="ABDOMINAL AORTIC ANEURYSM (AAA)"
  SET REMLIST(263)="PSA SCREENING"
  SET REMLIST(244)="LOW DOSE CT SCREENING"
  SET REMLIST(256)="LIPID PANEL"
  SET REMLIST(219)="SMOKING CESSATION"
  SET REMLIST(229)="HEPATITIS B RISK ASSESSMENT"
  QUIT
  ;"
