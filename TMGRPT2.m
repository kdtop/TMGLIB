...
XTRADATA(TMGRESULT,TMGDFN)  ;"
  ...
  NEW LASTPHQ9 SET LASTPHQ9=$$PAIDPHQ9^TMGSSQL7(TMGDFN)
  SET TMGRESULT(IDX)="<HR>^<HR>^<HR>",IDX=IDX+1
  ...
  SET TMGRESULT(IDX)="LAST PAID PHQ-9^"_LASTPHQ9,IDX=IDX+1
  QUIT
  ;"
...
BILLRPC(TMGRESULT,BDATE,EDATE,FILTER)   ;"
  ...
  IF FILTER=4 DO  QUIT
  . NEW NAME SET NAME=""
  . FOR  SET NAME=$O(DATA(NAME)) QUIT:NAME=""  DO
  . . NEW SEQNUM SET SEQNUM=$P($P(NAME,"(",2),")",1)
  . . NEW ITEM SET ITEM=""
  . . FOR  SET ITEM=$O(DATA(NAME,ITEM)) QUIT:ITEM=""  DO
  . . . NEW THISITEM,CPT,ICD
  . . . IF (ITEM["BMI")&(ITEM["w/") DO
  . . . . SET THISITEM=$P($P(ITEM,"(",2),")",1)
  . . . . SET CPT=$P(THISITEM," w/",1)
  . . . . SET ICD=$P(THISITEM," w/ ",2)
  . . . . SET TMGRESULT(SEQNUM,"CPT",CPT_" "_ICD)=ICD
  . . . ELSE  IF ($$UP^XLFSTR(ITEM)["PAIN")&($$UP^XLFSTR(ITEM)["DOCUMENTED") DO
  . . . . ;WRITE ITEM,!,!
  . . . . SET THISITEM=$P(ITEM," (",1)
  . . . . ;WRITE "->",THISITEM,"(",SEQNUM,")",!,!
  . . . . SET TMGRESULT(SEQNUM,"CPT",THISITEM)=""
  . . . ELSE  IF ITEM["Bill with CPT" DO
  . . . . SET CPT=$P(ITEM,"Bill with CPT ",2)
  . . . . SET TMGRESULT(SEQNUM,"CPT",CPT)=""
  . . . ELSE  IF ITEM["Meds Documented" DO
  . . . . SET TMGRESULT(SEQNUM,"CPT","1159F")=""
  . . . . SET TMGRESULT(SEQNUM,"CPT","1160F")=""
  . . . ELSE  IF ITEM["CKD" DO
  . . . . ;"IGNORE
  . . . ELSE  IF ITEM["BP Codes" DO
  . . . . SET THISITEM=$P(ITEM,"Enter: ",2)
  . . . . NEW CPT1,CPT2 
  . . . . SET CPT1=$P(THISITEM,",",1)
  . . . . SET CPT2=$P(THISITEM,",",2)
  . . . . SET TMGRESULT(SEQNUM,"CPT",CPT1)=""
  . . . . SET TMGRESULT(SEQNUM,"CPT",CPT2)=""
  . . . ELSE  IF ITEM["BMI is over" DO
  . . . . ;"IGNORE
  . . . ELSE  IF ITEM["(" DO
  . . . . SET CPT=$P($P(ITEM,"(",2),")",1)
  . . . . SET CPT=$P(CPT,"/",1)
  . . . . SET CPT=$P(CPT,"QW",1)
  . . . . IF CPT["FLOWSHEET" QUIT
  . . . . SET TMGRESULT(SEQNUM,"CPT",CPT)=""
  . . . ELSE  DO
  . . . . ;"IGNORE THE REST
...
FORMAT(TMGRESULT,DATA,SORT)  ;"
  ...
  . . . IF $LENGTH(Y,":")=3 SET Y=$PIECE(Y,":",1,2)  
...
RTFFMT(TMGRESULT,DATA,SORT)  ;"
  ...
  . SET LINE=LINE+1
  . SET TMGRESULT(LINE)="   ===CPTS===\line"
  . NEW ITEM SET ITEM=""
  . NEW ICDARR,CPTARR,OTHERARR
  . FOR  SET ITEM=$ORDER(DATA(NAME,ITEM)) QUIT:ITEM=""  DO
  . . NEW DATE SET DATE=0
  . . FOR  SET DATE=$ORDER(DATA(NAME,ITEM,DATE)) QUIT:DATE'>0  DO
  . . . NEW Y SET Y=DATE D DD^%DT
  . . . IF $LENGTH(Y,":")=3 SET Y=$PIECE(Y,":",1,2)
  . . . IF ITEM["Bill with ICD" DO
  . . . . NEW ICDSTR SET ICDSTR=$$TRIM^XLFSTR($P(ITEM,"Bill with ICD",2))_" "
  . . . . NEW ICDDEF SET ICDDEF=+$O(^ICD9("AB",ICDSTR,0))
  . . . . IF ICDDEF>0 SET ICDDEF=$P($G(^ICD9(ICDDEF,68,1,1)),"^",1)
  . . . . SET ICDSTR=ICDSTR_"- "_ICDDEF
  . . . . SET ICDARR("   -> {\i "_ICDSTR_" } \line")=""
  . . . ELSE  IF ITEM["Bill with CPT" DO
  . . . . SET CPTARR("   {\f1 [_] }  -> {\i "_$P(ITEM,"Bill with CPT",2)_" } \line")=""
  . . . ELSE  DO
  . . . . SET OTHERARR("   {\f1 [_] }  -> {\i "_ITEM_" } \line")=""
  . . . ;"SET LINE=LINE+1
  . . . ;"SET TMGRESULT(LINE)="   {\f1 [_] } "_Y_" -> {\i "_ITEM_" } \line"
  . IF $D(CPTARR) DO    ;"ADDED NEW CONDITIONAL 8/22/23
  . . NEW ONELINE SET ONELINE=""
  . . FOR  SET ONELINE=$O(CPTARR(ONELINE)) QUIT:ONELINE=""  DO
  . . . SET LINE=LINE+1
  . . . SET TMGRESULT(LINE)=ONELINE
  . IF $D(OTHERARR) DO    ;"ADDED NEW CONDITIONAL 8/22/23
  . . SET LINE=LINE+1
  . . SET TMGRESULT(LINE)="\line"
  . . NEW ONELINE SET ONELINE=""
  . . FOR  SET ONELINE=$O(OTHERARR(ONELINE)) QUIT:ONELINE=""  DO
  . . . SET LINE=LINE+1
  . . . SET TMGRESULT(LINE)=ONELINE 
  . IF $D(ICDARR) DO    ;"ADDED NEW CONDITIONAL 8/22/23
  . . SET LINE=LINE+1
  . . SET TMGRESULT(LINE)="\line"
  . . SET LINE=LINE+1
  . . SET TMGRESULT(LINE)="   ===ICDS===\line"
  . . NEW ONELINE SET ONELINE=""
  . . FOR  SET ONELINE=$O(ICDARR(ONELINE)) QUIT:ONELINE=""  DO
  . . . SET LINE=LINE+1
  . . . SET TMGRESULT(LINE)=ONELINE 
  . SET LINE=LINE+1
  . SET TMGRESULT(LINE)="\line"
...
GETITEMS(TMGRESULT,GROUP,BDATE,EDATE,FILTER)  ;"
  ...
  ;"       FILTER -- 0,1,2,3 (0=AllBillable, 1=CPT, 2=CPT-II, 3=ICDs)
  ...
  ;"SET END DATE
  ;"SET EDATE=$GET(EDATE)
  SET EDATE=$GET(EDATE,"T")
  ;"IF EDATE="" SET EDATE="T"
  SET X=EDATE DO ^%DT
  ;"SET EDATE=$PIECE(Y,".",1)_".9999"
  SET EDATE=(Y\1)+0.999999
  NEW OPTION
  IF (FILTER=0)!(FILTER=1)!(FILTER=4) SET OPTION("CPT")=1
  IF (FILTER=0)!(FILTER=2)!(FILTER=4) SET OPTION("CPT-II")=1 
  IF (FILTER=0)!(FILTER=3)!(FILTER=4) SET OPTION("ICD")=1
  IF (FILTER=0)!(FILTER=4) DO
  . SET OPTION("IMM")=1
  . SET OPTION("HF")=1
  . SET OPTION("TIU")=1
  . SET OPTION("BTAGS")=1
  ...
  ;"IF FILTER>1 GOTO C2
  IF $GET(OPTION("IMM")) FOR  SET BILLIEN=$ORDER(^TMG(22727,GROUPIEN,1,BILLIEN)) QUIT:BILLIEN'>0  DO       
  . NEW IMMIEN,TEXT
  ...
  ;"Get Health Factors
  SET BILLIEN=0
  IF $GET(OPTION("HF")) FOR  SET BILLIEN=$ORDER(^TMG(22727,GROUPIEN,2,BILLIEN)) QUIT:BILLIEN'>0  DO
  . NEW HFIEN,TEXT
  ...
  . . . NEW TMGDFN SET TMGDFN=$PIECE(ZN,"^",2)
  . . . NEW NAME SET NAME=$$GETNAME(TMGDFN)
  . . . IF TEXT["PHQ-9 Done" DO
  . . . . NEW PHQTEXT SET PHQTEXT=$$PHQ9CODE^TMGSSQL7(TMGDFN)
  . . . . IF PHQTEXT'="" SET TEXT=PHQTEXT
  . . . SET TMGRESULT(NAME,TEXT,DATE)=""
  ;"Get TIU Notes
  SET BILLIEN=0
  IF $GET(OPTION("TIU")) FOR  SET BILLIEN=$ORDER(^TMG(22727,GROUPIEN,3,BILLIEN)) QUIT:BILLIEN'>0  DO
  . NEW TIUIEN,TEXT
  ...
  . . . ;"   EDDIE REMOVED FOR NOW. 4/13/23 IF ACEARB=1 SET TMGRESULT(NAME,"ACE/ARB "_MEDSTR,DATE)=""
  ;"Look for billing tags
  ;"OLD NOTE SEARCH IS AT THE BOTTOM OF THIS FILE  1/16/25
  IF $GET(OPTION("BTAGS")) DO 
  ...
  . . . . NEW ONELINE SET ONELINE=$$UP^XLFSTR($GET(^TIU(8925,TIUIEN,"TEXT",LINE,0)))
  ...
  DO GETCODES(.ICDARRAY,.CPTARRAY,BDATE,EDATE,.OPTION)
  ...
  . . . . SET TMGRESULT(TMPPAT,"Bill with ICD "_ICDCODE,ADT)=""
  ...
  . . . . NEW QUANT SET QUANT=+$G(CPTARRAY(TMPPAT,CPTSTR,ADT))
  . . . . IF QUANT>1 SET CPTCODE=CPTCODE_" (x"_QUANT_")"
  . . . . SET TMGRESULT(TMPPAT,"Bill with CPT "_CPTCODE,ADT)=""
  ;"Handle Routines
  ;"IF FILTER=1 GOTO GDDN
C2 ;"CPT-II CODES  
  SET BILLIEN=0
  IF $GET(OPTION("CPT-II")) FOR  SET BILLIEN=$ORDER(^TMG(22727,GROUPIEN,5,BILLIEN)) QUIT:BILLIEN'>0  DO
...
GETICDS(TMGRESULT,BDATE,EDATE)  ;"
  ...
  QUIT
  ;
GETCODES(ICDARRAY,CPTARRAY,BDATE,EDATE,OPTION)  ;"
  ;"This report will return all the ICDs for patients seen for the date range specified
  ...
  . . NEW NAME SET NAME=$$GETNAME(TMGDFN)
  . . NEW ICDIEN SET ICDIEN=0
  . . IF $GET(OPTION("ICD")) FOR  SET ICDIEN=$ORDER(^AUPNVPOV("C",TMGDFN,ICDIEN)) QUIT:+ICDIEN'>0  DO
  . . . NEW ZN SET ZN=$GET(^AUPNVPOV(ICDIEN,0)) QUIT:ZN=""
  . . . NEW VSTIEN SET VSTIEN=+$PIECE(ZN,"^",3) QUIT:VSTIEN'>0
  . . . NEW VSTZN SET VSTZN=$GET(^AUPNVSIT(VSTIEN,0)) QUIT:VSTZN=""
  . . . NEW VSTDT SET VSTDT=$PIECE(VSTZN,"^",1)
  ...
  . . IF $GET(OPTION("CPT")) FOR  SET CPTIEN=$ORDER(^AUPNVCPT("C",TMGDFN,CPTIEN)) QUIT:+CPTIEN'>0  DO
  ...
  . . . NEW QUANT SET QUANT=$P(ZN,"^",16)
  . . . NEW CPTZN SET CPTZN=$GET(^ICPT(VCPTIEN,0)) QUIT:CPTZN=""
  . . . NEW ADT SET ADT=$ORDER(^ICPT(VCPTIEN,62,"B",""),-1)
  . . . NEW DESCR
  . . . IF ADT>0 DO
  . . . . NEW PTR SET PTR=$ORDER(^ICPT(VCPTIEN,62,"B",ADT,0)) QUIT:PTR'>0
  . . . . SET DESCR=$GET(^ICPT(VCPTIEN,62,PTR,1))
  . . . ELSE  DO
  . . . . SET DESCR=$P($GET(^ICPT(VCPTIEN,0)),"^",2)
  . . . NEW STR SET STR=$PIECE(CPTZN,"^",1)_" -- "_DESCR
  . . . SET CPTARRAY(NAME,STR,VSTDT)=QUANT
  QUIT  
...
PTMISSED  ;" 
  ...
  ;"         least 3 months
  NEW PTARRAY,VISITARR,CANCELIEN,NSIEN,DONE,EXTRAARR
  ...
  . . . IF VISITARR[DOCTYPE DO  QUIT
  . . . . SET DONE=1
  . . . . NEW NAME SET NAME=$PIECE($GET(^DPT(PATIEN,0)),"^",1)_" - "
  . . . . SET EXTRAARR(NAME,PATIEN)=""
  ...
  ;"
  QUIT  ;"I ADDED THIS LOWER PORTION BUT IT ISN'T NEEDED   2/13/25
  ;"Now add extra patients
  IF $D(EXTRAARR) DO
  . WRITE !,!,!,!,"****THE FOLLOWING ARE ACTIVE PATIENTS WITHOUT APPOINTMENTS.****",!,!
  . NEW NAME SET NAME=""
  . FOR  SET NAME=$O(EXTRAARR(NAME)) QUIT:NAME=""  DO
  . . IF NAME["TOPPENBERG" QUIT
  . . SET PATIEN=0
  . . FOR  SET PATIEN=$O(EXTRAARR(NAME,PATIEN)) QUIT:PATIEN'>0  DO
  . . . NEW PHONENODE SET PHONENODE=$GET(^DPT(PATIEN,.13))
  . . . NEW HOMEPHONE SET HOMEPHONE=$PIECE(PHONENODE,"^",1)
  . . . NEW CELLPHONE SET CELLPHONE=$PIECE(PHONENODE,"^",4)
  . . . NEW PHONEMESSAGE SET PHONEMESSAGE=""
...
NEXTAPPT(TMGRESULT,TMGDFN,TODAY)  ;"
  ...
  IF TODAY=2 DO  QUIT
  . NEW UPCOMING,TODAY
  . DO NEXTAPPT(.UPCOMING,TMGDFN,0)
  . DO NEXTAPPT(.TODAY,TMGDFN,1)
  . SET TMGRESULT=""
  . IF TODAY'["LAST" SET TMGRESULT=TODAY
  . IF UPCOMING'["NO UPCOMING" DO
  . . IF TMGRESULT'="" SET TMGRESULT=TMGRESULT_", "
  . . SET TMGRESULT=TMGRESULT_$P(UPCOMING,":",2,99)
  . IF TMGRESULT="" DO
  . . SET TMGRESULT="NO UPCOMING APPOINTMENT"
  . ELSE  DO
  . . SET TMGRESULT="Upcoming appt: "_TMGRESULT
  ...
  IF (TODAY=1)&(TMGRESULT="") DO  ;"8/1/23
  . NEW LASTAPPT,FOUND  ;"BASED ON NEWEST OFFICE NOTE
  . SET LASTAPPT=9999999,FOUND=0
  . FOR  SET LASTAPPT=$O(^TIU(8925,"ZTMGPTDT",TMGDFN,LASTAPPT),-1) QUIT:(LASTAPPT'>0)!(FOUND=1)  DO
  . . NEW TIUIEN SET TIUIEN=0
  . . FOR  SET TIUIEN=$O(^TIU(8925,"ZTMGPTDT",TMGDFN,LASTAPPT,TIUIEN)) QUIT:(TIUIEN'>0)!(FOUND=1)  DO
  . . . NEW TIUTYPE SET TIUTYPE=$P($G(^TIU(8925,TIUIEN,0)),"^",1)
  . . . NEW TIUOVNOTE SET TIUOVNOTE=$P($G(^TIU(8925.1,TIUTYPE,"TMGH")),"^",1)
  . . . IF TIUOVNOTE'="" SET FOUND=1
...
TEST  ;
  ...
  SET BEGINDATE=3250110.0000  ;"REMOVE!!!!
  ...
  . NEW FOUND1,FOUND2,FOUND,DATESTR SET FOUND=0
  . SET FOUND1=$$CHKNOTE(TMGDFN,2031,BEGINDATE,.DATESTR,"1111F",0)
  . SET FOUND2=$$CHKNOTE(TMGDFN,1400,BEGINDATE,.DATESTR,"1111F",0)
  . IF (FOUND1=1)!(FOUND2=1) SET FOUND=1
  . IF FOUND=1 DO
...
HOMEBPS(BDATE,DATAARR)  ;  
  ...
  NEW NOTETITLES SET NOTETITLES="^2066^2207^" ;" ^HOMEBP^BRIEFNURSENOTE^"
  NEW THISTITLE SET THISTITLE=0
  FOR  SET THISTITLE=$O(^TIU(8925,"B",THISTITLE)) QUIT:THISTITLE'>0  DO
  . NEW TEMPTITLE SET TEMPTITLE="^"_THISTITLE_"^"
  . IF NOTETITLES'[TEMPTITLE QUIT
  . NEW TIUIEN SET TIUIEN=0
  . FOR  SET TIUIEN=$O(^TIU(8925,"B",THISTITLE,TIUIEN)) QUIT:TIUIEN'>0  DO
...
GETCPT(TMGDFN,KEY,VALUE,TMGRESULT,DATE)   ;
  ...
  ;"IF KEY="LDL" DO    ;"removing as UHC doesn't monitor this code yet
  ;". IF TEMPVAL<100 SET CPT="3048F"
  ;". IF (TEMPVAL>99)&(TEMPVAL<131) SET CPT="3049F"
  ;". IF TEMPVAL>130 SET CPT="3050F"
  ;"IF KEY="MALB" DO   ;"removing as UHC doesn't monitor this code yet
...
INSCARDS   ;"GET ALL INSURANCE CARDS
  ...
  ;"
TSKNOADD  ;"
  ;" Report that prints showing signed Patient Tasks without addendums
  NEW PTTASK SET PTTASK=183
  NEW TIUIEN SET TIUIEN=0
  NEW SIGNEDARR,UNSIGNEDARR
  FOR  SET TIUIEN=$O(^TIU(8925,"B",PTTASK,TIUIEN)) QUIT:TIUIEN'>0  DO
  . NEW STATUS SET STATUS=$P($G(^TIU(8925,TIUIEN,0)),"^",5)
  . IF STATUS'=7 QUIT
  . NEW ADDENDIEN SET ADDENDIEN=0
  . NEW ADDEND SET ADDEND=0
  . FOR  SET ADDENDIEN=$O(^TIU(8925,"DAD",TIUIEN,ADDENDIEN)) QUIT:ADDENDIEN'>0  DO
  . . SET ADDEND=1
  . IF ADDEND=1 QUIT  ;"
  . NEW ADDLIEN SET ADDLIEN=0
  . NEW PATIENT SET PATIENT=$P($G(^TIU(8925,TIUIEN,0)),"^",2)
  . SET PATIENT=$P($G(^DPT(PATIENT,0)),"^",1)
  . IF $$UP^XLFSTR(PATIENT)["ZZTEST" QUIT
  . IF $$UP^XLFSTR(PATIENT)["TABLE" QUIT
  . NEW NOTEDATE SET NOTEDATE=$P($G(^TIU(8925,TIUIEN,0)),"^",7)
  . FOR  SET ADDLIEN=$O(^TIU(8925.7,"B",TIUIEN,ADDLIEN)) QUIT:ADDLIEN'>0  DO
  . . NEW ADDLZN SET ADDLZN=$G(^TIU(8925.7,ADDLIEN,0))
  . . NEW COMPLETE SET COMPLETE=$P(ADDLZN,"^",2)
  . . NEW COSIGNER SET COSIGNER=+$P(ADDLZN,"^",5)
  . . NEW ADDLUSER SET ADDLUSER=$P(ADDLZN,"^",3)
  . . ;"IF COMPLETE=0 DO
  . . IF COSIGNER>0 DO
  . . . SET SIGNEDARR(ADDLUSER,PATIENT,NOTEDATE)=""
  . . ELSE  DO
  . . . SET UNSIGNEDARR(ADDLUSER,PATIENT,NOTEDATE)=""
  NEW %ZIS,IOP
  SET IOP="S121-LAUGHLIN-LASER"
  DO ^%ZIS  ;"standard device call
  IF POP DO  GOTO ATDn
  ...
  USE IO
  WRITE "************************************************************",!
  WRITE "               PATIENT TASKS THAT DO NOT HAVE ADDENDUMS",! 
  WRITE "************************************************************",!,!
  IF $D(UNSIGNEDARR) DO
  . WRITE " ====UNSIGNED BY COSIGNER====",!
  . NEW USER SET USER=0
  . FOR  SET USER=$O(UNSIGNEDARR(USER)) QUIT:USER=""  DO
  . . WRITE "USER: ",$P($G(^VA(200,USER,0)),"^",1),!
  . . NEW PATIENT SET PATIENT=""
  . . FOR  SET PATIENT=$O(UNSIGNEDARR(USER,PATIENT)) QUIT:PATIENT=""  DO
  . . . NEW DATES SET DATES=""
  . . . NEW ONEDATE SET ONEDATE=""
  . . . FOR  SET ONEDATE=$O(UNSIGNEDARR(USER,PATIENT,ONEDATE)) QUIT:ONEDATE=""  DO
  . . . . IF DATES'="" SET DATES=DATES_","
  . . . . SET DATES=DATES_$$EXTDATE^TMGDATE(ONEDATE,1)
  . . . WRITE "    ",PATIENT,": ",DATES,!
  . WRITE !
    IF $D(SIGNEDARR) DO
  . WRITE " ====SIGNED BY COSIGNER====",!
  . NEW USER SET USER=0
  . FOR  SET USER=$O(SIGNEDARR(USER)) QUIT:USER=""  DO
  . . WRITE "USER: ",$P($G(^VA(200,USER,0)),"^",1),!
  . . NEW PATIENT SET PATIENT=""
  . . FOR  SET PATIENT=$O(SIGNEDARR(USER,PATIENT)) QUIT:PATIENT=""  DO
  . . . NEW DATES SET DATES=""
  . . . NEW ONEDATE SET ONEDATE=""
  . . . FOR  SET ONEDATE=$O(SIGNEDARR(USER,PATIENT,ONEDATE)) QUIT:ONEDATE=""  DO
  . . . . IF DATES'="" SET DATES=DATES_","
  . . . . SET DATES=DATES_$$EXTDATE^TMGDATE(ONEDATE,1)
  . . . WRITE "    ",PATIENT,": ",DATES,!
  . WRITE !
...
SETHTML2(ROOT,RESULTS,TITLE,HEADING,COLNUMS)  ;
  ...
  ;"
OLDCODE
  ;"IGNORE THIS AND USE NEWNOTESRCH
  SET BILLIEN=0
  IF $GET(OPTION("BTAGS")) FOR  SET BILLIEN=$ORDER(^TMG(22727,GROUPIEN,4,BILLIEN)) QUIT:BILLIEN'>0  DO 
  . NEW STR,TEXT
  . SET STR=$$UP^XLFSTR($PIECE($GET(^TMG(22727,GROUPIEN,4,BILLIEN,0)),"^",1))
  . SET TEXT=$PIECE($GET(^TMG(22727,GROUPIEN,4,BILLIEN,0)),"^",2)
  . NEW TIUDATE,TIUIEN,FOUND 
