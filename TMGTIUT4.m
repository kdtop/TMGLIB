...
DUPNOTE(TMGRESULT,TMGDFN,TMGDUZ,TIUDATE,TITLEIEN)  ;" RPC: TMG SCANNER DOES NOTE EXIST
  ...
  QUIT
  ;"
NEWSTATS(TMGRESULT,DA,NEWSTATUS) ; Set original status to new status
 ;"DA - IEN of TIU Note
 ;"NEWSTATUS - Name of new status
 NEW DIE,DR,DIC
 SET TMGRESULT="1^SUCCESS"
 NEW CURSTAT SET CURSTAT=$P($G(^TIU(8925,DA,0)),"^",5)
 IF CURSTAT=7 DO  QUIT
 . SET TMGRESULT="-1^CANNOT CHANGE THE STATUS OF CURRENT NOTE. IT IS COMPLETED."
 SET DIE=8925,DR=".05///"_NEWSTATUS D ^DIE
 IF NEWSTATUS="UNSIGNED" DO
 . D SEND^TIUALRT(DA),SENDID^TIUALRT1(DA):+$G(^TIU(8925,+DA,21))
 ELSE  DO
 . D ALERTDEL^TIUALRT(DA)  ;"I don't know if this is needed ->DELIRT^TIUDIRT(DA)
 Q
 ;"