...
DiffVars(tmgRefCurF,tmgRefPriorF,tmgRefDelta)
 ...
 ;"================================================================
DEVICE2ARR(DEVICE,OUT)  ;"Store off a device, with all it's parameters
   NEW TEMP ZSHOW "D":TEMP
   NEW DONE SET DONE=0
   NEW IDX SET IDX=0
   FOR  SET IDX=$ORDER(TEMP("D",IDX)) QUIT:(IDX'>0)!DONE  DO
   . NEW ENTRY SET ENTRY=$GET(TEMP("D",IDX)) QUIT:ENTRY="" 
   . NEW ARR DO SPLIT2AR^TMGSTUT2(ENTRY," ",.ARR)
   . NEW JDX SET JDX=0 FOR  SET JDX=$ORDER(ARR(JDX)) QUIT:JDX'>0  IF $GET(ARR(JDX))="" KILL ARR(JDX)
   . KILL ARR("MAXNODE")
   . NEW ADEVICE SET ADEVICE=$GET(ARR(1))
   . IF ADEVICE'=DEVICE QUIT
   . SET DONE=1
   . SET OUT("NAME")=ADEVICE KILL ARR(1)
   . SET OUT("STATUS")=$GET(ARR(2)) KILL ARR(2)
   . SET OUT("TYPE")=$GET(ARR(3)) KILL ARR(3)
   . MERGE OUT("PARAMS")=ARR
   . ;"ZWR OUT
   QUIT
   ;
USESAVEDEV(ARR) ;
