...
GET1FUDT(TMGRESULT,TMGDFN)  ;"Return info about followup
  ...
  SET TMGRESULT="No followup info found in last note (GET1FUDT^TMGTIUO9)"
  NEW TEMP SET TEMP=$GET(INFO(0)) KILL INFO(0)
  NEW MONTHS SET MONTHS=$PIECE(TEMP,"^",2)
  NEW MSG SET MSG=$PIECE(TEMP,"^",3)
  ...
  KILL INFO(1)
  NEW IDX SET IDX=""
  ;"FOR  SET IDX=$ORDER(INFO(IDX),-1) QUIT:(+IDX'>0)!(DONE=1)  DO
  ;". ;"SET IDX=$ORDER(INFO(IDX),-1)
  ;". SET STR=$GET(INFO(IDX))
  ;". SET FUDT=$PIECE(STR,"^",2)
  ;". IF FUDT>1 SET DONE=1  ;"THIS WILL KEEP THE ABOVE VALUES. IF IT RUNS OUT OF LINES, W/O GOOD VALUE,  THE IF FUDT>1 WILL CATCH
  SET IDX=$ORDER(INFO(IDX),-1)
