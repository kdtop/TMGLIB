...
SETGARBLE(VALUE) ;
  ...
  ;
SHOULDGARBLE() ;
  NEW RESULT SET RESULT=($GET(^TMG("TMP","GARBLE"))=1)
  QUIT RESULT
  ;                                                
GARBLERESULTS(ARR)  ;
...
RANDOMDOB() ;"Make random DOB in FM date format
  NEW DOB 
  SET DOB="2"_$$RJ^XLFSTR($RANDOM(100),2,0)  ;"YEAR:L 1900-1999
...
GARBLESTR(STR,FLAGS) ;"Garble string to random characters, keeping length the same
  ...
  ;  
GARBLEPTNAME(REF,ADFN) ;"Ensure document array doesn't include patients name
  ...
  . . FOR  QUIT:(LINE'[MATCH(PART))  DO
  . . . NEW PARTA SET PARTA=$PIECE(LINE,MATCH(PART),1)
  . . . NEW PARTB SET PARTB=$PIECE(LINE,MATCH(PART),2,999)
  ...
  QUIT
  ;                                                                
SETUPMATCH(MATCH,REPLACE,ADFN) ;   
  NEW MATCHNUM SET MATCHNUM=0
  NEW PART
  FOR PART=1:1:$LENGTH(NAME," ") DO
  . NEW ANAMEPART SET ANAMEPART=$PIECE(NAME," ",PART)
