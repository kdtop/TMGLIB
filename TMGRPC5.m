...
MAKEAPPT(RESULT,PATIENT,APPT,CLINIC,MODE,INFO)     ;
        ...
        ;
        KILL RESULT
        NEW TMGDFN,SC,APPTDATE
        SET RESULT(0)=1 ;"Default to success.
        SET TMGDFN=$$GETDFN^TMGSDAU2(.PATIENT)
        IF TMGDFN<1 DO  GOTO ENRDONE
        ...
        ;
        SET RESULT(0)=$$EN^TMGSDAM2(TMGDFN,APPTDATE,SC,MODE,.INFO)
        ;
ENRDONE ;
        QUIT
        ;
...
CANCAPPT(RESULT,PATIENT,APPT,CLINIC,INFO)     ;
        ...
        . SET RESULT(0)="-1^Invalid appt Date: "_$GET(APPT)
        ;
        SET RESULT(0)=$$CANCAPPT^TMGSDAC(TMGDFN,APPTDATE,SC,.INFO)
        ;
CANCDONE ;
        QUIT