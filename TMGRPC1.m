...
BLANKTIU(RESULT,TMGDFN,PERSON,LOC,DOS,TITLE) ;
        ...
        ;
        ;"  7/24/25 ADDITION.
        ;"     DR. DEE WANTS TO RECEIVE A MESSAGE WHEN A ROS IS SCANNED IN FOR HER ON TUESDAYS
        NEW X DO NOW^%DTC
        DO DW^%DTC
        ;"SET ^EDDIE("ROSMESSAGE","TEST")=TITLE_"^"_$G(DOCUMENT("PROVIDER IEN"))_"^"_X
        IF (TITLE="`1424")&($G(DOCUMENT("PROVIDER IEN"))=83)&(X="TUESDAY") DO
        . ;"SET ^EDDIE("ROSMESSAGE","SUCCESS")="LADIES AND GENTLEMEN, WE GOT IT"
        . NEW TEMPDFN SET TEMPDFN=TMGDFN
        . IF TEMPDFN["`" SET TEMPDFN=$P(TEMPDFN,"`",2)
        . NEW MSG SET MSG(1)="ROS has been scanned for:"
        . SET MSG(2)="    "_$P($G(^DPT(TEMPDFN,0)),"^",1)
        . NEW TMGRESULT DO SEND1MSG^TMGMESSG(.TMGRESULT,"DrDee","AutoGenerated",.MSG)
        . DO SEND1MSG^TMGMESSG(.TMGRESULT,"Eddie","AutoGenerated",.MSG)
        . DO SMSRPC^TMGKERN5(.TMGRESULT,"14233295446","ROS has been scanned for: "_$P($G(^DPT(TEMPDFN,0)),"^",1))
        QUIT
        ;
        ;
AUTOSIGN(RESULT,DOCIEN) ;
