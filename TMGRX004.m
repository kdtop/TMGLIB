...
GETDICT(ARR,IEN22733)  ;"GET A LISTING OF ALL WORDS ASSOCIATED WITH RX
  ...
  ;". ;"DELETE PARTS
  ;". NEW ADOSE SET ADOSE=""
  ;". FOR  SET ADOSE=$ORDER(TEMPARR("ZZIDX","STRENGTH",ADOSE)) QUIT:ADOSE=""  DO 
  ;". . KILL TEMPARR(ADOSE)
...
MATCHDSE(OUT,WORD)  ;"TRY TO MATCH DOSES TO DATABASE.  
  ...
  ;"            OUT("STRENGTH")=<DOSE>  <-- dose as found on input line. REQUIRED
  ;"          Output below: 
  ;"            OUT("STRENGTH","IENS",<IENS IN 22733.03>)=<FORM>^<Strength>^<Preferred Alias>  <-- may be multiple. 
  ;"            OUT("STRENGTH","DATABASE")=<registered dose strength for last IENS found> <-- if found.
  ...
  NEW DOSE SET DOSE=$GET(OUT("STRENGTH")) IF DOSE="" QUIT
  ...
  . . NEW ADOSE SET ADOSE=$PIECE($GET(^TMG(22733,IEN22733,2,FORMIEN,1,STRENGTHIEN,0)),"^",1)
  ...
  . . . . SET OUT("STRENGTH","IENS",IENS)=FORM_"^"_$$GET1^DIQ(22733.03,IENS,.01)_"^"_$$GETPSRA(IEN22733,FORMIEN,STRENGTHIEN)  
  . . NEW AIEN SET AIEN=0
  . . FOR  SET AIEN=$ORDER(^TMG(22733,IEN22733,2,FORMIEN,1,STRENGTHIEN,1,AIEN)) QUIT:AIEN'>0  DO  ;"DOSE ALIASES
  . . . SET ADOSE=$PIECE($GET(^TMG(22733,IEN22733,2,FORMIEN,1,STRENGTHIEN,1,AIEN,0)),"^",1)
  . . . IF (ADOSE=DOSE)!(ADOSE=STRIPDOSE) DO  QUIT
  . . . . SET IENS=STRENGTHIEN_","_FORMIEN_","_IEN22733_","
  . . . . SET OUT("STRENGTH","IENS",IENS)=FORM_"^"_$$GET1^DIQ(22733.03,IENS,.01)_"^"_$$GETPSRA(IEN22733,FORMIEN,STRENGTHIEN)  
  IF IENS'="" DO
