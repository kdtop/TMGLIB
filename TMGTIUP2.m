...
LASTHPI(TMGDFN,OPTION)  ;"Return the last HPI section, with processing, formatting etc.
  ...
  ;"   LASTHPI() <-- LASTHPI^TMGTIUOJ<-- TIU TEXT OBJECT 'TMG LAST HPI'
  ;"   LASTHPI() <-- TESTLHPI^TMGTIUP2
  ;
...
GETHPI(IEN8925,ITEMARRAY,OUT,OPTION) ;"Get HPI section as one long string, with processing, formatting etc.
  ;"Called from:  (as of 6/4/25)
  ;"  GETHPI() <-- LASTHPI^TMGTIUP2 <-- LASTHPI^TMGTIUOJ<-- TIU TEXT OBJECT 'TMG LAST HPI'
  ;"  GETHPI() <-- LASTHPI^TMGTIUP2 <-- TESTLHPI^TMGTIUP2 (testing code)  
  ;"  GETHPI() <-- TESTPARSE^TMGTIUP1 (testing code)
  ;"  GETHPI() <-- T2^TMGTIIUP2 (testing code)
...
PARSETIU(IEN8925,ITEMARRAY,OPTION) ;"parse HPI section of TIU NOTE with processing, formatting etc.
  ;"Called from:  (as of 6/4/25)
  ;"   PARSETIU() <-- T3^TMGTIUP2 (testing code)
  ;"   PARSETIU() <-- GETHPI^TMGTIUP2 <-- LASTHPI^TMGTIUP2 <-- LASTHPI^TMGTIUOJ<-- TIU TEXT OBJECT 'TMG LAST HPI'
  ;"   PARSETIU() <-- SUMNOTE^TMGTIUP1 <-- DODYNITM^TMGTIUT1 <-- RPC: 'TIU TEMPLATE GETITEMS'
  ;"   PARSETIU() <-- SUMNOTE^TMGTIUP1 <-- GETFULL^TMGTIUT3 <-- TOPICS^TMGTIUT3 <-- TOPICS^TMGRPT2 <-- (Entry point, as called from CPRS REPORT system)
  ;"   PARSETIU() <-- SUMNOTE^TMGTIUP1 <-- GETFULL^TMGTIUT3 <-- TOPICS^TMGTIUT3 <-- GETOPIC1^TMGTIUOB <-- GETTOPIC^TMGTIUOB <-- PROBSUM^TMGTIUOB <-- TIU TEXT OBJECT entry point for TMG NOTE PROB SUMMARY
  ;"   PARSETIU() <-- SUMNOTE^TMGTIUP1 <-- GETFULL4IEN^TMGTIUT3 <-- GETFULRPC^TMGTIUT3() <-- RPC: 'TMG CPRS GET ONE PATIENT TOPIC'
  ;"   PARSETIU() <-- SUMNOTE^TMGTIUP1 <-- HNDLFRGND^TMGTIUT5 <-- MAINTRIG^TMGTIUT5 <-- (POST-SIGNATURE TRIGGER)
  ;  
  ;"INPUT:  IEN8925 -- TIIU DOCUMENT IEN
  ;"        ITEMARRAY -- PASS BY REFERNCE.  An OUT PARAMETER.  See PARSEARR() for format
  ;"        OPTION -- PASS BY REFERENCE.  OPTIONAL
...
PARSEARR(TIUARRAY,ITEMARRAY,OPTION,RTNNOTE)  ;"Parse note array into formatted array 
  ;"Called from:  (as of 6/4/25)
  ;"   PARSEARR() <-- PARSETIU^TMGTIUP2 <-- T3^TMGTIUP2 (testing code)
  ;"   PARSEARR() <-- PARSETIU^TMGTIUP2 <-- GETHPI^TMGTIUP2 <-- LASTHPI^TMGTIUP2 <-- LASTHPI^TMGTIUOJ<-- TIU TEXT OBJECT 'TMG LAST HPI'
  ;"   PARSEARR() <-- PARSETIU^TMGTIUP2 <-- SUMNOTE^TMGTIUP1 <-- DODYNITM^TMGTIUT1 <-- RPC: 'TIU TEMPLATE GETITEMS'
  ;"   PARSEARR() <-- PARSETIU^TMGTIUP2 <-- SUMNOTE^TMGTIUP1 <-- GETFULL^TMGTIUT3 <-- TOPICS^TMGTIUT3 <-- TOPICS^TMGRPT2 <-- (Entry point, as called from CPRS REPORT system)
  ;"   PARSEARR() <-- PARSETIU^TMGTIUP2 <-- SUMNOTE^TMGTIUP1 <-- GETFULL^TMGTIUT3 <-- TOPICS^TMGTIUT3 <-- GETOPIC1^TMGTIUOB <-- GETTOPIC^TMGTIUOB <-- PROBSUM^TMGTIUOB <-- TIU TEXT OBJECT entry point for TMG NOTE PROB SUMMARY
  ;"   PARSEARR() <-- PARSETIU^TMGTIUP2 <-- SUMNOTE^TMGTIUP1 <-- GETFULL4IEN^TMGTIUT3 <-- GETFULRPC^TMGTIUT3() <-- RPC: 'TMG CPRS GET ONE PATIENT TOPIC'
  ;"   PARSEARR() <-- PARSETIU^TMGTIUP2 <-- SUMNOTE^TMGTIUP1 <-- HNDLFRGND^TMGTIUT5 <-- MAINTRIG^TMGTIUT5 <-- (POST-SIGNATURE TRIGGER)
  ;"   PARSEARR() <-- SORTBY^TMGRPC1I <-- 
  ;
  ...
  . IF TITLE["ALLERGIES" QUIT   ;"Was UPTITLE, but that included regular sections
...
COMPHPI(ITEMARRAY,OPTION,OUT)  ;"COMPILE HPI    
  ;"Called from:  (as of 6/4/25)
  ;"  COMPHPI() <-- GETHPI() <-- LASTHPI^TMGTIUP2 <-- LASTHPI^TMGTIUOJ<-- TIU TEXT OBJECT 'TMG LAST HPI'
  ;"  COMPHPI() <-- SORTBY^TMGRPC1I <--
  ;
...
SPLITTL(SECTION,TOPIC,TEXTARR,TABLES,OPTION) ;"Split TOPIC and main text of section, and parse section into parts
  ;"Called from:  (as of 6/4/25)  
  ;"  SPLITTL <-- PARSEARR^TMGTIUP2 <-- (multiple entry points)
  ;"  SPLITTL <-- ALLGRPS^TMGTIUP2 <-- PARSEARR^TMGTIUP2 <-- (multiple entry points)
  ;"  SPLITTL <-- TOCPARSE^TMGRPC1H <--  
  ;  
