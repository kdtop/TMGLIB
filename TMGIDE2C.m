...
SHOWVAR(zzREF,zzMODE)  ;"Show variable, using SCROLLER
  ...
  NEW tmgARRAY,IDX,JDX,LONGESTLINE SET IDX=0,JDX=1,LONGESTLINE=1
  FOR  SET IDX=$ORDER(zzTEMP(IDX)) QUIT:IDX'>0  DO
  . NEW ALINE,ALEN SET ALINE=$GET(zzTEMP(IDX)),ALEN=$LENGTH(ALINE)
  . IF ALEN>LONGESTLINE SET LONGESTLINE=ALEN
  . SET tmgARRAY(JDX,ALINE)="",JDX=JDX+1
  KILL zzTEMP
  NEW tmgOPTION
  SET tmgOPTION("HEADER",1)=" - Display of ["_zzREF_"] - "
  SET tmgOPTION("FOOTER",1)="Enter ^ to exit"
  SET tmgOPTION("ON SELECT")="HNDONSEL^TMGIDE2C"
  SET tmgOPTION("LR SCROLLING")=1
  ;"Does data have long lines?  If so, should display be wrapped?
  NEW SCRNW IF $$GETSCRSZ^TMGKERNL(,.SCRNW) SET SCRNW=+SCRNW-2
  NEW % SET %=2
  IF LONGESTLINE>SCRNW DO  GOTO:%=-1 SVRDN
  . WRITE !,"Wrap long lines" DO YN^DICN WRITE !
  . IF %=1 SET tmgOPTION("WRAP DATA")=1   
