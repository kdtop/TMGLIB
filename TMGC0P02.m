TMGC0P02   ; TMG - ELAB / Web Service main entry points;  1/21/17
...
WRAP(ZRTN,ZINARY,MODE,SUBTYPE)  ;WRAPS AN XML ARRAY (ZINARY) IN HTML FOR PROCESSING
  ...
  Q
  ;
GETINS(REF,DFN)  ;GET INSURANCE AND INSURED INFORMATION
  ;Input: REF -- PASS BY NAME.  AN OUTPUT PARAMETER.
  NEW RANK,TEMP
  NEW SUBIEN SET SUBIEN=0
  ;"Index insurance records by rank order
  FOR  SET SUBIEN=$ORDER(^DPT(DFN,.312,SUBIEN)) QUIT:SUBIEN'>0  DO  
  . NEW N0 SET N0=$GET(^DPT(DFN,.312,SUBIEN,0))
  ...
  . NEW TMG,TMGERR,TMGIENS SET TMGIENS=SUBIEN_","_DFN_","
