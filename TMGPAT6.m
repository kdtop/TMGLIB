TMGPAT6  ;TMG/kst/Patching tools ;10/19/08, 7/14/22
 ...
 ;"CHARACTERIZE(INFO,ISZIP,ISDIR,ISFILE,INZIPDIR,ISTEXT,ISKIDS) --Characterize file
...
FIXMISSINGSEQ() ;"Fix KIDS or TXT entries with missing SEQ
  ...
  . . . . . NEW TMGFDA,TMGMSG 
  . . . . . SET TMGFDA(22709.1211,IENS,.04)=SEQNUM
  . . . . . ZWR TMGFDA
...
FIXMISSINGINFOTXT()  ;
  ...
  . . . NEW TEXTIENS SET TEXTIENS=$GET(DATA("INFOTXT"))  ;"THIS IS IENS FOR 22709.121
  ...
  . . SET TMGFDA(22709.1211,SUBIENS,.04)=SEQ    ;"SEQUENCE NUM
...
LOADFILE(INFO,REF) ;"Download from web server to local HFS file AND copy head info into @REF
  ...
  NEW ISTEXT,ISKIDS DO CHARACTERIZE(.INFO,,,,,.ISTEXT,.ISKIDS)
  IF ISTEXT DO  GOTO LFDN
  . SET RESULT=$$HEADFILE(LOCALFILE,REF,40)
  IF ISKIDS DO  GOTO LFDN
  . NEW FILTER,ARR
  . SET FILTER("[","**KIDS**")=""
  . SET FILTER("[","""REQB"",")=""   
  . NEW TEMP SET TEMP=$$GLBOUTGREP^TMGKERNL(LOCALFILE,.FILTER,.ARR)
  . MERGE @REF=ARR
  . SET RESULT="1^OK"
  ELSE  DO  GOTO LFDN
...
HANDLEFILE(INFO) ;"Take file entry and manage it: search, process, store
  ...
  NEW ISZIP,ISDIR,ISFILE,INZIPDIR,ISTEXT,ISKIDS,RESULT,IENS,FOUND
  NEW SHOULDDELETE SET SHOULDDELETE=0
  DO CHARACTERIZE(.INFO,.ISZIP,.ISDIR,.ISFILE,.INZIPDIR,.ISTEXT,.ISKIDS) 
  ...
  IF FOUND,((RELOADTXT&ISTEXT)!(RELOADKIDS&ISKIDS)) DO  
...
PROCESSFILE(INFO) ;"Take file entry and parse, depending on if KIDS etc. 
  ...
  NEW RESULT SET RESULT="1^OK"
  NEW ISZIP,ISDIR,ISFILE,INZIPDIR,ISTEXT,ISKIDS
  DO CHARACTERIZE(.INFO,.ISZIP,.ISDIR,.ISFILE,.INZIPDIR,.ISTEXT,.ISKIDS) 
  IF (ISTEXT=0)&(ISKIDS=0)!INZIPDIR GOTO PFDN
  NEW TEMP,TMGTEMP,REF SET REF="TMGTEMP"
  SET TEMP=$$LOADFILE(.INFO,REF) ;"DOWNLOAD FILE into @REF  
...
PFDN ;  
  QUIT RESULT
  ;
PARSETXT(INFO,ARR)  ;"Open info TEXT FILE (that is typically distributed with KIDS patches) and extract metadata
  ;"Input: See INFO details above
...
GI2 ;   
  ...
  . IF SHOWPROG DO PROGBAR^TMGUSRI2(IDX,PCK,1,MAX,60,STARTH)
  . DO GETPINFO(.OUT,PCK)
  . SET IDX=IDX+1
...
GETPVPINFO(OUT,PCKINIT,VER,PATCHNUM)  ;Get info on patch, by Package,Ver,Patch# (PVP)
  ...
  ;"        }~"VERSTR" = 3.0
  ;"          
  IF ($GET(PCKINIT)="")!(VER'>0)!(PATCHNUM'>0) GOTO GIDN
  NEW VERSTR SET VERSTR=VER IF $PIECE(VERSTR,".",2)="" SET $PIECE(VERSTR,".",2)="0"
  ...
  NEW PENDINGDEPENDENCIES SET PENDINGDEPENDENCIES=+$GET(OUT("KIDS","PENDING DEPENDENCIES"))
