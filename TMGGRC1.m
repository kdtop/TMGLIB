...
WTLENPCT(AGEYR,GENDER,WT,LEN,NONULL)	       ;
	       ...
	       QUIT RESULT
	       ;
WHHAPCTL(AGEYR,GENDER,LEN,NONULL,ZSCORE)	         ;
	       ...
	       NEW RESULT
	       NEW MONTHS SET MONTHS=+$GET(AGEYR)*12
	       SET RESULT=$$COMMON("WHO-HEIGHT BY AGE",.LEN,.MONTHS,.GENDER,.NONULL,.ZSCORE) ;
	       QUIT RESULT
	       ;
WHHCPCTL(AGEYR,GENDER,HC,NONULL,ZSCORE)	         ;
	       ...
	       NEW MONTHS SET MONTHS=+$GET(AGEYR)*12
	       NEW RESULT
	       SET RESULT=$$COMMON("WHO-HEAD CIRC BY AGE",.HC,.MONTHS,.GENDER,.NONULL,.ZSCORE) ;
	       QUIT RESULT
	       ;
WHWAPCTL(AGEYR,GENDER,WT,NONULL,ZSCORE)	         ;
	       ...
	       NEW RESULT
	       NEW MONTHS SET MONTHS=+$GET(AGEYR)*12
	       SET RESULT=$$COMMON("WHO-WEIGHT BY AGE",.WT,.MONTHS,.GENDER,.NONULL,.ZSCORE) ;
	       QUIT RESULT
	       ;
WHBAPCTL(AGEYR,GENDER,BMI,NONULL,ZSCORE)	 ;
	       ;"Purpose: Return percentile of WHO BMI FOR age
	       ;"Input: AGEYR: age in *Years*
	       ;"       GENDER: should be M or F
	       ;"       BMI: weight in Kg/M**2
	              ;"       ZSCORE: OPTIONAL, Default=0.   If 1 then return z-score rather than PCTL
	       ;"       NONNULL: OPTIONAL, default=0.  If 1 then 'N/A' is returned instead of ''
	       NEW MONTHS SET MONTHS=+$GET(AGEYR)*12
	       NEW RESULT
	       SET RESULT=$$COMMON("WHO-BMI BY AGE",.BMI,.MONTHS,.GENDER,.NONULL,.ZSCORE) ;
	       QUIT RESULT
	       ;
WHWLPCTL(AGEYR,GENDER,WT,LEN,NONULL,ZSCORE)	        ;
	       ...
	       ;"Results: e.g. 54th %tile, or "" IF invalid.  If NONNULL=1, then return 'N/A'
	       NEW RESULT
	       SET RESULT=$$COMMON("WHO-WEIGHT FOR LENGTH",.WT,.LEN,.GENDER,.NONULL,.ZSCORE) ;
	       QUIT RESULT
	       ;
WHWSPCTL(AGEYR,GENDER,WT,LEN,NONULL,ZSCORE)	        ;
	       ...
	       ;"Results: e.g. 54th %tile, or "" IF invalid.  If NONNULL=1, then return 'N/A'
	       NEW RESULT
	       SET RESULT=$$COMMON("WHO-WEIGHT FOR STATURE",.WT,.LEN,.GENDER,.NONULL,.ZSCORE) ;
	       QUIT RESULT
	       ;
COMMON(GRAPH,YVAL,XVAL,GENDER,NONULL,ZSCORE)	       ;
	       ...
	       IF ZSCORE=1 SET RESULT=$JUSTIFY(Z,0,2) GOTO COMQT
	              NEW W SET W=$$Z2PCTL^TMGGRCU(Z)\1
	       IF W=-1 SET RESULT="Invalid value" GOTO COMQT
	       IF W<1 SET RESULT="< 1st %tile" GOTO COMQT  ;"11-6-12
	              IF W>99 SET RESULT="> 99th %tile" GOTO COMQT  ;"11-6-12
...
COMMNREF(MODE,GENDER,GRAPH,ARRAY,RLINES)	       ;
	       ...
	       ;"        7 age range 6-24 months, 8 age range 60-120 months, 9 age range 60-228 months,
	       ;"        10 age range 0-13 months
	       ;"       If piece 2 of mode is "Z", then z-scores will be returned
	       ;"       GENDER -- M OR F
	       ;"       GRAPH -- Name of graph
	       ;"       ARRAY -- PASS BY REFERENCE.  AN OUT PARAMETER.  PRIOR VALUES KILLED.
	       ;"           ARRAY(%tile,x)=x^y
	       ...
	       ;"Result: none
	       NEW ZMODE SET ZMODE=0
	       IF $PIECE(MODE,"^",2)="Z" DO
	       . SET ZMODE=1
	       ...
	       . SET X0=45,XINC=5,XMAX=110
	       ELSE  IF (GRAPH="WHO-WEIGHT FOR STATURE") DO
	       ...
	       . ELSE  IF MODE=8 SET X0=60,XINC=1,XMAX=120
	       . ELSE  IF MODE=9 SET X0=60,XINC=1,XMAX=228
	       ...
	       . IF ZMODE=1 DO
	       . . NEW MYZ
	       . . SET RLINES=$GET(RLINES,"-3,-2,-1,0,1,2,3")
	       . . FOR MYZ=1:1:$LENGTH(RLINES,",")  DO
	       . . . SET Z=$P(RLINES,",",MYZ) ;"QUIT:(Z'>0)
	       . . . NEW VAL SET VAL=$$LMSZ2Y^TMGGRCU(L,M,S,Z)
	       . . . SET ARRAY(Z,X)=X_"^"_VAL
	       . ELSE  DO
	       . . NEW P,PCTLNUM
