...
SCRIPT	;
	      ...
	      ;;      ALine.VMLstroke="weight='2pt'; color='GETLCOLOR'; dashstyle='solid';"; // EHS
	      ;;      ALine.pointfillcolor="GETLCOLOR"; // EHS
	      ;;    }
	      ;;    else {
...
SETGRAPH(ROOT,TITLE,XMIN,XMAX,XTITLE,YMIN,YMAX,YTITLE,XINC,YINC,SOURCE,ACCESSDT,GRAPHTYP)	       ;
	      ...
	      IF (GRAPHTYP="WHO-HA-B5")!(GRAPHTYP="ZWO-HA-B5")  DO SETITEM^TMGGRC2A(.ROOT,"  WHOLine();")
...
ADDWHOL2(ROOT)	 ;
	      ...
	      QUIT
	          ;
ADDDESC(ROOT,GRAPHTYP)	  ;
	             ;"Purpose: Add table description
	             ;"Input: ROOT -- The root to output to .
	      ...
	      DO SETITEM^TMGGRC2A(.ROOT,"   c=c+""   <td>"_TEXT_"</td>"";")
	      DO SETITEM^TMGGRC2A(.ROOT,"   c=c+""</tr></table>."";")
	      DO SETITEM^TMGGRC2A(.ROOT,"   MyGraph=c;")
ADDN	      QUIT
	           ;
ADDTABLE(ROOT,ARRAY,GRAPHTYP)	 ;
	      ...
	      KILL ^TMG("EDDIE","ADDTABLEARRAY")
	      MERGE ^TMG("EDDIE","ADDTABLEARRAY")=ARRAY
	      ...
	      . SET TTLCOL4="%tile"
	      . SET TTLCOL5=""
	      ELSE  IF (GRAPHTYP'["WT4")&(GRAPHTYP'["WHO-WL")&(GRAPHTYP'["WHO-WH")&(GRAPHTYP'["ZWO-WL")&(GRAPHTYP'["ZWO-WH") DO
	      . SET TTLCOL2="Age (mos)"
	      . SET ROUND=0
	      . IF GRAPHTYP["ZWO" SET TTLCOL4="Z-Score"
	             . ELSE  SET TTLCOL4="%tile"
	      . SET TTLCOL5=""
	      ELSE  DO  ;" if WT4 , process X Y and AGE here
	      . SET TTLCOL2="Length (cm)"
	      . SET TTLCOL3="Weight (kg)"
	      . SET ROUND=0
	      . IF GRAPHTYP["ZWO" SET TTLCOL5="Z-Score"
	             . ELSE  SET TTLCOL5="%tile"
	      . SET TTLCOL4="Age (mos)"
	      ;
	      ;" Determine what the Y value is
	      IF (GRAPHTYP["HT")!(GRAPHTYP["LN")!(GRAPHTYP["WHO-HA")!(GRAPHTYP["ZWO-HA") DO
	      ...
	      . SET TTLCOL3="BMI"
	      ELSE  IF ((GRAPHTYP["WT")&(GRAPHTYP'["WT4"))!(GRAPHTYP["WHO-W")!(GRAPHTYP["ZWO-W") DO
	      . SET TTLCOL3="Weight (kg)"
	      ;
	      DO SETITEM^TMGGRC2A(.ROOT,"}")
	      DO SETITEM^TMGGRC2A(.ROOT,"    ")
