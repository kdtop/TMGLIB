TMGSTUT3 ;TMG/kst/SACC Compliant String Util Lib ;9/20/17, 11/24/24
	...
	;"$$EXTRACTMATCHED(STR,POS,ENCAPS,ENDPOS,ESCQT)  --Extract substring, bound by encapsulators, starting at POS
	;"$$STRPOSMATCH(STR,SUBSTR,POS)  --Does STR match SUBSTR, starting at POS?
	;"MAPMATCH3(STR,MAPREF,ENCAPS,ESCQT) --MAP MATCHING ENCAPSULATORS, allowing multi-char, arbitarily-paired encapsulators, honoring nesting and ignoring encapsulators in strings.;
	...
	;"$$NEXTFRAG(STR,STARTPOS,FRAGS,FOUNDPOS,ESCQT) --Get first next character (or string fragment), matching from array of possible inputs.;
...
EXTRACTMATCHED(STR,POS,ENCAPS,ENDPOS,ESCQT)  ;"Extract substring, bound by encapsulators, starting at POS
	...
	;"       ESCQT -- OPTIONAL.  string to indicate pattern used to escape quotes inside string.  E.g. mumps uses <"">, JSON uses <\"> etc
	...
	SET ENDPOS=0
	IF $DATA(ENCAPS)=0 DO GETMATCHENCAP(.ENCAPS,"({|<""#'[")
	NEW WIDE SET WIDE=$$WIDECAPS(.ENCAPS) ;"Returns 1 if any of the encapsulators is 'wide', i.e. multi-char (e.g. '{{')
	NEW CH SET CH=$EXTRACT(STR,POS)
	NEW OPEN,CLOSE
	NEW TEST SET TEST=$$CHISENCAP(CH,.ENCAPS,.OPEN,.CLOSE)  ;"Does CH map (or partly match) an encapsulator. OPEN is matched opening encapsulator
	...
	;"NEW INDQT SET INDQT=(CH=QT2)  ;"INDQT means IN DOUBLE QUOTE, i.e. we are scanning inside a "..." string
	NEW STRHASESCQT SET STRHASESCQT=((ESCQT'="")&(STR[ESCQT))
	NEW ESCQTPOS SET ESCQTPOS=-1  ;"This will be index of START of next expected escaped quote sequence, eg <\">
	IF STRHASESCQT SET ESCQTPOS=$FIND(STR,ESCQT) IF ESCQTPOS>0 SET ESCQTPOS=ESCQTPOS-$LENGTH(ESCQT)
	NEW OPENISQT SET OPENISQT=(QTS[OPEN)  ;"Is the encapsulator itself a quote char?
	NEW INDQT SET INDQT=(OPEN=QT2)  ;"INDQT means the entire search string is a "..." string. If something follows 2nd quote, will be handled later.;
	...
	;"Example: ["This is a \"test\" or two"]  with ESCQT=<\">
	FOR POS=POS:1:$LENGTH(STR) QUIT:DONE  DO
	. NEW CH SET CH=$EXTRACT(STR,POS)
	. IF STRHASESCQT,POS=ESCQTPOS DO
	. . SET POS=POS+$LENGTH(ESCQT)  ;"move position to PAST escaped quote
	. . SET ESCQTPOS=$FIND(STR,ESCQT,POS) IF ESCQTPOS>0 SET ESCQTPOS=ESCQTPOS-$LENGTH(ESCQT)  ;"get ready for next escaped quote (if any)
	. . IF (INQT!INDQT) DO
	. . . SET CH=$EXTRACT(STR,POS)
	. . ELSE  DO  ;"We have encountered an escaped quote, but are not currently inside a quoted string, so convert to unescaped quote
	. . . SET CH=QT2
...
STRPOSMATCH(STR,SUBSTR,POS)  ;"Does STR match SUBSTR, starting at POS?
	...
	QUIT RESULT
	;
MAPMATCH3(STR,MAPREF,ENCAPS,ESCQT)  ;" MAP MATCHING ENCAPSULATORS, allowing multi-char, arbitarily-paired encapsulators,
	...
	. IF OPENTAG=QT2,ESCQT'="" DO  ;"Quote was found at position OPENPOS, but was it an escaped quote that should be ignored?
	. . NEW OK SET OK=1
	. . NEW POSOFFSET SET POSOFFSET=0
	. . FOR  DO  QUIT:OK
	. . . NEW P2 SET P2=$FIND(STR,ESCQT,POS) QUIT:P2=0   ;"P2 is position immediately AFTER ESCQT, or 0 if not found
	. . . IF OPENPOS-$LENGTH(ESCQT)'=P2 QUIT
	. . . SET POSOFFSET=OPENPOS-POS+1
	. . . SET OPENTAG=$$NEXTFRAG(STR,POS+POSOFFSET,.ENCAPS,.OPENPOS)
	. . . SET OK=0  ;"Force recheck for escaped quote
	. IF OPENTAG'="" DO
	. . NEW PARTA SET PARTA=$EXTRACT(STR,POS,OPENPOS-1)
	. . NEW PARTB,ENDPOS SET PARTB=$$EXTRACTMATCHED(STR,OPENPOS,.ENCAPS,.ENDPOS,.ESCQT)
	. . IF PARTB="" SET DONE=1 QUIT  ;"error state
	. . NEW CLOSETAG SET CLOSETAG=$GET(ENCAPS(OPENTAG))
	. . IF CLOSETAG="" SET DONE=1 QUIT  ;"error state
	. . NEW UNESCAPE SET UNESCAPE=((OPENTAG=QT2)&(CLOSETAG=QT2)&(ESCQT'=""))
	. . SET PARTB=$EXTRACT(PARTB,$LENGTH(OPENTAG)+1,$LENGTH(PARTB)-$LENGTH(CLOSETAG))
	. . IF UNESCAPE DO
	. . . NEW SPEC SET SPEC(ESCQT)=QT2 SET PARTB=$$REPLACE^XLFSTR(PARTB,.SPEC)
	...
	. . . IF OPENTAG="'" KILL SUBENCAPS("""")
	. . . DO MAPMATCH3(PARTB,SUBREF,.SUBENCAPS,.ESCQT)
	. . . SET PARTB=""
...
NEXTFRAG(STR,STARTPOS,FRAGS,FOUNDPOS,ESCQT) ;"Get first next character (or string fragment), matching from array of possible inputs.;
	...
	;"        ESCQT -- OPTIONAL.  string to indicate pattern used to escape quotes inside string.  E.g. mumps uses <"">, JSON uses <\"> etc
	;"										NOTE: if FRAGS includes quotes ("), and ESCQT includes matching escaped quotes, e.g. \" then match will NOT be made on escaped quote character
	...
	. NEW P1 SET P1=$FIND(STR,TEST,STARTPOS)
	. NEW P2 SET P2=P1-$LENGTH(TEST)
	. NEW SKIP SET SKIP=0
	. IF TEST=QT2,STRHASESCQT,P2=ESCQTPOS DO  QUIT:SKIP    ;"Escaped quote found
	. . ;"Now check if any subsequent unescaped quotes follow?
	. . NEW OK SET OK=1,SKIP=1
	. . FOR  DO  QUIT:OK
	. . . SET ESCQTPOS=$FIND(STR,ESCQT,P1) IF ESCQTPOS>0 SET ESCQTPOS=ESCQTPOS-$LENGTH(ESCQT)  ;"Get next position of escaped quote (if any)
	. . . SET P1=$FIND(STR,TEST,P1)
	. . . SET P2=P1-$LENGTH(TEST)
	. . . IF P2=ESCQTPOS SET OK=0 QUIT
	. . . SET SKIP=0  ;"We have found an unescaped quote
	. SET POS(IDX)=P2
