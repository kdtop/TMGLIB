...
GETARR(OUT,TMGDFN,DT)  ;"USED BY FIXDISP ABOVE
  ...
  QUIT
  ;
SET1DAY(OUT,TMGDFN,DAY,DUZ)  ;"SET ALL LABS FOR ONE USER/PT COMBO FOR ENTIRE DAY (IF NOT ALREADY SET)
  ...
  . SET DAY=$GET(^TMP("TMGLRU01","DAY"))
  . SET DUZ=$GET(^TMP("TMGLRU01","DUZ"))
  ELSE  DO
  . SET ^TMP("TMGLRU01","DFN")=TMGDFN
  . SET ^TMP("TMGLRU01","DAY")=DAY
  . SET ^TMP("TMGLRU01","DUZ")=DUZ
  SET OUT="1^SUCCESSFUL"
  NEW LABDT SET LABDT=0
  NEW LRDFN SET LRDFN=+$GET(^DPT(TMGDFN,"LR"))
  ;"NEW FOUND,RDT SET RDT=9999999-DAY
  FOR  SET LABDT=$ORDER(^LR(LRDFN,"CH",LABDT)) QUIT:(LABDT'>0)  DO
  . NEW THISDT SET THISDT=9999999-LABDT
  . IF $P(THISDT,".",1)'=DAY QUIT
  . DO WASSEEN(.OUT,TMGDFN,THISDT,DUZ)
  QUIT
  ;"  
