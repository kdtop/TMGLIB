TMGLRU01 ;TMG/kst-Entry point LAB REPORTS ;12/16/15, 4/1/18, 3/24/21
 ...
 ;"=======================================================================
 ;"LABDATES(ORY,TMGDFN,REPORTID,HSTYPE,DAYSBACK,SECTION,SDT,EDT) -- CPRS Lab report (Lab Dates)
...
ADDLINE(REF,TEXT) ;
  ...
  QUIT
  ;
LABDATES(ORY,TMGDFN,REPORTID,HSTYPE,DAYSBACK,SECTION,SDT,EDT)   ;"  
...
GETARR(OUT,TMGDFN,DT)  ;"USED BY FIXDISP ABOVE
  KILL OUT NEW TEMP
  DO INTERIMG^ORWLRR(.TEMP,TMGDFN,DT-0.000001,-1,1)
...
WSERR
  SET OUT="-1^"_$$SETSEEN(TMGDFN,DATE,DUZ)
WSDN
  QUIT
  ;"
SETSEEN(TMGDFN,DATE,DUZ)   ;"SET LAB SET AS SEEN BY THE USER
  NEW TMGFDA,TMGIEN,TMGMSG,PTIEN,TMGRESULT
  SET TMGRESULT="ENTRY CREATED"
  SET TMGIEN=+$ORDER(^TMG(22732,"B",TMGDFN,0))
  IF TMGIEN'>0 DO
  . SET TMGFDA(22732,"+1,",.01)=TMGDFN 
