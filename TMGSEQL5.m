TMGSEQL5 ;TMG/kst/Interface with SequelSystems PMS for ICD codes ; 9/20/16
...
PROCESS1(ONELINE) ;"Process one line from the CSV file.  
  ...
  NEW DFN,DT
  SET DFN=$$FINDDFN(.ONELINE)        
  IF +DFN<1 DO  GOTO PRODN
  . SET TMGRESULT=DFN
  NEW TEMP,DISCARD
  SET TEMP=$$ENSURINS(.ONELINE,.DISCARD,DFN)  ;"Ensure insurance name is a valid record, and linked to patient.
...
FINDDFN(ONELINE)  ;"Try and find patient's DFN
  ...
  NEW DFN SET DFN=0
  SET DFN=$ORDER(^DPT("TMGS",SEQLNUM,0))
  IF DFN>0 DO  GOTO ADFNDN
...
ENIS3 ;
  ...
  KILL TMGFDA,TMGMSG
  SET TMGFDA(2.312,SUBIEN_","_DFN_",",1.03)=$$NOW^XLFDT
  DO FILE^DIE("E","TMGFDA","TMGMSG")
  IF $DATA(TMGMSG("DIERR")) DO  GOTO ENSDN
  . SET TMGRESULT="-1^"_$$GETERRST^TMGDEBU2(.TMGMSG)  
  ;
...
ENSRVIST(CSV,OUT) ;"Process date time of CPT event.
  ...
  NEW ADT SET ADT=$ORDER(^AUPNVSIT("AA",DFN,RDT-0.000001))
  NEW IEN SET IEN=0
  IF ADT\1=RDT\1 DO   
  . SET IEN=$ORDER(^AUPNVSIT("AA",DFN,ADT,0))
  . SET RDT=ADT
...
FILEDATA(DATA)  ;"ENSURE DATA FROM DATA IS FILED INTO DATABASE
  ...
  ;"Result: 1 if OK, or -1^Message if problem.
  NEW TMGRESULT SET TMGRESULT=1
  NEW DFN SET DFN=+$GET(DATA("DFN"))
  IF DFN'>0 DO  GOTO FDDN
  ...
  IF $DATA(^AUPNVCPT("AA",DFN))=0 GOTO ADD
  IF $DATA(^AUPNVCPT("AA",DFN,CPTIEN))=0 GOTO ADD
  IF $DATA(^AUPNVCPT("AA",DFN,CPTIEN,RDT\1))=0 GOTO ADD
  NEW IEN SET IEN=$ORDER(^AUPNVCPT("AA",DFN,CPTIEN,RDT\1,0))
  IF IEN>0 GOTO FD2
ADD ;  
  ...
  SET TMGFDA(9000010.18,"+1,",.02)=DFN  ;" 9000001 <--> PATIENT FILE
