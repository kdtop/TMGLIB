TMGHL71	;TMG/kst-Entry Point for HL7 processing ;11/14/16, 4/1/18
...
HL7FIN(FNAME,NOALERT,DONEPATH,OPTION)  ;"POC file input HL7 message files from lab
  ...
  NEW HLREC,HL,HLQUIT,HLNODE,HLNEXT,HLHDRO,HLMTIEN,HLMTIENS,TEMPRESULT 
  SET (HLMTIEN,HLMTIENS)=0  ;"NOTE: I THINK HLMTIEN,HLMTIENS ARE USED IN GLOBAL SCOPE DOWNSTREAM
  SET OPTION("FILEPATHNAME")=FNAME  ;"Used in HL7MSGIN()
...
FHL2 ;
  IF $GET(OPTION("NO MOVE")) GOTO FHLDN
  SET TEMPRESULT=$$MOVE(+TMGRESULT,FNAME,.DONEPATH,$GET(OPTION("HL7 DATE")))
  IF TEMPRESULT'>0 DO
...
FHL3 ;  
  ...
  SET TEMPRESULT=$$MAKEMETA(+TMGRESULT,FNAME,.DONEPATH,.OPTION,.IEN772,.IEN773)
  IF TEMPRESULT'>0 DO
  . DO SETALRT2^TMGHL7E(TEMPRESULT)
  . IF +TMGRESULT=1 SET TMGRESULT=TEMPRESULT
  . ELSE  SET TMGRESULT=TMGRESULT_" AND ALSO "_$PIECE(TEMPRESULT,"^",2,99)
...
HL7MSGIN(TMGMSG,NOALERT,OPTION)    ;
  ...
  IF TMGRESULT<0 GOTO HLI3ERR
HLI3ERR  ;
  IF (TMGRESULT<0),(NOALERT'=1) DO SETALERT(TMGRESULT)
H7IN3DN  ;
  QUIT TMGRESULT
  ;
