...
BMITREND(TMGDFN,TIU) ;
  ...
  NEW DATE SET DATE=+$GET(TIU("EDT"))
  ;"SET DATE=3230101
  IF DATE'>0 SET DATE=$$FIRSTYR^TMGDATE,DATE=DATE+10000
  ...
  IF NHT'>0 SET RESULT="BMI trend: (No height entered for patient)" GOTO BMITRDONE
  NEW DATE SET DATE=""
...
GETSPECL(TMGDFN,STARTMARKERS,ENDMARKERS,MONTHS,ARRAY,MODE,SPACES,OPTIONS) ;"GET SPECIAL 
  ...
  . . . . NEW NOTEDT SET NOTEDT=+$PIECE($GET(^TIU(8925,IEN8925,13)),"^",1)  ;"+$PIECE($GET(^TIU(8925,IEN8925,0)),"^",7)
