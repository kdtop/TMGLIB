...
PARSHTML(HTMLARR,HANDLE)  ;"PARSE HTML ARRAY WITH EWD 
 ...
 ;"INPUT: HTMLARR -- PASS BY REFERENCE.  Format:  HTMLARR(#)=<TEXT>
 ;"       HANDLE -- A user supplied handle name, to be used with later deletion
 ;"Result: "" if OK, or error message
 NEW ok,%error set %error=""
...
PARSHTM2(HTMLREF,HANDLE)  ;"PARSE HTML ARRAY WITH EWD 
 ;"NOTE: copied and modified from parseURL^%zewdHTMLParser()
 ;"INPUT: HTMLREF -- PASS BY NAME.  Format:  @HTMLREF@(#)=<TEXT>
 ;"       HANDLE -- A user supplied handle name, to be used with later deletion
 ;"Result: "" if OK, or error message
 NEW %error
 k ^CacheTempEWD($j)
...
httpResponse ;
 ...
 ;
 ;//kt mod block -----------
 NEW SRVTIMEOUT SET SRVTIMEOUT=0
 NEW SRVDONE SET SRVDONE=(rlen=0)
 FOR pos=1:1 QUIT:SRVDONE  DO  QUIT:(SRVTIMEOUT)
 . NEW MAXLEN SET MAXLEN=$SELECT(rlen>10000:10000,1:rlen)
 . READ str#MAXLEN:timeout
 . SET SRVTIMEOUT=($TEST=0)
 . NEW SRVLEN SET SRVLEN=$LENGTH(str) 
 . SET:(SRVLEN>0) str(pos)=str
 . SET rlen=rlen-SRVLEN
 . SET SRVDONE=((rlen'>0)!(SRVLEN=0))
 IF SRVTIMEOUT GOTO httpTimeout  
 ;//kt end mod block-----------------------
 ;
 i $g(test)'=1 c dev
 s rawResponse=""
