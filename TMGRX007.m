TMGRX007 ;TMG/kst/Patient medication listing code; 05/16/18
...
PICKSCAN(MODE) ;"PICK PATIENT, THEN SCAN THEM
  NEW DIC,X,Y,DFN
  SET DIC=2,DIC(0)="MAEQ" DO ^DIC WRITE ! SET DFN=+Y  
  IF DFN>0 DO
  . IF MODE=1 DO SCANPT(DFN)
  . ;"IF MODE=2 DO SCANPT2(DFN) ;"<--- depreciated
  . ;"IF MODE=3 DO SCANPT3(DFN) ;"<--- depreciated
...
SCANALL(MODE)  ;"SCAN ALL PATIENTS...
  ...
  . IF DFN#10=0 DO PROGBAR^TMGUSRI2(DFN,"DFN: "_DFN,1,MAXDFN,60,STARTH)
  . IF MODE=0 DO SCANPT(DFN)    
  . ;"IF MODE=1 DO SCANPT2(DFN)  ;"<--- depreciated
  QUIT
  ;
SCANPT(DFN) ;"SCAN 1 PATIENT  -- This method is same as used by POST-SIGNATURE code
  ;"WRITE !,"This scans all patients, and gets last note with table information.",!
  NEW TABLENAME SET TABLENAME="[FINAL MEDICATIONS]"
  NEW NOTES IF $$LASTNOTE^TMGTIUO5(DFN,.NOTES,TABLENAME)=0 QUIT ;"GET LAST NOTE POINTED TO IN 22729
  ...
  QUIT
  ;  
  ;"SCANPT2(DFN) ;"SCAN 1 PATIENT --  This method just gets patients MED ARRAY  <--- depreciated
  ...
  ;"  NEW MEDARR DO MEDARR^TMGTIUOJ(0,DFN,.MEDARR,,.OPTION)
  ;"  KILL MEDARR(0)
  ;"  IF $DATA(MEDARR)=0 QUIT
  ;"  NEW TMGRESULT SET TMGRESULT=$$SAVETABL(DFN,.MEDARR)  ;"SAVE MEDICATION TABLE.
  ;"  IF TMGRESULT'>0 WRITE !,!,"ERROR: ",$PIECE(TMGRESULT,"^",2),!
  ;"  QUIT
  ;                                                                  
  ;"SCANPT3(DFN) ;"SCAN 1 PATIENT with 2 methods, and comparing, NO SAVING  <--- depreciated
  ;"  NEW TABLENAME SET TABLENAME="[FINAL MEDICATIONS]"
  ;"  NEW NOTES IF $$LASTNOTE^TMGTIUO5(DFN,.NOTES,TABLENAME)=0 QUIT ;"GET LAST NOTE POINTED TO IN 22729
  ...
  ;"  SET OPTION("USEOLDMETHOD")=1  ;"<-- needed to ensure everything pulled together to current date
  ;"  NEW ARR2 DO MEDARR^TMGTIUOJ(0,DFN,.ARR2,,.OPTION)
...
PICKVIEW  ;"PICK PATIENT, THEN VIEW RECORD
  NEW DIC,X,Y,DFN
  SET DIC=2,DIC(0)="MAEQ" DO ^DIC WRITE ! SET DFN=+Y  
  IF DFN>0 DO VIEW1(DFN)
  QUIT
  ;
VIEW1(DFN) ;"VIEW RECORD FOR 1 PATIENT
  NEW OPTION SET OPTION("NO LOOP")=1
  DO ASKDUMP^TMGDEBU3(22733.2,DFN,.OPTION)
  QUIT
  ;
HANDLTIU(IEN8925)  ;"Evaluate a TIU DOCUMENT for medication table, and save if found
  ...
  NEW DFN SET DFN=$PIECE($GET(^TIU(8925,IEN8925,0)),"^",2) GOTO:DFN'>0 HANDLDN
  NEW ARR IF $$SCANDOC(IEN8925,.ARR)=0 GOTO HANDLDN
  SET TMGRESULT=$$SAVETABL(DFN,.ARR)  ;"SAVE MEDICATION TABLE. 
HANDLDN ;
  QUIT TMGRESULT
  ;
SAVETABL(DFN,ARR,OPTION)  ;"SAVE MEDICATION TABLE.
  ...
  IF $DATA(^TMG(22733.2,DFN))'>0 DO  GOTO:(TMGRESULT'>0) SAVTBLDN
  . NEW TMGFDA,TMGIEN,TMGMSG
  . SET TMGIEN(1)=DFN
  . SET TMGFDA(22733.2,"+1,",.01)=DFN
  . DO UPDATE^DIE("","TMGFDA","TMGIEN","TMGMSG")
  . IF $DATA(TMGMSG("DIERR")) DO  QUIT
  . . SET TMGRESULT="-1^"_$$GETERRST^TMGDEBU2(.TMGMSG)
  ;
  NEW REF SET REF=$NAME(^TMG(22733.2,DFN,"TABLE"))
  NEW KEYVAL MERGE KEYVAL=ARR("KEY-VALUE") KILL ARR("KEY-VALUE")  
  DO ARRAY2WP^TMGSTUT2("ARR",REF)  ;" <-- saves to field 2, TABLE RAW
  ;
  DO
  . NEW TMGFDA,TMGMSG
  . SET TMGFDA(22733.2,DFN_",",.02)=$$NOW^XLFDT
  ...
  NEW DIK SET DIK="^TMG(22733.2,"_DFN_",""RX"","
  NEW DA SET DA=0,DA(1)=DFN
  FOR  SET DA=$ORDER(^TMG(22733.2,DFN,"RX",DA)) QUIT:DA'>0  DO ^DIK  ;"kill prior entries. 
  NEW IDX SET IDX=0   
  ...
  . . SET REF=$NAME(^TMG(22733.2,DFN,"RX",SUBIEN,"PARSED"))
  . . DO ARRAY2WP^TMGSTUT2("TEXT",REF)
  ;
  ;"-- Now store KEY-VALUE pairs
  NEW DIK SET DIK="^TMG(22733.2,"_DFN_",""KV"","
  NEW DA SET DA=0,DA(1)=DFN
  FOR  SET DA=$ORDER(^TMG(22733.2,DFN,"KV",DA)) QUIT:DA'>0  DO ^DIK  ;"kill prior entries. 
  NEW KEY SET KEY=""   
...
SCANDOC(IEN8925,ARR)  ;"Scan document for MEDICATION TABLE or FINAL MEDICATION TABLE, extract it if found 
  KILL ARR
  NEW TABLENAME,TEMP
  NEW DFN SET DFN=$PIECE($GET(^TIU(8925,IEN8925,0)),"^",2) GOTO:DFN'>0 SCDCDN
  ...
  . DO GETRXTBL^TMGRX007(.TEMP,.DT,DFN)  ;"doesn't return table name.
  . SET FOUNDTABLE="[MEDICATIONS]"
  IF $DATA(TEMP),+$G(ERXARR(0))>0 DO
  . NEW CT SET CT=+$ORDER(TEMP(""),-1)
  . NEW IDX SET IDX=0
...
SCDCDN ;  
  QUIT ($DATA(ARR)>0)
  ;
GETRXTBL(OUTARR,OUTDT,DFN)  ;"GET MED TABLE FOR PATIENT from file 22733.2.  
  ...
  ;"Result: none
  SET DFN=+$GET(DFN)
  SET OUTDT=$PIECE($GET(^TMG(22733.2,DFN,0)),"^",2)
  IF $ORDER(^TMG(22733.2,DFN,"KV",0))'>0 DO
  . DO SCANPT^TMGRX007(DFN)  ;"//kt 10/16/18 For some reason, sometimes this is missing, so recreate 
  KILL OUTARR
  SET OUTARR("KEY-VALUE","SOURCE-DATE")=OUTDT
  NEW CT SET CT=0
  NEW SUBIEN SET SUBIEN=0
  FOR  SET SUBIEN=$ORDER(^TMG(22733.2,DFN,"TABLE",SUBIEN)) QUIT:SUBIEN'>0  DO
  . NEW LINE SET LINE=$GET(^TMG(22733.2,DFN,"TABLE",SUBIEN,0))
  . IF LINE["[FINAL MEDICATION" QUIT
  . IF LINE["[MEDICATION" QUIT
  ...
  NEW IDX SET IDX=0
  FOR  SET IDX=$O(^TMG(22733.2,DFN,"KV",IDX)) QUIT:+IDX'>0  DO
  . NEW VALUE,LINE,KEY
  . SET KEY=$P($G(^TMG(22733.2,DFN,"KV",IDX,0)),"^",1)
  . SET VALUE=$P($G(^TMG(22733.2,DFN,"KV",IDX,0)),"^",2)
  . SET LINE=$G(^TMG(22733.2,DFN,"KV",IDX,1))
