...
GTCONSLT(OUT)   ;"Gather all consults data from the TMG_TO TIU Fields
 ...
 QUIT
 ;"
STCONSLT(SPECIEN,OFFICEIEN,NAME,PHONE,FAX)
 ;"This will update a consultant information in 8927.1
 NEW FAXLINE,PHONELINE
 SET FAXLINE="FAX: "_FAX,PHONELINE="PHONE: "_PHONE
 NEW LINE SET LINE=NAME_$J("",22-$L(NAME))_FAXLINE_$J("",39-(22+$L(FAXLINE)))_PHONELINE
 NEW SPECIALTY SET SPECIALTY="TMG_TO_CONSULT_"_$G(^TMG(22762,SPECIEN,0))
 NEW TEMPLIEN SET TEMPLIEN=+$O(^TIU(8927.1,"B",SPECIALTY,0))
 IF TEMPLIEN'>0 QUIT
 NEW SUBIEN SET SUBIEN=0
 FOR  SET SUBIEN=$O(^TIU(8927.1,TEMPLIEN,10,SUBIEN)) QUIT:SUBIEN'>0  DO
 . NEW ONELINE SET ONELINE=$$UP^XLFSTR($G(^TIU(8927.1,TEMPLIEN,10,SUBIEN,0)))
 . IF ONELINE'[NAME QUIT
 . SET ^TIU(8927.1,TEMPLIEN,10,SUBIEN,0)=LINE
 QUIT
 ;"
...
SAVEOFFICE
 ...
 DO FILE^DIE("","TMGFDA","TMGERR") 
 ;"HERE WE NEED TO UPDATE THE ACTUAL 
 DO STCONSLT(SPECIEN,OFFICEIEN,NAME,PHONE,FAX)
 QUIT
 ;"
 SET TMGRESULT="-1^UNKNOWN COMMAND: "_COMMAND
 QUIT
