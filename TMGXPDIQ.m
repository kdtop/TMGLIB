TMGXPDIQ ;TMG/kst/Custom version of XPDIQ ;10/7/08, 2/2/14
         ;;1.0;TMG-LIB;**1**;10/7/08
 ;"Original header....
 ...
 ;
        Q
DIR(XPFR,XPFP,Option)
        ;"Input: XPFR -- prefix
        ;"       XPFP -- file no._# or Mail Group ien
        ;"       Option -- an array for supply answers to user questions, so not asked.
        ;"                  Option(QuestionText)=1 for YES, 0 for NO
        ;"                  Option(QuestionText,"DEFAULT")=default user answer
        ;"NOTE: XPFP is for XPF  or XPM questions
        ...
        . S:$L($G(XPDQUES(XPDJ))) DIR("B")=$G(XPDQUES(XPDJ,"B"),XPDQUES(XPDJ))
        . D  Q:'$D(Y)
        . . N FLAG,X,Z K Y
        ...
        . . IF $DATA(Option(DIR("A"),"DEFAULT")) SET DIR("B")=$GET(Option(DIR("A"),"DEFAULT"))  ;"//KT
        . . K:$G(DIR("B"))="" DIR("B")
        . . KILL Y SET Y=$GET(Option(DIR("A")))
        . . IF Y="" D ^DIR
        . S %=$P(DIR(0),U)
