...
GTHL7MSG(OUT,PATH,FILE,HTML)  ;"retrieve HL7 message into array, in HTML format (optional)
 ...
 IF TMGDEBUG=0 DO
 . KILL ^TMG("TMP","GTHL7MSG^TMGHL7U4")
 . MERGE ^TMG("TMP","GTHL7MSG^TMGHL7U4","PATH")=PATH
 . MERGE ^TMG("TMP","GTHL7MSG^TMGHL7U4","FILE")=FILE
 . MERGE ^TMG("TMP","GTHL7MSG^TMGHL7U4","HTML")=HTML
 ELSE  DO
 . KILL PATH,FILE,HTML
 . MERGE PATH=^TMG("TMP","GTHL7MSG^TMGHL7U4","PATH")
 . MERGE FILE=^TMG("TMP","GTHL7MSG^TMGHL7U4","FILE")
 . MERGE HTML=^TMG("TMP","GTHL7MSG^TMGHL7U4","HTML")
 ; 
 NEW RESULT,TMGMSG,MSH
 SET HTML=+$GET(HTML,1)
 SET RESULT=$$LOADHL7^TMGHL7U2(PATH_FILE,.TMGMSG,.MSH)
 NEW DIV1 SET DIV1="|"  ;"Default
 ;
 NEW INIDX SET INIDX=0
 NEW OUTIDX SET OUTIDX=1
 FOR  SET INIDX=$O(TMGMSG(INIDX)) QUIT:INIDX'>0  DO
 . NEW LINE SET LINE=$GET(TMGMSG(INIDX))
 . IF $EXTRACT(LINE,1,3)="MSG" SET DIV1=$EXTRACT(LINE,4)
 . NEW TYPE SET TYPE=$PIECE(LINE,DIV1,1)
 . IF $$SHOULDGARBLE^TMGMISC4() DO  ;"//kt Check for special mode to hide patient info during demos	
 . . NEW PRIVATE SET PRIVATE=(TYPE="PID")!(TYPE="NK1")
 . . IF PRIVATE SET LINE=TYPE_DIV1_"-- Redacted For Privacy --"
 . IF TYPE="ZEF" QUIT  ;"filter out encoded pdf segments. 
 . SET OUT(OUTIDX)=LINE,OUTIDX=OUTIDX+1
 IF HTML DO
 . SET OUT(0)="<HTML><PRE>"
