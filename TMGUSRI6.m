...
TESTEDITDLG  ;
  ...
  DO SPLITCOLOR2REF^TMGUSRI8(CLR,$NAME(OPTION("COLOR","EDITOR")))
	...
	;
TESTEDITBOX ;
	WRITE "Testing edit box.  Enter text below and then press [ENTER] when done",!
	NEW RESULT SET RESULT=$$EDITBOX("<prefilled text>",40,"_",,,60)  ;"Edit box for editing strings
	WRITE !,RESULT,!
	QUIT
	;
...
EDITBOX2(INITVAL,OPTION)  ;"Edit box for editing strings
	...
	NEW DRAWIDX SET DRAWIDX=1  ;"Index in string of first character to draw on screen during rendering
	NEW CPOS SET CPOS=$LENGTH(VAL)+1  ;"Abs position of the cursor in text (or 1 position after text)
	NEW INPUT,STRA,STRB,CH,DONE,ABORT SET (DONE,ABORT)=0
	NEW LNUMHIDE,RNUMHIDE,LNUMDOTS,RNUMDOTS,RNUMSPC,SCRNCPOS,LENVAL,ESCKEY
LOOP ;
	...
	. IF (ESCKEY="FIND")!(ESCKEY="HOME") DO  QUIT     ;"HOME KEY
	. . SET CPOS=1
	. . IF CPOS<DRAWIDX SET DRAWIDX=CPOS
	. IF (ESCKEY="SELECT")!(ESCKEY="END") DO  QUIT   ;"END KEY
	. . SET CPOS=LENVAL+1
