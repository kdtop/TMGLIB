TMGTIUOD ;TMG/kst-TIU OBJECTS ; 01/12/17, 5/2/18
 ...
 ;"STATNINT(DFN) -- STATIN INTENSITY CLASS INFORMATION 
 ;"
 ;"TEST  
 ;"TESTPTS 
 ;"TESTGRP(ARR) 
...
CVDR(TMGDFN,AODT,TMGTEXT)  ;"10 YR CVD RISK with explaination  
  ...
  SET TMGTEXT=TMGTEXT_"AGE, SEX --> 10 yr CVD risk="_TMGRESULT_"%."
PCRDN2 ;  
  QUIT TMGRESULT
  ;
...
STATNINT(DFN)  ;"STATIN INTENSITY CLASS INFORMATION
  ...
  ;"FOR  SET SUBIEN=$ORDER(^TMG(22733.2,DFN,"RX","ACLASS",CLASSIEN,SUBIEN)) QUIT:(SUBIEN'>0)!(RESULT'="")  DO
  FOR  SET SUBIEN=$ORDER(^TMG(22733.2,DFN,"RX",SUBIEN)) QUIT:(SUBIEN'>0)!(RESULT'="")  DO
  . NEW ZN SET ZN=$GET(^TMG(22733.2,DFN,"RX",SUBIEN,0))  ;"e.g.  26^atorvastatin 40 mg po QDAY^40^58
...
TEST1GRP ;
  ...
  IF +Y'>0 QUIT
  NEW DFN SET DFN=+Y
  NEW GROUP,TEXT SET GROUP=$$RISKGRP^TMGTIUOD(DFN,,.TEXT)
  WRITE !,GROUP,!,TEXT,!
  QUIT 
  ;
TESTGRP(ARR) ;
...
TESTSTN() ;
  ...
  IF +Y'>0 GOTO TSTDN
  NEW DFN SET DFN=+Y  
  SET RESULT=$$STATNINT(DFN)
TSTDN ;  
  QUIT RESULT  
  ;"
URINCULT(DFN,OUTARRAY)  ;"
  NEW TMGRESULT SET TMGRESULT=""
  NEW LABARR
  NEW COUNT,MAX
  SET COUNT=0,MAX=5
  DO GETLABS^TMGLRR02(.LABARR,DFN,$$ADDDAYS^TMGDATE("-1825"),$$TODAY^TMGDATE,.OPTION)
  NEW LABDATE SET LABDATE=9999999
  FOR  SET LABDATE=$O(LABARR("TEST","NAME","URINE CULTURE",LABDATE),-1) QUIT:(LABDATE'>0)!(COUNT>MAX)  DO
  . SET COUNT=COUNT+1
