...
HTMTABLE(DATA) ;"Assemble HTML  table. 
  ...
  . NEW TABLEARR
  . ;"SET TABLETEXT=$$GETTABLX^TMGTIUOJ(+$G(DFN),TBLTAG,.TABLEARR)
  . IF $$GETTABLX^TMGTIUOJ(+$G(DFN),TBLTAG,.TABLEARR)
  . SET TABLETEXT=""
  . NEW IDX SET IDX=""
...
PLNTABLE(DATA) ;"Assemble PLAIN TEXT table. 
  ...
  FOR  SET TABLENAME=$ORDER(DATA("TABIX",TABLENAME)) QUIT:TABLENAME=""  DO
  . NEW TEMP SET TEMP=0
  . NEW TABARR IF $$GETTABLX^TMGTIUOJ(DFN,TABLENAME,.TABARR)  ;"drop result
