...
GETLTOPICS(OUT,ADFN,TERM,SDT,EDT) ;"RPC ENTRY POINT 'TMG CPRS PATIENT TOPIC SEARCH'
  ...
  NEW TERMS DO SPLIT2AR^TMGSTUT2(TERM," ",.TERMS)  ;"2/11/25
  NEW TOPICARR
  ...
  . SET TOPIC=$$UP^XLFSTR(TOPIC) ;"  2/11/25 ,TERM=$$UP^XLFSTR(TERM)
  . ;" 2/11/25IF (TERM="")!(TOPIC[TERM) DO
  . IF (TERM="")!($$ARRCONTS(.TERMS,TOPIC)=1) DO
  . . SET OUT(OUTIDX)=$G(TOPICARR(IDX)),OUTIDX=OUTIDX+1
  QUIT
  ;
ARRCONTS(ARR,STRING) ;"
  NEW TMGRESULT SET TMGRESULT=1  ;"DEFAULT TO TRUE
  NEW TOPICTERMS DO SPLIT2AR^TMGSTUT2(STRING," ",.TOPICTERMS)
  NEW FRAGIDX SET FRAGIDX=0
  FOR  SET FRAGIDX=$O(ARR(FRAGIDX)) QUIT:(FRAGIDX'>0)!(TMGRESULT=0)  DO
  . NEW ONEFRAG SET ONEFRAG=$$UP^XLFSTR($G(ARR(FRAGIDX)))
  . NEW TERMIDX SET TERMIDX=0
  . NEW CONTAINS SET CONTAINS=0  ;"THIS IS THE TEST FOR EACH FRAGMENT
  . FOR  SET TERMIDX=$O(TOPICTERMS(TERMIDX)) QUIT:TERMIDX'>0  DO
  . . NEW ONETERM SET ONETERM=$$UP^XLFSTR($G(TOPICTERMS(TERMIDX)))
  . . IF ONETERM[ONEFRAG SET CONTAINS=1
  . IF CONTAINS=0 SET TMGRESULT=0  ;"ONE MISSING FRAGMENT WILL RESULT IN FALSE RESULT
  QUIT TMGRESULT
  ;"
...
ENCTEST(TMGRESULT,TMGDFN,CPTS,ICDS,HTML) ;"RPC TMG ENC SAVE PRETEST
  ...
  DO INSAWV(.CPTARR,TMGDFN,.MESSAGE,LF)
...
LASTCPE(TMGDFN)
 ...
 QUIT TMGRESULT
 ;"
INSAWV(CPTARR,TMGDFN,MESSAGE,LF)
  NEW AWVBILLED SET AWVBILLED=0
  NEW CODE SET CODE=""
  NEW CPT SET CPT=""
  FOR  SET CPT=$O(CPTARR(CPT)) QUIT:CPT=""  DO
  . IF (CPT["G043") SET AWVBILLED=1,CODE="("_CPT_")"
  IF AWVBILLED'=1 QUIT
  NEW INS SET INS=$$GETPINS(TMGDFN)
  IF INS'="MEDICARE" QUIT
  NEW LASTAWV SET LASTAWV=$$LASTAWV(TMGDFN)
  IF $$DAYSDIFF^TMGDATE(LASTAWV,$$TODAY^TMGDATE)<366 DO
  . DO ADDMSG(.MESSAGE,LF,"CPT "_CODE_" was selected but it appears a AWV was done on "_$$EXTDATE^TMGDATE(LASTAWV,1)_". Medicare requires 365 days between charges.")
  QUIT
  ;"
LASTAWV(TMGDFN)
 ;"Return the date of the last AWV that was billed for the patient
 ;"also return "(MEDICARE)" at the end if they have Medicare
 NEW TMGRESULT SET TMGRESULT="-1"
 NEW AWVARRAY,IEN,VISITIEN,AWVIEN,VISITDATE
 SET IEN=0
 FOR  SET IEN=$O(^AUPNVCPT("C",TMGDFN,IEN)) QUIT:IEN'>0  DO 
 . SET AWVIEN=$PIECE($GET(^AUPNVCPT(IEN,0)),"^",1)
 . IF (AWVIEN'=110219)&(AWVIEN'=110220)&(AWVIEN'=109747) QUIT
 . SET VISITIEN=$PIECE($GET(^AUPNVCPT(IEN,0)),"^",3)
 . SET VISITDATE=$PIECE($GET(^AUPNVSIT(VISITIEN,0)),"^",1)
 . SET VISITDATE=$PIECE(VISITDATE,".",1)
 . IF VISITDATE=$$TODAY^TMGDATE QUIT
 . SET AWVARRAY(VISITDATE)=AWVIEN
 IF $D(AWVARRAY) DO
