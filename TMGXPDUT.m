TMGXPDUT ;TMG/kst/Custom version of XPDUTL ;09/17/08, 2/2/14
...
LAST(PKG,VER) ;returns last patch applied for a Package, PATCH^DATE
 ...
 I $G(VER)="" S VER=$$VERSION(PKG) Q:'VER -1
...
MES(X,Msg)
        ;"Purpose: Record message, X=message or an array passed by reference
        ;"Input:  X -- string of message
        ...
        SET Msg=$GET(Msg,1)
        F %=1:1 Q:'$D(X(%))  do
        . SET Msg(Msg)=X(%),Msg=Msg+1
        Q:'$G(XPDA)
        D WP^DIE(9.7,XPDA_",",20,"A","X")  ;"Store message in file 9.7
        Q

BMES(X,Msg)
        ;"Purpose Add blank line before message
