...
DOWNLOAD(GREF,FPATH,FNAME,LOCIEN)  ;
  ...
  SET FNAME=$GET(FNAME)
  SET LOCIEN=+$GET(LOCIEN)
  IF FNAME["TM0" DO     ;"OLDER IMAGES (AROUND 2010) GOT THE WRONG PREFIX. THIS IS A QUICK FIX TO THOSE  11/4/22
  . SET FNAME=$P(FNAME,"TM0",1)_"TMG"_$P(FNAME,"TM0",2)
  . SET FPATH=$P(FPATH,"TM0",1)_"TMG"_$P(FPATH,"TM0",2)
  IF LOCIEN'>0 SET LOCIEN=$$GETDEFNL()
  SET GREF="^TMP(""DOWNLOAD^TMGRPC1"","_$J_")"
  KILL @GREF
  NEW INITPATH 
  IF $$SHOULDGARBLE^TMGMISC4() DO      ;"check for special mode to hide patient info during demos
  . IF FNAME["KTESIG.JPG" QUIT
  . NEW EXT SET EXT=$$UP^XLFSTR($PIECE(FNAME,".",2))
  . NEW ISIMAGE SET ISIMAGE=(EXT="JPG")!(EXT="JPEG")!(EXT="BMP")!(EXT="ABS")!(EXT="PNG")!(EXT="TIFF")!(EXT="PDF")
  . IF 'ISIMAGE QUIT
  . ;"If garbling, then change all images to refer to 1 image, showing 'confidential'
  . SET FPATH="\TMG0\00\00\00\00"
  . IF EXT="PNG" SET FNAME="TMG00000000000.png"
  . ELSE  IF EXT="ABS" SET FNAME="TMG00000000000.bmp"
  . ELSE  SET FNAME="TMG00000000000.jpg"  
DL1 ;
  SET INITPATH=FPATH
...
LSTALIMG(TMGOUT,ADFN,SDT,EDT,EXCLUDE)  ;"Get listing of all images
  DO GETEXCL(.EXCLUDE)    ;" 2/20/25
  DO LISTALLIMAGES(.TMGOUT,.ADFN,.SDT,.EDT,.EXCLUDE) 
  QUIT
  ;
GETEXCL(EXCLUDE)  ;"This will set the default exclusion notes  2/20/25
  SET EXCLUDE(1424)=""   ;"PMH/ROS QUESTIONAIRE (IMAGE)
  NEW TITLEIDX SET TITLEIDX=0
  FOR  SET TITLEIDX=$O(^TIU(8925.1,TITLEIDX)) QUIT:TITLEIDX'>0  DO
  . NEW TITLENAME SET TITLENAME=$P($G(^TIU(8925.1,TITLEIDX,0)),"^",1)
  . NEW OFFNOTE SET OFFNOTE=$P($G(^TIU(8925.1,TITLEIDX,"TMGH")),"^",1)
  . IF (OFFNOTE'="Y")&(TITLENAME'["PICTURE") SET EXCLUDE(TITLEIDX)=""
  . ;IF TITLENAME["HOSPITAL" SET EXCLUDE(TITLEIDX)=""
  . ;IF TITLENAME["HOME HEALTH" SET EXCLUDE(TITLEIDX)=""
  . ;IF TITLENAME["CONSULTANT" SET EXCLUDE(TITLEIDX)=""
  . ;IF TITLENAME["INSURANCE" SET EXCLUDE(TITLEIDX)=""
  QUIT
  ;"
LISTALLIMAGES(TMGOUT,ADFN,SDT,EDT,EXCLUDE)  ;"Get listing of all images
  ...
  IF $DATA(EXCLUDE) DO FILTERLST(.LIST,.EXCLUDE)
  NEW CT SET CT=1
...
FILTERLST(ARR,EXCLARR)  ;"Remove entries from ARR based on EXCLARR
  ...
  SET IEN8925=0
  FOR  SET IEN8925=$ORDER(ARR(IEN8925)) QUIT:IEN8925'>0  DO
