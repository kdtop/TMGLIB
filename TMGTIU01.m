...
DISPCOS()  ;
        ...
        IF (TMGIEN8925D1=22)&(AUTHOR=TMGDFN) GOTO DCDN    ;"IEN OF MED ADMINISTRATION
        IF (TMGIEN8925D1=2132)&(AUTHOR=TMGDFN) GOTO DCDN  ;"IEN OF HOME BP    EDUCATION
        IF (TMGIEN8925D1=1424)&(AUTHOR=TMGDFN) GOTO DCDN  ;"IEN OF ROS
        IF (TMGIEN8925D1=2207)&(AUTHOR=TMGDFN) GOTO DCDN  ;"IEN OF BRIEF NURSE NOTE
        IF (TMGIEN8925D1=2066)&(AUTHOR=TMGDFN) GOTO DCDN  ;"IEN OF HOME BP READING
        IF (TMGIEN8925D1=2165)&(AUTHOR=TMGDFN) GOTO DCDN  ;"IEN OF NURSE ORDER
        IF (TMGIEN8925D1=2236)&(AUTHOR=TMGDFN) GOTO DCDN  ;"IEN OF REFERRAL NOTE
        IF (TMGIEN8925D1=18)&(AUTHOR=TMGDFN) GOTO DCDN  ;"IEN OF IMMUNIZATION NOTE
        IF (TMGIEN8925D1=207)&(AUTHOR=TMGDFN) GOTO DCDN  ;"IEN OF REQUEST RECORDS TASK NOTE
        ;"IF (TMGIEN8925D1=183)&(AUTHOR=TMGDFN)&(AUTHOR'=168) GOTO DCDN  ;"IEN OF PATIENT TASK
        IF (TMGIEN8925D1=183)&(AUTHOR=TMGDFN) GOTO DCDN  ;"IEN OF PATIENT TASK
        IF (TMGIEN8925D1=221)&(AUTHOR=TMGDFN) GOTO DCDN  ;"IEN OF CRITICAL LAB
DCDN    QUIT
        ;
DISPADD() ;
        ;"Output of function is $TEST:1 means ask for cosigner
...
POSTCODE(TMGRESULT,TMGIEN8925) ;
        NEW TMGIEN8925D1 SET TMGRESULT=""
        SET TMGIEN8925D1=+$PIECE($GET(^TIU(8925,TMGIEN8925,0)),"^",1)
        IF TMGIEN8925D1>0 SET TMGRESULT=$GET(^TIU(8925.1,TMGIEN8925D1,"TMGPSC"))
        QUIT
        ;"      
GOODBP(TMGDFN)  ;"
...
CANSIGN(TMGRESULT,TMGDFN,TIUIEN) ;"
  ...
  NEW TMGWARN SET TMGWARN=""
  NEW PHRASE1 SET PHRASE1="NOTE: ABOVE IS UNEDITED EXAM TEMPLATE."
  NEW PHRASE2 SET PHRASE2="**PHYSICAL EXAM**"
  NEW PHRASE3 SET PHRASE3="[*FOLLOWUP*]"
  NEW PHRASE4 SET PHRASE4="[*LABORDER*]"
  NEW PHRASE5 SET PHRASE5="[^^!"
  NEW PHRASE6 SET PHRASE6="!^^]"
  NEW PHRASE7 SET PHRASE7="MDM_Target"
  NEW PHRASE8 SET PHRASE8="MEDICAL DECISION MAKING ITEM"
  NEW PHRASE9 SET PHRASE9="OPEN SUSPECT MEDICAL CONDITIONS"
  NEW PHRASE10 SET PHRASE10="****CHAPERONE****"
  NEW REFDATE SET REFDATE=$P($G(^TIU(8925,TIUIEN,13)),"^",1)
  SET REFDATE=$P(REFDATE,".",1)
  NEW TODAY SET TODAY=$$TODAY^TMGDATE
  IF REFDATE>TODAY SET TMGERROR="CANNOT SIGN A NOTE THAT IS DATED IN THE FUTURE." 
  ...
  IF (TIUTEXT[PHRASE5)&(TIUTEXT[PHRASE6) DO
  . IF TMGERROR'="" SET TMGERROR=TMGERROR_$C(13,10)
  . NEW TAG SET TAG=$P($P(TIUTEXT,PHRASE5,2),PHRASE6,1)
  . SET TMGERROR=TMGERROR_""""_TAG_""""_" WAS NOT REPLACED IN THE NOTE"
  IF (TIUTEXT[PHRASE7)&(TIUTEXT'[PHRASE8) DO
  . IF TMGERROR="" DO
  . . SET TMGWARN="MDM HELPER DOESN'T SEEM TO BE ENTERED INTO THE NOTE. GO TO 'ACTION' -> 'Launch MDM Helper' TO USE"
  . ELSE  DO
  . . IF TMGERROR'="" SET TMGERROR=TMGERROR_$C(13,10)
  . . SET TMGERROR=TMGERROR_"MDM HELPER DOESN'T SEEM TO BE ENTERED INTO THE NOTE. GO TO 'ACTION' -> 'Launch MDM Helpe' TO USE"
  IF TIUTEXT[PHRASE9 DO
  . IF TMGERROR'="" SET TMGERROR=TMGERROR_$C(13,10)
  . SET TMGERROR=TMGERROR_"NOTE CONTAINS SUSPECT MEDICAL CONDITIONS. THESE MUST BE DELETED BEFORE SIGNING NOTE."
  IF TIUTEXT[PHRASE10 DO
  . IF TMGERROR'="" SET TMGERROR=TMGERROR_$C(13,10)
  . SET TMGERROR=TMGERROR_"CHAPERONE TAG EXISTS IN NOTE. MUST BE REMOVED BEFOR BEING SIGNED."
  IF TMGWARN'="" DO
  . SET TMGRESULT="0^The following error(s) have been returned."_$C(13,10)_$C(13,10)_TMGWARN
  IF TMGERROR'="" DO   ;"TMGERROR should supercede TMGWARN as more serious condition
