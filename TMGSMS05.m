TMGSMS05 ;TMG/kst/OS SMS Message -- Reporting ;1/28/15, 3/24/21
...
AAPLP2 ;  
  QUIT
  ;
RPTDFN(TMGDFN,ERR) ;"Output report for patient
  ;"Input: TMGDFN -- The patient to query.
  ;"       ERR -- PASS BY REFERENCE.  Format: ERR(#)=Message
  NEW ERR2
  SET TMGDFN=+$GET(TMGDFN)
  NEW PHONEARR,ONEPHONE  
  DO GETPHONS^TMGSMS01(TMGDFN,.PHONEARR,.ERR2)
  IF $DATA(ERR2) DO  GOTO RDFNDN
  . DO ADDMSG(.ERR,$GET(ERR2(TMGDFN)))
  . DO SHOWIFER(.ERR)  
...
RPDLP1 ;  
  ...
  SET ONEPHONE=""
  FOR  SET ONEPHONE=$ORDER(PHONEARR(TMGDFN,ONEPHONE)) QUIT:+ONEPHONE'>0  DO
  ...
  . FOR  SET PHONENAME=$ORDER(PHONEARR(TMGDFN,"NAME",PHONENAME)) QUIT:PHONENAME=""  DO
  . . NEW ONENUM SET ONENUM=$GET(PHONEARR(TMGDFN,"NAME",PHONENAME))
  ...
  . FOR  SET SUBIEN=$ORDER(^DPT(TMGDFN,"TMGSMS",SUBIEN)) QUIT:+SUBIEN'>0  DO
  . . DO DUMPREC^TMGDEBU3(2.022705,SUBIEN_","_TMGDFN_",")
  . . SET FOUND=1
  . IF FOUND=0 WRITE "  (none)",!
  . DO PRESS2GO^TMGUSRI2 WRITE !  
  IF USRSLCT="OPT_OUT" DO  GOTO RPDLP1
  . DO OPTOTDFN^TMGSMS02(TMGDFN)
  . DO PRESS2GO^TMGUSRI2 WRITE !  
  IF USRSLCT="PATIENT" DO  GOTO RPDLP1
  . DO DUMPREC^TMGDEBU3(2,TMGDFN_",")
  . DO PRESS2GO^TMGUSRI2 WRITE !  
  IF USRSLCT="APPT" DO  GOTO RPDLP1
  . DO DUMPREC^TMGDEBU3(22723,TMGDFN_",")
  . DO PRESS2GO^TMGUSRI2 WRITE !  
  IF USRSLCT="SMS" DO  GOTO RPDLP1
  . WRITE !,"Information from TMG SMS MESSAGES file:",!
  . NEW FOUND,ABORT SET (FOUND,ABORT)=0 
  . NEW IEN SET IEN=0
  . FOR  SET IEN=$ORDER(^TMG(22724,"DFN",TMGDFN,IEN)) QUIT:(+IEN'>0)!(ABORT)  DO
...
TESTSMS ;
  ...
  DO ^DIC WRITE !
  NEW TMGDFN SET TMGDFN=+Y IF Y'>0 SET Y=0
  DO SNDTSMS^TMGSMS01(TMGDFN,PHONE)
  WRITE "  ... SENT",! 
TSMSDN ;
  DO PRESS2GO^TMGUSRI2 WRITE !
  QUIT
