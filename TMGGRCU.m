...
GETLMS(CHART,MONTHS,GENDER,L,M,S)	       ;
	       ...
	       KILL L,M,S
	       NEW MO1,MO2
	       IF (CHART="13")!(CHART["13^WHO")!(CHART="14")!(CHART["14^WHO") DO  ;"WHO-WEIGHT FOR LENGTH
...
Z2PCTL(Z)	       ;
	       ...
	       NEW $ETRAP SET $ETRAP="G ERRZ2PTL^TMGGRCU"
	       ...
	       IF $$ABS^XLFMTH(P)>200 QUIT -1  ;"Screen for unreasonable percentiles
	       QUIT P
	       ;
ERRZ2PTL	   ; Process Error from Z2Percenticle function.
	       S $ETRAP="D ^%ZTER H"
