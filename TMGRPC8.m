TMGRPC8 ;TMG/kst/RPC Functions  ;08/28/12, 2/2/14
...
NOTIFY(RESULT,LIST)   ;
        ...
        NEW TMGIEN,TMGMSG
        NEW TMGERROR
        NEW DFN,NOTIFIED,SEQUELNUM,TYPE
        NEW TMGRESULTARRAY SET TMGRESULTARRAY=1
        SET RESULT(0)=1
        SET PTNUM=0
        FOR  SET PTNUM=$ORDER(LIST(PTNUM)) QUIT:+PTNUM<1  DO
        . SET DFN=+$PIECE(LIST(PTNUM),"^",1)
        . SET SEQUELNUM=+$PIECE(LIST(PTNUM),"^",2)
        . IF DFN<1 SET DFN=$$GETDFN(SEQUELNUM)
        . IF DFN<1 QUIT
        . SET TYPE="T"
        . SET NOTIFIED=$PIECE(LIST(PTNUM),"^",3)
        . SET TMGERROR=0
        . IF $DATA(^TMG(22716,DFN))=0 DO  QUIT:TMGERROR
        . . SET TMGFDA(22716,"+1,",.01)=DFN
        . . SET TMGIEN(1)=DFN
        ...
        . . . SET TMGERROR=1
        . KILL TMGFDA,TMGIEN
        . SET TMGFDA(22716.01,"+1,"_DFN_",",.01)="NOW"
        . SET TMGFDA(22716.01,"+1,"_DFN_",",.02)=NOTIFIED
        . SET TMGFDA(22716.01,"+1,"_DFN_",",.03)=TYPE
...
GETDFN(SEQUELNUM)    ;
        NEW DFN
        SET DFN=+$ORDER(^DPT("TMGS",SEQUELNUM,""))
        QUIT DFN
