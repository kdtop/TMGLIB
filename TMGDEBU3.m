TMGDEBU3 ;TMG/kst/Debug utilities: logging, record dump ;6/12/17, 12/29/22
...
DUMPREC(FILENUM,IENS,SHOWEMPTY,FIELDSARRAY,OPTION)  ;
        ...
        NEW RECS 
        IF $$ISSUBFIL^TMGFMUT2(FILENUM) DO   ;"//kt 8/26/21 to make work with subfiles properly
        ...
        NEW FLAGS SET FLAGS="ip"_$SELECT($GET(SHOWEMPTY)=1:"b",1:"")
        DO WRIT1FIL^TMGXMLE3(FILENUM,IENS,.RECS,FLAGS,"",SHOWPROG,.WRITERS)
        QUIT
        ;
...
WTFLDLAB(INDENTS,LABEL,FIELD,TYPE,ENDER,PROPS) ;
  ...
  . . NEW SFNUM SET SFNUM=+$GET(PROPS("SUBFILE-NUMBER"))
  . . IF SFNUM>0 WRITE " SUBFILE #",SFNUM
  QUIT
  ;
WTLINE(INDENTS,LINE)  ;
