TMGHL7E2 ;TMG/kst-HL7 Processing Error/Alert handling; 11/18/16, 3/24/21, 5/10/21
  ...
  ;"SETALERT(ERRTEXT,AMSG,IEN772,IEN773) --set up alerts for error handling of Rad filer problems. 
  ;"
  ;"=======================================================================
...
SETALERT(ERRTEXT,AMSG,IEN772,IEN773) ;       
  DO SETALERT^TMGHL7E(.ERRTEXT,.AMSG,.IEN772,.IEN773,"RAD") 
  QUIT
 ;"note: 5/10/21 -- I am merging error handlers back together.  Consider deleting COMMENTED PARTS below after 7/10/21
 ...
 ;"  ;"          HLMTIEN (which is really an IEN772), HLMTIENS (an IEN773) -- NO
 ;"  ;"          TMGHL7MSG
 ;"  ;"Results: NONE:
...
ERRLABEL(TMGHL7MSG)  ;
 ...
 ;"  . . IF ($DATA(^TMG("TMP","TMGHL7E2",JN,DH,"ERROR VARS"))=0)!(+DH<HCUTOFF) DO
 ;"  . . . KILL ^TMG("TMP","TMGHL7E2",JN,DH)
 ;"  . . . KILL ^TMG("TMP","TMGHL7E2","$H",DH,JN)
 ;"  QUIT
 ;"  ;"---------------------------------------------------------------------
 ;"  ;"---------------------------------------------------------------------
 ;"  ;
HNDLERR ;
 ...
 ;"note: 5/10/21 -- Consider deleting COMMENTED PARTS below after 7/10/21
 ;"
 ...
 ;"  IF TMGUSERINPUT="TryAgain" IF +$$TRYAGAIN(.TMGTESTMSG,0,IEN772,IEN773,.TMGENV)=1 GOTO HE2DN
 ;"  IF TMGUSERINPUT="TryAgainDebugger" IF $$TRYAGAIN(.TMGTESTMSG,1,IEN772,IEN773,.TMGENV)=1 GOTO HE2DN
 ;"  IF TMGUSERINPUT="SearchPt" NEW TMGSRCH SET TMGSRCH=0 DO  GOTO HE2DN:TMGSRCH=1,M2
 ;"  . NEW TEMP,TMGHNDLERR
 ;"  . IF $$FINDPT^TMGHL7E(.TEMP,.TMGHL7MSG)=0 QUIT
 ;"  . SET TMGHNDLERR("SSN")=$GET(TEMP("SSN"))
 ;"  . SET TMGHNDLERR("DFN")=$GET(TEMP("DFN"))
 ;"  . NEW TEMP2 SET TEMP2=$$TRYAGAIN(.TMGTESTMSG,0,IEN772,IEN773,.TMGENV)
 ;"  . SET TMGSRCH=+TEMP2
 ...
 ;"  IF $$TRYAGAIN(.TMGTESTMSG,0,IEN772,IEN773,.TMGENV)'=1 DO
 ;"  . DO CLEANUP2(TMGJOBN,TMGTIME) 
 ;"HE2DN  ;
 ...
 ;"        QUIT
 ;"        ;
 ;"TRYAGAIN(TMGMSG,DEBUG,IEN772,IEN773,TMGENV) ;
