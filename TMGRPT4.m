...
NOFUAPPT  ;"PRINT A LIST OF PATIENT SEEN LAST WEEK, WHO DON'T HAVE APPTS SCHEDULED
  ...
  . . IF OVNOTE'="Y" QUIT
  . . NEW DFN SET DFN=$P($G(^TIU(8925,TIUIEN,0)),"^",2)
  . . NEW PHONE SET PHONE=$P($G(^DPT(DFN,.13)),"^",1)
  . . NEW CELL SET CELL=$P($G(^DPT(DFN,.13)),"^",4)
  ...
  . . SET APPTARRAY(DFN)=$P($G(^DPT(DFN,0)),"^",1)_" was seen on "_$$EXTDATE^TMGDATE(DATE,1)_" "_PHONELINE
  ...
  . . . IF (STATUS="A")&($D(APPTARRAY(DFN))) DO
  . . . . ;"SET APPTARRAY(DFN)="HAS APPT ON "_DATE
  . . . . KILL APPTARRAY(DFN)
  IF '$D(APPTARRAY) QUIT
  NEW REPORTARRAY
  SET DFN=0
  FOR  SET DFN=$O(APPTARRAY(DFN)) QUIT:DFN'>0  DO
  . SET REPORTARRAY($G(APPTARRAY(DFN)))=""
  NEW %ZIS
  SET %ZIS("A")="Enter Output Device: "
  SET IOP="S121-LAUGHLIN-LASER"
  DO ^%ZIS  ;"standard device call
