...
PETAGEST(BDATE,EDATE,SUPPRESS)   ;"Physical Exam tag exists
  ...
  NEW PHRASE SET PHRASE="NOTE: ABOVE IS UNEDITED EXAM TEMPLATE."
...
UNSIGNAPPT  ;"
  ...
  . . . WRITE " ",!
  . . . SET HEADER=1
  . . SET MESSAGEARR(MSGIDX)="URGENT: "_$$UP^XLFSTR($P($G(^DPT(TMGDFN,0)),"^",1))_" HAS AN UNSIGNED OFFICE NOTE AND IS SCHEDULED FOR TODAY!"
  . . SET MSGIDX=MSGIDX+1
  ...
  SET MSGIDX=0
  FOR  SET MSGIDX=$O(MESSAGEARR(MSGIDX)) QUIT:MSGIDX'>0  DO
  . NEW TEMPMSG SET TEMPMSG(1)=$G(MESSAGEARR(MSGIDX))
...
UNSIGNTIU  ;"
  ...
  . . IF XTVOBJ'["TIU" QUIT
  . . SET XTVOBJ=$P($P(XTVOBJ,"TIU",2),";",1)
  . . NEW TIUTYPE,TIUHLIGHT,TMGDFN,COLOR
  . . SET TIUTYPE=+$P($G(^TIU(8925,XTVOBJ,0)),"^",1)
  . . IF TIUTYPE'>0 QUIT
  . . SET TMGDFN=$P($G(^TIU(8925,XTVOBJ,0)),"^",2)
  . . SET TIUHLIGHT=$P($G(^TIU(8925.1,TIUTYPE,"TMGH")),"^",1)
